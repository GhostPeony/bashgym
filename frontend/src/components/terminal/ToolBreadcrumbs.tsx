...
function getToolIcon(tool: string) {
  ...
  if (toolLower.includes('edit') || toolLower.includes('write'))
    return <Edit3 className="w-2.5 h-2.5" />
  ...
  if (toolLower.includes('glob') || toolLower.includes('find'))
    return <FolderSearch className="w-2.5 h-2.5" />
...
function getToolColor(tool: string): string {
  ...
  if (toolLower.includes('read'))
    return 'bg-blue-100 text-blue-700 border-blue-700 dark:bg-blue-900 dark:text-blue-300 dark:border-blue-400'
  if (toolLower.includes('edit') || toolLower.includes('write'))
    return 'bg-amber-100 text-amber-700 border-amber-700 dark:bg-amber-900 dark:text-amber-300 dark:border-amber-400'
  if (toolLower.includes('bash') || toolLower.includes('terminal'))
    return 'bg-green-100 text-green-700 border-green-700 dark:bg-green-900 dark:text-green-300 dark:border-green-400'
  if (toolLower.includes('grep') || toolLower.includes('search') || toolLower.includes('glob'))
    return 'bg-purple-100 text-purple-700 border-purple-700 dark:bg-purple-900 dark:text-purple-300 dark:border-purple-400'
  if (toolLower.includes('web'))
    return 'bg-cyan-100 text-cyan-700 border-cyan-700 dark:bg-cyan-900 dark:text-cyan-300 dark:border-cyan-400'
  if (toolLower.includes('todo'))
    return 'bg-pink-100 text-pink-700 border-pink-700 dark:bg-pink-900 dark:text-pink-300 dark:border-pink-400'
  if (toolLower.includes('ask'))
    return 'bg-orange-100 text-orange-700 border-orange-700 dark:bg-orange-900 dark:text-orange-300 dark:border-orange-400'
...
export const ToolBreadcrumbs = memo(function ToolBreadcrumbs({
  ...
  return (
    <div className={clsx('flex items-center gap-1 overflow-hidden font-mono', className)}>
      {hasMore && <span className="text-[10px] text-text-muted font-mono">...</span>}
      {displayItems.map((item, index) => (
        <div key={`${item.tool}-${item.timestamp}`} className="flex items-center gap-1">
          {index > 0 && <ChevronRight className="w-2.5 h-2.5 text-text-muted flex-shrink-0" />}
          <div
            className={clsx(
...
export const ToolBreadcrumbsCompact = memo(function ToolBreadcrumbsCompact({
  ...
  return (
    <div className={clsx('flex items-center gap-0.5', className)}>
      {hasMore && (
        <span className="text-[9px] text-text-muted mr-0.5 font-mono">
