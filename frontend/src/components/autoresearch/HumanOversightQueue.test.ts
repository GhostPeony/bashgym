...
function rawQueue(state: 'pending' | 'claimed' | 'submitted' = 'claimed'): Record<string, unknown> {
  ...
  const raw: Record<string, unknown> = {
    schema_version: 'human_work_queue.v1', workspace_id: WORKSPACE_ID, campaign_id: CAMPAIGN_ID, campaign_revision: 3,
    reviewer: { authenticated: true, review_capability: true },
    items: [{
      work_id: WORK_ID, campaign_revision: 3, version: 2, state, blocking: true,
      rubric: { rubric_id: 'rub_01234567', version: 1, instructions: 'Choose the stronger blinded sample.', choices: [{ choice_id: 'left', label: 'Sample A' }, { choice_id: 'right', label: 'Sample B' }] },
      sample: { prompt: 'Assess both samples.', left: { label: 'Sample A', display: 'Visible blinded sample A.' }, right: { label: 'Sample B', display: 'Visible blinded sample B.' } },
      lease_expires_at: state === 'claimed' ? '2026-07-16T21:00:00.000Z' : null,
      claimed_by_current_reviewer: state === 'claimed',
      claim_idempotency_key: 'idem_claim_0123456789abcdef', submit_idempotency_key: 'idem_submit_0123456789abcdef',
      receipt: submitted ? {
        receipt_id: RECEIPT_ID, workspace_id: WORKSPACE_ID, campaign_id: CAMPAIGN_ID, work_id: WORK_ID,
        campaign_revision: 3, item_version: 2, rubric_version: 1, decision: 'prefer_left',
        sealed_at: '2026-07-16T19:59:00.000Z', receipt_digest: `sha256:${'a'.repeat(64)}`,
      } : null,
    }],
    promotion: {
      state: submitted ? 'awaiting_human_decision' : 'blocked_by_review', version: 5,
...
function model(state: 'pending' | 'claimed' | 'submitted' = 'claimed', overrides: Record<string, unknown> = {}): HumanOversightViewModel {
  ...
  return buildHumanOversightModel({
    ...
    freshness: 'live', error: null, now: NOW,
    ...overrides,
  })
}

function response(view: HumanOversightViewModel, overrides: Partial<HumanReviewResponse> = {}): HumanReviewResponse {
  return {
...
function props(view = model()): HumanOversightQueueProps {
  ...
  return {
    ...
    onResponseChange: () => {}, onClaim: () => {}, onSubmit: () => {}, onPromotion: () => {},
  }
}

type ElementProps = Record<string, unknown> & { children?: ReactNode }

function expand(node: ReactNode, found: ReactElement<ElementProps>[] = []): ReactElement<ElementProps>[] {
...
function byLabel(elements: ReactElement<ElementProps>[], label: string): ReactElement<ElementProps> {
...
test('C3 keys response state by the complete scope and revision binding', () => {
  ...
  assert.notEqual(humanReviewResponseKey(first.items[0]!.responseBinding), humanReviewResponseKey(revised.items[0]!.responseBinding))
})
...
test('C3 accepts only decisions offered by the current rubric and a bounded rationale', () => {
  ...
  for (const answer of [
    ...
    const elements = controls({ ...props(view), responses: { [humanReviewResponseKey(answer)]: answer } })
...
test('I7 invokes submit with a trimmed revision-bound answer and stable replay key', () => {
  ...
  const captured: unknown[] = []
  const elements = controls({
    ...props(view), responses: { [humanReviewResponseKey(answer)]: answer },
  ...
  assert.equal((captured[0] as { submitted: HumanReviewResponse }).submitted.rationale, 'clear reason')
})
...
test('I7 blocks duplicate submit handlers once the controlled mutation is pending', () => {
  ...
  let calls = 0
  const elements = controls({ ...props(view), onSubmit: () => { calls += 1 } })
  const button = byLabel(elements, 'Submit sealed review')
  assert.equal(button.props.disabled, true)
  ;(button.props.onClick as () => void)()
...
test('I7 invokes explicit promote and hold handlers with complete bound requests', () => {
  ...
  const captured: unknown[] = []
  const elements = controls({ ...props(view), responses: {}, onPromotion: (request) => captured.push(request) })
...
test('I7 globally disables handlers while stale, offline, error, or reconciling', () => {
  for (const freshness of ['stale', 'offline', 'error', 'reconciling'] as const) {
    for (const state of ['pending', 'claimed', 'submitted'] as const) {
      ...
      const elements = controls({
        ...props(view),
        onClaim: () => { calls += 1 }, onSubmit: () => { calls += 1 }, onPromotion: () => { calls += 1 },
      })
      for (const label of ['Claim review', 'Submit sealed review', 'Promote with human decision', 'Hold for further review']) {
      ...
      assert.equal(calls, 0)
      const html = renderToStaticMarkup(createElement(HumanOversightQueue, { ...props(view), model: view }))
...
test('I7 suppresses submit after authoritative time advances through lease expiry', () => {
  ...
  assert.equal(elements.some((candidate) => candidate.props.children === 'Submit sealed review'), false)
...
test('I7 exposes current response binding through change handlers', () => {
  ...
  const elements = controls({ ...props(view), responses: {}, onResponseChange: (key, answer) => changes.push({ key, answer }) })
  const select = elements.find((element) => element.type === 'select')
  assert.ok(select)
  ;(select.props.onChange as (event: { target: { value: string } }) => void)({ target: { value: 'prefer_left' } })
