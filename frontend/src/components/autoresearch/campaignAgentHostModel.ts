...
const states = new Set(['eligible', 'registered', 'authorized', 'credential_ready', 'active', 'credential_consumed', 'failed'])
...
function exactDescriptor(value: unknown): value is Record<string, unknown> {
  ...
  return keys.length === 3 && keys[0] === 'family' && keys[1] === 'state' && keys[2] === 'terminalId'
}

export function parseCampaignAgentEligibleSessions(value: unknown): CampaignAgentEligibleSession[] | null {
  if (!Array.isArray(value) || value.length > 32) return null
  const parsed: CampaignAgentEligibleSession[] = []
  const terminalIds = new Set<string>()
  for (const entry of value) {
    if (!exactDescriptor(entry)
      ...
      || terminalIds.has(entry.terminalId)) return null
