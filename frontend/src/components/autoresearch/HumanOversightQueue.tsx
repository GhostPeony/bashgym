...
export interface HumanOversightQueueProps {
  ...
  onSubmit: (
    ...
    response: HumanReviewResponse & { decision: HumanReviewDecision }
  ) => void
  onPromotion: (request: HumanPromotionRequest) => void
}

function responseMatchesBinding(
  response: HumanReviewResponse | undefined,
  binding: HumanReviewResponseBinding
): response is HumanReviewResponse {
  return (
    Boolean(response) &&
...
function ReviewItem({
  ...
  const currentResponse = responseMatchesBinding(candidate, view.responseBinding)
    ? candidate
    : undefined
  ...
  const rationaleValid =
    rationale.trim().length > 0 && rationale.length <= HUMAN_WORK_LIMITS.maxRationale
  const canSubmit =
    model.mutationsEnabled && view.submit !== null && decisionValid && rationaleValid
  ...
  const submit = () => {
    ...
    onSubmit(view.submit, {
      ...
      rationale: currentResponse.rationale.trim()
    })
  }

  return (
    <article
      className="border-t border-border-subtle py-4 first:border-t-0 first:pt-0"
      aria-labelledby={titleId}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 id={titleId} className="font-brand text-lg text-text-primary">
            Blinded review
          </h3>
          <p className="mt-1 text-xs text-text-secondary">
            Version {item.version} · Rubric {item.rubric.rubric_id} v{item.rubric.version}
          </p>
        </div>
        <div
          className={`font-mono text-[10px] font-semibold uppercase tracking-wide ${stateClass(view.effectiveState)}`}
        >
          {readableState(view.effectiveState)}
          {view.invalidated ? ' · Revision changed' : ''}
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        {[item.sample.left, item.sample.right].map((sample) => (
          <section
            key={sample.label}
            className="border border-border-subtle bg-background-secondary p-3"
            aria-label={sample.label}
          >
            <h4 className="font-mono text-[10px] font-semibold uppercase tracking-wide text-text-muted">
              {sample.label}
            </h4>
      ...
      {view.claim || view.effectiveState === 'pending' || view.effectiveState === 'expired' ? (
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <Button
            ...
            size="sm"
            onClick={() => {
              if (model.mutationsEnabled && view.claim) onClaim(view.claim)
            }}
            disabled={!model.mutationsEnabled || !view.claim}
          >
            Claim review
          </Button>
          {view.effectiveState === 'expired' ? (
            <span className="flex items-center gap-1 text-xs text-status-warning">
              <Clock3 className="h-3.5 w-3.5" />
              The previous lease expired; claim the current version.
            </span>
          ) : null}
        </div>
      ) : null}
      ...
      {view.effectiveState === 'claimed' ? (
        <fieldset
          ...
          disabled={!model.mutationsEnabled || !view.submit}
        >
          <legend className="sr-only">Submit blinded review</legend>
          <div className="grid gap-3 md:grid-cols-[minmax(0,220px)_minmax(0,1fr)]">
            <div>
              <label htmlFor={decisionId} className="block text-xs font-semibold text-text-primary">
                Review decision
              </label>
              <select
                id={decisionId}
                ...
                onChange={(event) => {
                  ...
                  updateResponse({
                    decision: view.offeredDecisions.includes(next as HumanReviewDecision)
                      ? (next as HumanReviewDecision)
                      : '',
                    rationale
                  })
                }}
              >
                ...
                {item.rubric.choices.map((choice) => (
                  <option key={choice.choice_id} value={reviewDecisionForChoice(choice.choice_id)!}>
                    {choice.label}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(event) =>
                  updateResponse({
                    decision: selected,
                    rationale: event.target.value.slice(0, HUMAN_WORK_LIMITS.maxRationale)
                  })
                }
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs text-text-muted">
              The response and replay key are bound to this exact campaign, item, and rubric
              revision.
            </span>
          </div>
        </fieldset>
      ...
      {item.receipt ? (
        <p
          className={`mt-3 flex flex-wrap items-center gap-2 text-xs ${view.receiptCurrent ? 'text-text-secondary' : 'text-status-warning'}`}
        >
          {view.receiptCurrent ? (
          ...
          {view.receiptCurrent ? 'Current sealed receipt' : 'Receipt not current'}{' '}
          {item.receipt.receipt_id} · {item.receipt.sealed_at}
        </p>
      ) : null}
    </article>
  )
...
export function HumanOversightQueue({
  model,
  responses,
  onResponseChange,
  onClaim,
    ...
    <section aria-label="Human oversight queue">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <p className="min-w-0 flex-1 text-sm leading-6 text-text-secondary">
          One authenticated desktop reviewer evaluates opaque samples against a versioned rubric.
        </p>
        <div className="flex shrink-0 items-center gap-2 font-mono text-[10px] uppercase tracking-wide text-text-muted">
          <UserRound className="h-4 w-4 text-accent" /> Campaign revision{' '}
          {model.scope.campaignRevision}
        </div>
      </div>

      <div
        className="mt-3 flex items-start gap-2 border-l-2 border-accent bg-background-secondary px-3 py-2 text-xs text-text-secondary"
        role="status"
        aria-live="polite"
      >
      ...
      {model.mutationMessage ? (
        <p className="mt-2 text-xs text-status-warning" role="alert">
          {model.mutationMessage}
        </p>
      ) : null}
      ...
      <div className="mt-4">
        {model.items.length ? (
          ...
          <p className="text-sm text-text-muted">
            No blinded human review work is currently queued.
          </p>
        )}
      </div>
      ...
      <footer className="mt-4 border-t border-border-subtle pt-4">
        <p className="text-sm text-text-primary">
          {model.blocksComparison
            ? 'Comparison remains blocked by human review.'
            : 'No current human-review comparison block.'}{' '}
          {model.blocksPromotion
            ? 'Promotion remains blocked until an explicit human decision.'
            : 'Promotion is not blocked by this queue.'}
        </p>
        {model.promotion || queue.promotion.state === 'awaiting_human_decision' ? (
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <span className="text-xs text-text-secondary">
              A human reviewer must explicitly decide promotion from the current sealed receipt.
            </span>
            <Button
              type="button"
              size="sm"
              onClick={() => decidePromotion('promote')}
              disabled={!model.mutationsEnabled || !model.promotion}
            >
