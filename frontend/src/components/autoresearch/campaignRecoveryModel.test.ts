...
function recovery(overrides: Partial<CampaignRecoveryPublicV1> = {}): CampaignRecoveryPublicV1 {
  ...
  const lineage = {
    ...
    parent_campaign_id: 'autoresearch-smoke-0',
  ...
  const scope = {
    ...
    campaign_revision: lineage.campaign_revision,
    event_cursor: lineage.event_cursor,
    aggregate_version: lineage.aggregate_version
  }
  return {
  ...
  assert.notEqual(
    parseCampaignRecovery(
      recovery({
        ...
        bindings: { ...recovery().bindings, model_display_label: 'default' }
      })
    ),
    null
  )
...
test('strictly parses the bounded latest recovery execution lifecycle and binds it to projection scope', () => {
  ...
  for (const mutate of [
    (value: typeof executing) => {
      value.latest_execution.workspace_id = 'workspace-b'
    },
    (value: typeof executing) => {
      value.latest_execution.status = 'running'
    },
    (value: typeof executing) => {
      Object.assign(value.latest_execution, { outcome_code: 'private_host_failed' })
    },
    (value: typeof executing) => {
      Object.assign(value.latest_execution, { worker_id: 'private-worker' })
    }
  ]) {
...
test('requires current resident consumer authority and blocks duplicate in-flight recovery requests', () => {
  ...
  const unavailable = buildCampaignRecoveryModel({
    snapshot: unowned!,
    freshness: 'live',
    now: NOW
  })
  ...
  assert.equal(
    unavailable.blockers.some((item) => item.code === 'recovery_consumer_unavailable'),
    true
  )
  ...
  assert.equal(
    pending.blockers.some((item) => item.code === 'recovery_request_in_flight'),
    true
  )
...
test('accepts bounded public model labels without blacklisting operator model families', () => {
  for (const label of [
    ...
    assert.notEqual(
      parseCampaignRecovery(
        recovery({ bindings: { ...recovery().bindings, model_display_label: label } })
      ),
      null,
      label
    )
...
test('accepts a registered model without an optional public display label and does not report a missing binding', () => {
  ...
  assert.equal(
    model.blockers.some((item) => item.code === 'missing_binding'),
    false
  )
...
test('rejects additive secret/path fields and unprefixed values in opaque recovery fields', () => {
  ...
  const setters: Array<[string, (copy: CampaignRecoveryPublicV1, canary: string) => void]> = [
    [
      'installation',
      (copy, canary) => {
        copy.installation.installation_id = canary
      }
    ],
    [
      'model',
      (copy, canary) => {
        copy.bindings.model_id = canary
      }
    ],
    [
      'data',
      (copy, canary) => {
        copy.bindings.data_id = canary
      }
    ],
    [
      'evaluator',
      (copy, canary) => {
        copy.bindings.evaluator_id = canary
      }
    ],
    [
      'compute',
      (copy, canary) => {
        copy.bindings.compute_id = canary
      }
    ],
    [
      'checkpoint',
      (copy, canary) => {
        copy.lineage.checkpoint_id = canary
      }
    ],
    [
      'artifact',
      (copy, canary) => {
        copy.lineage.artifact_id = canary
      }
    ],
    [
      'schema',
      (copy, canary) => {
        copy.lineage.schema_version = canary
      }
    ],
    [
      'lease',
      (copy, canary) => {
        copy.controller.lease_id = canary
      }
    ],
    [
      'doctor evidence',
      (copy, canary) => {
        copy.doctor.evidence_id = canary
      }
    ],
    [
      'receipt',
      (copy, canary) => {
        copy.receipts[0]!.receipt_id = canary
      }
    ]
  ]
...
test('rejects incompatible controller state, owner, and lease combinations', () => {
  ...
  for (const controller of invalid)
    assert.equal(
      parseCampaignRecovery(
        recovery({ controller: controller as CampaignRecoveryPublicV1['controller'] })
      ),
      null
    )
})
...
test('rejects out-of-order, duplicate, and unbounded receipt or evidence arrays', () => {
  ...
  assert.equal(
    parseCampaignRecovery(recovery({ receipts: [...value.receipts, value.receipts[1]!] })),
    null
  )
  assert.equal(
    parseCampaignRecovery(
      recovery({ receipts: Array.from({ length: 33 }, () => value.receipts[0]!) })
    ),
    null
  )
  assert.equal(
    parseCampaignRecovery(
      recovery({
        doctor: { ...value.doctor, checks: [...value.doctor.checks, value.doctor.checks[0]!] }
      })
    ),
    null
  )
...
test('gates mutation for all five freshness states', () => {
  const expected: Array<[RecoveryFreshness, boolean, string]> = [
    ['live', true, 'resumable'],
    ['reconciling', false, 'degraded'],
    ['stale', false, 'read_only'],
    ['offline', false, 'read_only'],
...
test('treats an invalid authority time as unverified rather than bypassing receipt expiry', () => {
  const model = buildCampaignRecoveryModel({
    snapshot: recovery(),
    freshness: 'live',
    now: 'not-a-timestamp'
  })
  assert.equal(model.mutationsEnabled, false)
  assert.equal(
    model.blockers.some((item) => item.code === 'eligibility_unverified'),
    true
  )
})
...
test('enables recovery only from a complete, current, latest eligibility receipt bound to exact scope and doctor evidence', () => {
  ...
  const cases: Array<[string, CampaignRecoveryPublicV1]> = [
    ...
    [
      'wrong workspace',
      recovery({ eligibility: { ...base.eligibility!, workspace_id: 'workspace-b' } })
    ],
    [
      'wrong campaign',
      recovery({ eligibility: { ...base.eligibility!, campaign_id: 'autoresearch-other-1' } })
    ],
    ...
    [
      'wrong evidence',
      recovery({ eligibility: { ...base.eligibility!, doctor_evidence_id: REF.otherEvidence } })
    ],
    [
      'wrong evidence digest',
      recovery({ eligibility: { ...base.eligibility!, doctor_evidence_digest: DIGEST_B } })
    ],
    [
      'not latest',
      recovery({
    ...
    [
      'incomplete doctor',
      recovery({ doctor: { ...base.doctor, checks: base.doctor.checks.slice(0, 4) } })
    ]
  ]
...
test('derives an explicit blocker when current eligibility has no controller-compatible permitted action', () => {
  for (const [name, allowedActions] of [
    ['empty', []],
    ['owned cannot takeover', ['takeover']]
  ] as Array<[string, RecoveryAction[]]>) {
    const base = recovery()
    const snapshot = recovery({
      eligibility: { ...base.eligibility!, allowed_actions: allowedActions }
    })
    const model = buildCampaignRecoveryModel({ snapshot, freshness: 'live', now: NOW })
    assert.equal(model.mutationsEnabled, false, name)
    assert.equal(
      model.blockers.some((item) => item.code === 'no_permitted_recovery_action'),
      true,
      name
    )
  }
...
test('constructs exact workspace-scoped resume and repair requests only after confirmation with a fresh unused idempotency key', () => {
  ...
  for (const [action, idempotencyKey] of [
    ['resume', REF.idempotencyResume],
    ['repair', REF.idempotencyRepair]
  ] as Array<[RecoveryAction, string]>) {
    assert.equal(
      buildRecoveryRequest({ model, action, humanConfirmed: false, idempotencyKey }),
      null
    )
    assert.deepEqual(
      buildRecoveryRequest({ model, action, humanConfirmed: true, idempotencyKey }),
      { action, idempotencyKey, ...expected }
    )
  }
  assert.equal(
    buildRecoveryRequest({
  ...
  assert.equal(
    buildRecoveryRequest({
      ...
      action: 'resume',
      humanConfirmed: true,
      idempotencyKey: REF.idempotencyUsed
    }),
    null
  )
})
...
test('keeps takeover visible but disabled when an expired or foreign controller cannot prove a ready consumer', () => {
  ...
  assert.equal(
    buildRecoveryRequest({
      ...
      idempotencyKey: REF.idempotencyTakeover
    }),
    null
  )
  assert.equal(
  ...
  assert.equal(
    buildCampaignRecoveryModel({ snapshot: foreign, freshness: 'live', now: NOW }).actions.takeover
      .enabled,
    false
  )
})
