...
function publicView(attached = false) {
  return parseCampaignAgentPublicView({
    ...
    attachment: attached ? {
      ...
      attachment_id: 'attachment-1', attachment_version: 3, status: 'attached',
      ...
      provenance: {
        agent_family: 'codex', agent_origin: 'desktop-safe-origin', agent_origin_status: 'verified',
        session_id: 'desktop-safe-session', agent_principal_id: 'desktop-safe-principal',
        attached_at: '2026-07-16T20:00:30Z', attached_by: 'human-operator',
        grant_receipt_id: 'grant-receipt-7', grant_receipt_digest: `sha256:${'a'.repeat(64)}`,
        credential_issued_at: '2026-07-16T20:00:30Z', credential_expires_at: '2026-07-16T21:00:30Z',
        credential_status: 'active', credential_revocation_revision: 3,
        credential_status_source: 'campaign_authority', liveness: 'live', resume_cursor: null,
        revoked_at: null, revoked_by: null,
      },
      receipts: [{
        receipt_id: 'attach-receipt-1', kind: 'attach', actor_id: 'human-operator',
...
function props(overrides: Partial<CampaignAgentSessionPanelProps> = {}): CampaignAgentSessionPanelProps {
  return {
...
function findByLabel(node: ReactNode, label: string): ReactElement {
  if (isValidElement(node)) {
    ...
    for (const child of Children.toArray(nodeProps.children)) {
      try { return findByLabel(child, label) } catch { /* keep walking */ }
    }
  }
...
test('keeps a compact read-only shell and guided steps visible for offline, error, loading, and empty host states', () => {
  for (const [hostState, hostError] of [
    ['loading', null], ['offline', 'Desktop host unavailable.'], ['error', 'Host request failed.'], ['ready', null],
  ] as const) {
    const eligibleSessions = hostState === 'ready' ? [] : props().eligibleSessions
    const tree = CampaignAgentSessionPanel(props({ hostState, hostError, eligibleSessions, selectedTerminalId: '' }))
    const html = renderToStaticMarkup(tree)
    assert.match(html, /Campaign agent session/)
    assert.match(html, /Campaign capabilities/)
    assert.match(html, /No eligible Codex terminal|Desktop host unavailable|Host request failed|Discovering the desktop campaign-agent host/)
    assert.match(html, /Launch Codex/)
    assert.match(html, /Register/)
    assert.match(html, /Approve scope/)
    assert.match(html, /Activate/)
    for (const label of ['Register selected session', 'Approve campaign attachment', 'Activate campaign agent']) {
      assert.equal(interactive(findByLabel(tree, label)).disabled, true, `${hostState}: ${label}`)
    }
    assert.equal(interactive(findByLabel(tree, 'Launch Codex campaign agent')).disabled, hostState !== 'ready')
  }
})
...
test('exposes an explicit register then approve then activate progression', () => {
  ...
  const registered = CampaignAgentSessionPanel(props({
  ...
  const attached = CampaignAgentSessionPanel(props({
  ...
  const retry = CampaignAgentSessionPanel(props({
    eligibleSessions: [{ terminalId: 'terminal-codex-1', family: 'codex', state: 'credential_ready' }],
    publicView: publicView(true),
  }))
  const active = CampaignAgentSessionPanel(props({
...
test('fails closed for stale campaign state, failed sessions, unsupported capabilities, and mismatched scope', () => {
  const cases: Partial<CampaignAgentSessionPanelProps>[] = [
    ...
    { publicView: parseCampaignAgentPublicView({
      schema_version: 'campaign_agent_public_view.v1', observed_at: '2026-07-16T20:01:00Z',
      scope: { workspace_id: 'workspace-a', campaign_id: 'campaign-other' }, attachment: null, audit_events: [],
    }) },
  ]
...
test('changes only terminal selection and supported read-only capability declarations', () => {
  ...
  const tree = CampaignAgentSessionPanel(props({
    onSelectTerminal: (terminalId) => { selected = terminalId },
    onRequestedCapabilityChange: (capability, checked) => { requested.push([capability, checked]) },
    onGrantedCapabilityChange: (capability, checked) => { granted.push([capability, checked]) },
  }))
  interactive(findByLabel(tree, 'Eligible live terminal')).onChange?.({ target: { value: 'terminal-hermes-2' } })
  interactive(findByLabel(tree, 'Request artifact read')).onChange?.({ target: { checked: true } })
  interactive(findByLabel(tree, 'Grant campaign observe')).onChange?.({ target: { checked: false } })
...
test('revoke stays visible but requires a live authoritative attachment', () => {
  ...
  const orphaned = CampaignAgentSessionPanel(props({
    publicView: publicView(true), eligibleSessions: [], selectedTerminalId: '',
  }))
  const stale = CampaignAgentSessionPanel(props({
    publicView: publicView(true), freshness: 'stale',
    eligibleSessions: [{ terminalId: 'terminal-codex-1', family: 'codex', state: 'authorized' }],
  }))
...
test('shows the credential lifecycle without exposing credential material', () => {
  const ready = renderToStaticMarkup(createElement(CampaignAgentSessionPanel, props({
    eligibleSessions: [{ terminalId: 'terminal-codex-1', family: 'codex', state: 'credential_ready' }],
    publicView: publicView(true),
  })))
  const consumed = renderToStaticMarkup(createElement(CampaignAgentSessionPanel, props({
    eligibleSessions: [{ terminalId: 'terminal-codex-1', family: 'codex', state: 'credential_consumed' }],
    publicView: publicView(true),
  })))
  const active = renderToStaticMarkup(createElement(CampaignAgentSessionPanel, props({
    eligibleSessions: [{ terminalId: 'terminal-codex-1', family: 'codex', state: 'active' }],
    publicView: publicView(true),
  })))
