...
test('balances incomplete final rows across the full dashboard width', () => {
  assert.deepEqual(
    [0, 1, 2].map((index) => getGridColumnSpan(index, 3)),
    [2, 2, 4]
  )
  assert.deepEqual(
    [0, 1, 2, 3, 4].map((index) => getGridColumnSpan(index, 5)),
    [2, 2, 2, 3, 3]
  )
  assert.equal(getGridColumnSpan(6, 7), 6)
  assert.deepEqual(
    [6, 7].map((index) => getGridColumnSpan(index, 8)),
    [3, 3]
  )
...
test('every grid row consumes all available tracks', () => {
  for (let total = 1; total <= 18; total += 1) {
    ...
    for (let start = 0; start < total; start += columns) {
      ...
      const rowSpan = Array.from({ length: end - start }, (_, offset) =>
        getGridColumnSpan(start + offset, total)
