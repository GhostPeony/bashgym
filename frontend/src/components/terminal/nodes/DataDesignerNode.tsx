...
export const DataDesignerNode = memo(function DataDesignerNode({
  data,
  selected
}: NodeProps<DataDesignerNodeType>) {
  ...
  const configuredJobId =
    nodeConfig.designerJobId || nodeConfig.runtimeJobId || nodeConfig.lastJobId
  const displayJobs = useMemo(() => {
    ...
    if (
      !configuredJobId ||
      !nodeConfig.status ||
      merged.some((job) => job.job_id === configuredJobId)
    ) {
      return merged
    }
    return [
  ...
  const selectedPct =
    selectedJob?.progress && selectedJob.progress.total > 0
  ...
  const selectedCanPublish = Boolean(
    selectedJob && selectedJob.status === 'completed' && selectedJob.output_dir
  )
  ...
  const generateDataset = async () => {
    ...
    try {
      ...
      setJobs((current) =>
        [job, ...current.filter((candidate) => candidate.job_id !== job.job_id)].slice(0, 6)
      )
      saveDraft(job)
      ...
      setActionError(
        caught instanceof Error ? caught.message : 'Unable to start Data Designer job.'
      )
    } finally {
      setGenerating(false)
    }
  ...
  return (
    <>
      <DataNodeShell
        ...
        statusBarClass={
          displayJobs.some((j) => j.status === 'failed')
            ? 'bg-status-error'
            : displayJobs.some((j) => j.status === 'running' || j.status === 'queued')
        ...
        {!loaded && displayJobs.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {displayJobs.map((job) => {
              ...
              const canSend =
                data.hasTerminalConnections && job.status === 'completed' && !!job.output_dir
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5 text-[10px] font-mono">
                    ...
                    {canSend && (
                      <button
                        ...
                        className={clsx(
                          'nodrag node-btn',
                          sentJobId === job.job_id ? 'node-btn-success' : 'node-btn-accent'
                        )}
                    ...
                    <div className="mt-1.5 grid grid-cols-1 gap-1 border-t border-border-subtle pt-1.5 font-mono text-[9px] text-text-muted">
                      ...
                      >
                        ...
                        <span className="truncate">
                          {job.model || job.provider || 'Provider default'}
                        </span>
                      </span>
                  ...
                  {job.error && (
                    <div
                      className="text-[9px] font-mono text-status-error mt-0.5 truncate"
                      title={job.error}
                    >
                      {job.error}
                    </div>
      ...
      <NodeConfigModal
        ...
        footer={
          <div className="flex w-full items-center justify-between gap-3">
            <span className="min-w-0 font-mono text-[10px] text-status-error">{actionError}</span>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, numRecords: Number(event.target.value) }))
                }
              />
            </label>
            <label className="node-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, seedType: event.target.value }))
                }
              >
            ...
            {draft.seedType === 'agent_rollouts' ? (
              <label className="node-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, seedFormat: event.target.value }))
                  }
                >
            ...
            <label className="node-field md:col-span-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, seedSource: event.target.value }))
                }
            ...
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, textModel: event.target.value }))
                }
            ...
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, codeModel: event.target.value }))
                }
                placeholder="Provider default"
              />
            </label>
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, judgeModel: event.target.value }))
                }
                placeholder="Provider default"
              />
            </label>
            {draft.pipeline === 'mcp_tool_use' ? (
              <label className="node-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, mcpBackend: event.target.value }))
                  }
                >
            ...
            <label className="flex items-center gap-2 border-brutal border-border-subtle bg-background-card px-3 py-2 md:col-span-2">
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, keepOnlyPassing: event.target.checked }))
                }
              />
              <span className="font-mono text-[11px] text-text-secondary">
                Keep only passing generated records
              </span>
            </label>
          </div>
        </ConfigSection>
          ...
          <>
            ...
            <ConfigSection title="Run Details">
              <ConfigRows>
                <ConfigRow
                  label="Dataset"
                  value={selectedJob.dataset || selectedRuntimeJob?.dataset}
                />
                <ConfigRow
                  label="Model"
                  value={
                    selectedJob.model ||
                    selectedRuntimeJob?.model ||
                    selectedJob.provider ||
                    selectedRuntimeJob?.provider
                  }
                />
                <ConfigRow
                  label="Provider"
                  value={selectedJob.provider || selectedRuntimeJob?.provider}
                />
                <ConfigRow
                  label="Started"
                  value={formattedDate(selectedJob.started_at || selectedRuntimeJob?.started_at)}
                />
                <ConfigRow
                  label="Completed"
                  value={formattedDate(selectedRuntimeJob?.completed_at)}
                />
                <ConfigRow
                  label="Output"
                  value={selectedJob.output_dir || selectedRuntimeJob?.output_dir}
                />
                <ConfigRow
                  label="Hugging Face"
                  value={
                    nodeConfig.publishedDatasets?.[selectedJob.job_id] ? (
                      <button
                        type="button"
                        className="inline-flex items-center gap-1 text-accent hover:text-accent-dark"
                        onClick={() =>
                          window.open(
                            nodeConfig.publishedDatasets?.[selectedJob.job_id].url,
                            '_blank'
                          )
                        }
                      >
        ...
        {publishJob ? (
          <>
            <ConfigSection title="Destination">
              <div className="flex flex-wrap gap-1.5">
                {linkedHuggingFace.length ? (
                  linkedHuggingFace.map((target) => (
                    <ConfigPill key={target.panelId} tone="accent">
                      {target.title}
                    </ConfigPill>
                  ))
                ) : (
            ...
            <ConfigSection title={publishResult ? 'Published' : 'Source'}>
              <ConfigRows>
                ...
                <ConfigRow
                  label="Records"
                  value={publishJob.progress?.current || publishJob.num_records}
                />
