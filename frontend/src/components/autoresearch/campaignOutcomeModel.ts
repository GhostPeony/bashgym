...
function completedOutcome(
  ...
  const preferred =
    role === 'candidate' && bestProposalId
      ? eligible.filter((item) => item.result.proposal_id === bestProposalId)
      : eligible
  return newestBy(
    preferred.length ? preferred : eligible,
    (item) => item.result.recorded_at,
    (item) => item.result.result_id
  )
}
...
function findEvaluation(
  ...
  if (explicitId) {
    const explicit = evaluations.find(
      (evaluation) => evaluation.evaluation_result_id === explicitId
    )
    if (explicit) return explicit
  }
  if (!outcome) return undefined
  const byEvidence = evaluations.filter((evaluation) =>
    outcome.result.evidence_references.includes(evaluation.evaluation_result_id)
  )
  if (byEvidence.length)
    return newestBy(
      byEvidence,
      (item) => item.created_at,
      (item) => item.evaluation_result_id
    )
  const byRun = evaluations.filter((evaluation) =>
    outcome.result.attempt_ids.includes(evaluation.run_id)
  )
  return newestBy(
    byRun,
    (item) => item.created_at,
...
function baselineLedgerEvaluation(
  ...
  for (const project of detail.ledger?.projects || []) {
    const baselineRunIds = new Set(
      project.runs
        .filter((run) => run.run_kind === 'baseline' && run.is_simulation !== true)
        .map((run) => run.run_id)
    )
    candidates.push(
      ...project.evaluations.filter((evaluation) => baselineRunIds.has(evaluation.run_id))
    )
  }
  return newestBy(
    candidates,
...
function candidateLedgerEvaluation(
  detail: CampaignDetailState
): CampaignLedgerEvaluation | undefined {
  const candidates: CampaignLedgerEvaluation[] = []
  for (const project of detail.ledger?.projects || []) {
...
function candidateLooksLikeLora(
  detail: CampaignDetailState,
  candidate: CampaignAutoResearchOutcome | undefined
): boolean {
  ...
  const proposal = (detail.ledger?.autoresearch?.proposals || []).find(
    (item) =>
...
function projectLoss(
  ...
  const attemptId = [
    ...new Set([
      ...preferredAttemptIds,
      ...trainingAttemptIds,
      ...Object.keys(detail.lossByAttempt)
    ])
...
function lifecycleLabel(detail: CampaignDetailState): string {
  ...
  if (
    autoresearch?.next_action === 'stop' &&
    !['completed', 'exhausted', 'failed', 'cancelled'].includes(detail.campaign.status)
  ) {
...
export function projectCampaignOutcome(
  ...
  const candidateOutcome = completedOutcome(
    outcomes,
    'candidate',
    autoresearch?.state.best_proposal_id
  )
  const evaluations = allEvaluations(detail)
  const baselineEvaluation =
    findEvaluation(evaluations, baselineOutcome) || baselineLedgerEvaluation(detail)
  const candidateEvaluation =
    findEvaluation(
      evaluations,
      candidateOutcome,
      detail.snapshot?.candidate?.latest_comparable_evaluation_id
  ...
  if (!Object.keys(baselineMetrics).length && !Object.keys(candidateMetrics).length && !loss)
    return null

  const primaryMetricId =
    autoresearch?.spec.primary_metric ||
  ...
  const sameEvaluationSuite = comparisonValidity(
    baselineEvaluation,
    candidateEvaluation,
    baselineOutcome,
    candidateOutcome
  ...
  const decision =
    candidateOutcome?.decision.decision || autoresearch?.state.latest_decision || null
  const verdict =
    sameEvaluationSuite === false
      ? 'pending'
      : decision === 'keep'
        ? 'success'
  ...
  const checkpointWarning =
    autoresearch?.diagnostics?.signals.find(
      (signal) => signal.code === 'checkpoint_evidence_missing' || signal.severity === 'critical'
    )?.summary || null
  ...
  return {
    ...
    candidateLabel: candidateLooksLikeLora(detail, candidateOutcome)
      ? 'LoRA candidate'
      : 'Candidate',
    ...
    evaluationSuiteId:
      sameEvaluationSuite === true
        ? baselineEvaluation?.evaluation_suite_id ||
          candidateEvaluation?.evaluation_suite_id ||
          null
        : null,
    sameEvaluationSuite,
    metrics,
