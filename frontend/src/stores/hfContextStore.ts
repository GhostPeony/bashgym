...
import { hfApi, type HFContextBundle, type HFContextEvalPreview } from '../services/api'
...
interface HFContextState {
  ...
  prepareEval: (
    workspaceId: string,
    bundle: HFContextBundle
  ) => Promise<HFContextEvalPreview | null>
}
...
export async function pollHFContextBundle(
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    if (!response.ok || !response.data) {
      const error = new Error(
        response.error || 'Unable to reconcile Hugging Face discovery'
  ...
  throw new Error(
    'Hugging Face discovery is still collecting. You can close this view and return later.'
  )
}
...
export const useHFContextStore = create<HFContextState>((set, get) => ({
          ...
          response.ok && response.data
            ? {
                ...
                active: response.data.active
                  ? {
                      bundle_id: response.data.active.bundle_id,
                      version: response.data.active.version
                    }
                  : null,
            ...
            : {
                ...(state.workspaces[workspaceId] || emptyWorkspace()),
                loading: false,
                error: response.error || 'Unable to load Hugging Face context',
  ...
  discover: async (workspaceId, intent, task, origin) => {
    const response = await hfApi.discoverContext({
    ...
    if (!response.ok || !response.data) {
      set((state) => ({
        workspaces: {
          ...
          [workspaceId]: {
            ...(state.workspaces[workspaceId] || emptyWorkspace()),
            error: response.error || 'Discovery failed',
            errorCode: response.code || null
          }
        }
      }))
    ...
    try {
      const ready = await pollHFContextBundle(response.data, () =>
    ...
    if (!response.ok || !response.data) {
      set((state) => ({
        workspaces: {
          ...
          [workspaceId]: {
            ...(state.workspaces[workspaceId] || emptyWorkspace()),
            error: response.error || 'Unable to pin context',
            errorCode: response.code || null
          }
        }
      }))
    ...
    if (!response.ok || !response.data) {
      set((state) => ({
        workspaces: {
          ...
          [workspaceId]: {
            ...(state.workspaces[workspaceId] || emptyWorkspace()),
            error: response.error || 'Unable to refresh context',
            errorCode: response.code || null
          }
        }
      }))
    ...
    if (!response.ok || !response.data) {
      set((state) => ({
        workspaces: {
          ...
          [workspaceId]: {
            ...(state.workspaces[workspaceId] || emptyWorkspace()),
            error: response.error || 'Unable to cancel discovery',
            errorCode: response.code || null
          }
        }
      }))
    ...
    if (!response.ok) {
      set((state) => ({
        workspaces: {
          ...
          [workspaceId]: {
            ...(state.workspaces[workspaceId] || emptyWorkspace()),
            error: response.error || 'Unable to activate context',
            errorCode: response.code || null
          }
        }
      }))
    ...
    if (!response.ok) {
      set((state) => ({
        workspaces: {
          ...
          [workspaceId]: {
            ...(state.workspaces[workspaceId] || emptyWorkspace()),
            error: response.error || 'Unable to deactivate context',
            errorCode: response.code || null
          }
        }
      }))
  ...
  projection: async (workspaceId, bundle) => {
    ...
    set((state) => ({
      workspaces: {
        ...
        [workspaceId]: {
          ...(state.workspaces[workspaceId] || emptyWorkspace()),
          error: response.error || 'Unable to project active Hugging Face context',
          errorCode: response.code || null
        }
      }
    }))
    const error = new Error(
      response.error || 'Unable to project active Hugging Face context'
    ) as Error & { code?: string }
    error.code = response.code
    throw error
  },
  ...
  prepareEval: async (workspaceId, bundle) => {
    ...
    set((state) => ({
      workspaces: {
        ...
        [workspaceId]: {
          ...(state.workspaces[workspaceId] || emptyWorkspace()),
          error: response.error || 'Unable to prepare Eval preview',
          errorCode: response.code || null
        }
      }
    }))
