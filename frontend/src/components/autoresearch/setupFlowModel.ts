...
const stepDetails: Record<SetupStepId, Pick<SetupStepViewModel, 'label' | 'guidance'>> = {
  ...
  model: {
    ...
    guidance: 'Choose an explicit trainable model identity and immutable revision. No model is selected or downloaded automatically.',
  },
  data: {
    label: 'Data',
  ...
  compute: {
    ...
    guidance: 'Select a registered local or private SSH compute binding; no alternate compute target is selected implicitly.',
  },
  budget_stops: {
    label: 'Budget & stops',
  ...
  activation_doctor: {
    ...
    guidance: 'Bind the registered SSH device and executor profile, bring the resident controller online, then retain the latest server doctor receipt.',
  },
  campaign_creation: {
...
function requiredIdentifier(value: string): string | null {
  ...
  return SAFE_IDENTIFIER.test(value) ? null : 'Use a safe logical identifier (letters, numbers, dot, colon, underscore, or hyphen).'
}
...
const editorDefinitions: Partial<Record<DurableSetupStepId, readonly EditorFieldDefinition[]>> = {
  source: [
    { id: 'source_profile_id', label: 'Approved source profile ID', placeholder: 'registered-source-profile-id', validate: requiredIdentifier },
  ],
  model: [
    { id: 'model_ref', label: 'Trainable model reference', placeholder: 'operator-selected immutable model reference', validate: requiredReference },
    { id: 'model_revision', label: 'Immutable content revision', placeholder: '40 or 64 character revision', validate: (value) => IMMUTABLE_REVISION.test(value) ? null : 'Enter an exact 40 or 64 character content revision.' },
  ],
  data: [
    { id: 'dataset_version_id', label: 'Dataset version ID', placeholder: 'ledger-dataset-version-id', validate: requiredIdentifier },
  ],
  evaluator: [
    { id: 'evaluation_suite_id', label: 'Evaluation suite ID', placeholder: 'ledger-evaluation-suite-id', validate: requiredIdentifier },
    { id: 'primary_metric', label: 'Exact primary metric ID', placeholder: 'primary-metric-id', validate: requiredIdentifier },
  ],
  compute: [
    { id: 'compute_profile_id', label: 'Registered private compute profile ID', placeholder: 'registered-private-compute-profile-id', validate: requiredIdentifier },
  ],
  budget_stops: [
    { id: 'budget_unit', label: 'Budget unit', placeholder: 'gpu_hours', validate: requiredIdentifier },
    { id: 'budget_limit', label: 'Budget limit', placeholder: 'bounded limit', inputMode: 'decimal', validate: positiveNumber },
    { id: 'max_attempts', label: 'Maximum attempts', placeholder: 'bounded count', inputMode: 'numeric', validate: positiveInteger },
    { id: 'minimum_improvement', label: 'Minimum improvement', placeholder: 'required delta', inputMode: 'decimal', validate: positiveNumber },
  ],
  activation_doctor: [
    { id: 'ssh_device_id', label: 'Registered SSH device ID', placeholder: 'registered-ssh-device-id', validate: requiredIdentifier },
    { id: 'executor_profile_id', label: 'Executor profile ID', placeholder: 'installation-executor-profile-id', validate: requiredIdentifier },
  ],
  campaign_creation: [
    { id: 'template_id', label: 'Installed template ID', placeholder: 'installation-template-id', validate: requiredIdentifier },
    { id: 'campaign_id', label: 'Campaign ID', placeholder: 'operator-chosen-campaign-id', validate: requiredIdentifier },
  ],
}

function editorFor(stepId: DurableSetupStepId, drafts: SetupEditorDraft): SetupStepEditorViewModel | null {
...
function validReceipt(receipt: SetupReceipt): boolean {
  return SAFE_IDENTIFIER.test(receipt.receiptId)
...
function firstReceiptFor(stepId: DurableSetupStepId, receipts: readonly SetupReceipt[]): SetupReceipt | null {
  const safe = receipts.filter((receipt) => receipt.stepId === stepId && validReceipt(receipt))
  return safe.find((receipt) => receipt.status === 'complete')
    ?? safe[0]
    ?? null
}

function hasInvalidReceiptFor(stepId: DurableSetupStepId, receipts: readonly SetupReceipt[]): boolean {
...
const actionGuidance: Record<DurableSetupStepId, string> = {
  source: 'Use `bashgym campaign setup-autoresearch` in the workspace terminal. Desktop definition writes do not yet have a durable bridge contract.',
  model: 'Use `bashgym campaign setup-autoresearch` in the workspace terminal. Desktop definition writes do not yet have a durable bridge contract.',
  ...
  evaluator: 'Use `bashgym campaign setup-autoresearch` in the workspace terminal. Desktop definition writes do not yet have a durable bridge contract.',
  compute: 'Use `bashgym campaign activate-autoresearch` in the workspace terminal. Private device and compute material stay installation-owned.',
  budget_stops: 'Use `bashgym campaign setup-autoresearch` in the workspace terminal. Desktop definition writes do not yet have a durable bridge contract.',
  activation_doctor: 'Use `bashgym campaign activate-autoresearch`, then `bashgym campaign doctor`, in the workspace terminal. No durable desktop activation or doctor contract is mounted.',
  campaign_creation: 'Mount the authenticated `campaign.create_from_template` contract, or run `bashgym campaign create` in the workspace terminal.',
  baseline_review: 'Use the authenticated campaign baseline contract from an existing campaign. This pre-campaign setup shell cannot invent baseline state.',
}
...
function startReason(doctor: ServerDoctorReadiness | null): string | null {
  ...
  if (!doctor.controllerIdentityVerified) return 'Verify the controller execution identity with the server doctor.'
  if (!doctor.computeIdentityVerified) return 'Verify the registered compute identity with the server doctor.'
...
export function buildSetupFlowModel(input: SetupFlowInput): SetupFlowViewModel {
  ...
  const hasCompletedDurableSetup = durableSteps.every((step) => firstReceiptFor(step, input.receipts)?.status === 'complete')
  const start = { enabled: hasCompletedDurableSetup && startReason(input.doctor) === null, reason: startReason(input.doctor) }
  ...
  durableSteps.forEach((id) => {
    ...
    else if (receipt?.status === 'failed' || blockers.some((blocker) => blocker.stepId === id)) statuses.set(id, 'blocked')
  ...
  const steps = setupStepOrder.map((id): SetupStepViewModel => {
    ...
    if (id === 'start') {
      ...
      return {
        ...
        actionGuidance: nextStepId === 'start' ? start.reason : 'Complete the durable setup receipts before reviewing Start readiness.',
      }
    }
    const receipt = firstReceiptFor(id, input.receipts)
    ...
    const actionAvailable = action !== null && (
      isLocalRemediation
    ...
    return {
      ...
      receipt,
      action,
      actionReceiptId: action === 'resume' || action === 'retry' ? receipt?.receiptId ?? null : null,
      mutationContract,
      actionAvailable,
      actionGuidance: sequence === 'locked' && lockedBy
  ...
  return {
    steps,
    blockers,
    optionalIntegrations: (input.optionalIntegrations ?? []).map((integration) => ({ ...integration, required: false })),
    nextStepId,
    start,
  }
}
