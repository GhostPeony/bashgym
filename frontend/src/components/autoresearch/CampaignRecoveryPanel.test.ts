...
const REF = {
  installation: `ins_${'1'.repeat(32)}`, model: `mdl_${'2'.repeat(32)}`, data: `dat_${'3'.repeat(32)}`,
  evaluator: `evl_${'4'.repeat(32)}`, compute: `cpu_${'5'.repeat(32)}`, checkpoint: `ckpt_${'6'.repeat(32)}`,
  artifact: `art_${'7'.repeat(32)}`, schema: `sch_${'8'.repeat(32)}`, lease: `lease_${'9'.repeat(32)}`,
  expiredLease: `lease_${'a'.repeat(32)}`, foreignLease: `lease_${'b'.repeat(32)}`, evidence: `evd_${'c'.repeat(32)}`,
  doctorReceipt: `rcpt_${'d'.repeat(32)}`, eligibilityReceipt: `rcpt_${'e'.repeat(32)}`,
  idempotencyResume: `idem_${'1'.repeat(32)}`, idempotencyRepair: `idem_${'2'.repeat(32)}`, idempotencyTakeover: `idem_${'3'.repeat(32)}`,
}

function recovery(overrides: Partial<CampaignRecoveryPublicV1> = {}): CampaignRecoveryPublicV1 {
  const scope = { workspace_id: 'workspace-a', campaign_id: 'autoresearch-smoke-1', campaign_revision: 12, event_cursor: 453, aggregate_version: 19 }
  const doctor = { evidence_id: REF.evidence, evidence_digest: DIGEST_A, checks: [
    { check: 'binding_verified', status: 'pass' }, { check: 'checkpoint_available', status: 'pass' },
    { check: 'compute_reachable', status: 'pass' }, { check: 'schema_compatible', status: 'pass' }, { check: 'lease_observed', status: 'pass' },
  ] } as CampaignRecoveryPublicV1['doctor']
  return {
    schema_version: 'campaign_recovery.v1', workspace_id: scope.workspace_id, campaign_id: scope.campaign_id,
    ...
    lineage: { checkpoint_id: REF.checkpoint, artifact_id: REF.artifact, parent_campaign_id: null, campaign_revision: 12, event_cursor: 453, aggregate_version: 19, schema_version: REF.schema },
    controller: { state: 'owned', owner_status: 'current_controller', lease_id: REF.lease },
    compute: { availability: 'reachable', integration_label: null }, artifacts: { checkpoint: 'available', artifact: 'available' },
    ...
    consumed_idempotency_keys: [], ...overrides,
  }
}

function props(overrides: Partial<CampaignRecoveryPanelProps> = {}): CampaignRecoveryPanelProps {
  return {
    snapshot: recovery(), freshness: 'live', now: NOW,
    confirmations: { resume: false, repair: false, takeover: false },
    idempotencyKeys: { resume: REF.idempotencyResume, repair: REF.idempotencyRepair, takeover: REF.idempotencyTakeover },
    onConfirmationChange: () => {}, onRequest: () => {}, ...overrides,
  }
}
...
test('renders a registered model identity when its optional public display label is absent', () => {
  ...
  const html = render({ snapshot: { ...source, bindings: { ...source.bindings, model_display_label: null } } })
  assert.match(html, new RegExp(REF.model))
...
test('parses unknown panel input and withholds every unsafe value in a fully visible invalid read-only shell', () => {
  const unsafe = { ...recovery(), bindings: { ...recovery().bindings, model_id: 'mdl_ghp_visible_secret', compute_id: 'cpu_cades-macbook' }, ssh_private_key: 'ghp_visible_secret' }
...
test('withholds unsafe model display labels at the panel boundary', () => {
  for (const label of ['ghp_0123456789secret', 'sk-proj-0123456789secret', 'sk_live_0123456789secret', 'AKIAIOSFODNN7EXAMPLE', 'xoxb-0123456789-secret', 'AIza0123456789abcdefghijklmnop', 'ssh-private-key', 'BEGIN PRIVATE KEY', 'C:\\Users\\Cade', 'ssh://private-host', 'cades-macbook', 'dev-laptop', 'home-workstation']) {
    const html = render({ snapshot: recovery({ bindings: { ...recovery().bindings, model_display_label: label } }) })
...
test('withholds embedded credential tokens and collapsed device names from panel rendering', () => {
  for (const label of ['Qwen ghp_0123456789secret', 'Gemma sk-proj-0123456789secret', 'Model AKIAIOSFODNN7EXAMPLE', 'Qwen xoxb-0123456789-secret', 'Model AIza0123456789abcdefghijklmnop', 'CadesMacBookPro', 'CadesLaptop', 'HomeWorkstation']) {
    const html = render({ snapshot: recovery({ bindings: { ...recovery().bindings, model_display_label: label } }) })
    assert.match(html, /Recovery projection unavailable/, label)
    assert.equal(html.includes(label), false, label)
    assert.match(html, /disabled=""/, label)
...
test('renders an explicit no-action blocker and never green success copy for empty or controller-incompatible actions', () => {
  for (const allowedActions of [[], ['takeover'] as RecoveryAction[]]) {
    const base = recovery()
    const html = render({ snapshot: recovery({ eligibility: { ...base.eligibility!, allowed_actions: allowedActions } }) })
    assert.match(html, /No recovery action in the current eligibility receipt is compatible with the controller state/)
...
test('invokes repair only after controlled confirmation and emits the exact scope-bound request', () => {
  ...
  const unconfirmed = CampaignRecoveryPanel(props({ onRequest: (request) => requests.push(request) }))
  click(unconfirmed, 'Request repair')
  assert.equal(requests.length, 0)
  const confirmed = CampaignRecoveryPanel(props({ confirmations: { resume: false, repair: true, takeover: false }, onRequest: (request) => requests.push(request) }))
  click(confirmed, 'Request repair')
  assert.deepEqual(requests, [{
    action: 'repair', idempotencyKey: REF.idempotencyRepair, workspaceId: 'workspace-a', campaignId: 'autoresearch-smoke-1', eligibilityReceiptId: REF.eligibilityReceipt, doctorEvidenceId: REF.evidence,
    expectedCampaignRevision: 12, expectedEventCursor: 453, expectedAggregateVersion: 19, expectedControllerLeaseId: REF.lease,
    checkpointId: REF.checkpoint, artifactId: REF.artifact, humanConfirmed: true,
  }])
})
...
test('keeps confirmed takeover disabled when the backend cannot prove a ready recovery consumer', () => {
  ...
  click(CampaignRecoveryPanel(props({ snapshot: expired, confirmations: { resume: false, repair: false, takeover: true }, onRequest: (request) => requests.push(request) })), 'Take over expired lease')
  assert.equal(requests.length, 0)
  ...
  click(CampaignRecoveryPanel(props({ snapshot: foreign, confirmations: { resume: false, repair: false, takeover: true }, onRequest: (request) => requests.push(request) })), 'Take over expired lease')
  assert.equal(requests.length, 0)
})
