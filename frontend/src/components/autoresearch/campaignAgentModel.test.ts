...
function confirmationFor(
  ...
  return {
    ...
    grantedCapabilities,
    capabilityDigest: capabilityBindingDigest(
...
function createDraft(overrides: Partial<CampaignAgentAttachDraft> = {}): CampaignAgentAttachDraft {
  ...
  return {
    ...merged,
    confirmation:
      overrides.confirmation === undefined ? confirmationFor(merged) : overrides.confirmation
  }
}
...
function revokedPublicPayload(): Record<string, unknown> {
  ...
  ;(attachmentPayload(payload).receipts as Array<Record<string, unknown>>).push({
    receipt_id: 'revoke-receipt-1',
    kind: 'revoke',
    actor_id: 'human-operator-8',
    occurred_at: '2026-07-16T20:00:50Z',
    idempotency_key: 'revoke-attachment-1',
...
function reattachedPublicPayload(): Record<string, unknown> {
  ...
  ;(attachmentPayload(payload).receipts as Array<Record<string, unknown>>).push({
    receipt_id: 'reattach-receipt-1',
    kind: 'attach',
    actor_id: 'human-operator-9',
    occurred_at: '2026-07-16T20:00:55Z',
    idempotency_key: 'reattach-campaign-a-01',
...
test('uses explicit least-privilege capabilities and rejects every prohibited authority action', () => {
  ...
  for (const action of [
    ...
    assert.match(
      validateAttachDraft(
        createDraft({ requestedCapabilities: [action as never] }),
        selectedScope,
        now
      )[0],
      /prohibited|supported/i
    )
  }
})
...
test('requires a current server grant receipt bound to a distinct human and exact draft authority', () => {
  ...
  for (const mutation of mutations) {
    assert.match(
      validateAttachDraft({ ...createDraft(), ...mutation }, selectedScope, now).join(' '),
      /selected campaign|selected workspace|current draft|subset/i
    )
  }
...
test('rejects principal-only substitution that retains a receipt issued for another agent', () => {
  ...
  assert.match(
    validateAttachDraft(substituted, selectedScope, now).join(' '),
    /exact current draft/i
  )
  assert.throws(
    () => buildAttachRequest(selectedScope, substituted, now, null),
    /exact current draft/i
  )
})
...
test('runtime-validates the complete server grant receipt and requires a public distinct human identity', () => {
  ...
  assert.equal(
    parseCampaignAgentGrantConfirmation(valid).humanPrincipal.principalId,
    'human-operator-7'
  )
  for (const principalId of [
    ...
    'h'.repeat(129),
    '../private/human',
    '//server/private/human'
  ]) {
    const draft = createDraft()
    draft.confirmation = { ...valid, humanPrincipal: { principalId, principalType: 'human' } }
    assert.match(
      validateAttachDraft(draft, selectedScope, now).join(' '),
      /human principal|public value|between 1 and 128/i,
  ...
  assert.throws(
    () => parseCampaignAgentGrantConfirmation(malformedNested),
    /human principal.*object/i
  )
})
...
test('binds attach construction to the authoritative selected scope and blocks a second live attachment', () => {
  ...
  assert.throws(
    () => buildAttachRequest({ ...selectedScope, campaignId: 'campaign-b' }, draft, now, null),
    /selected campaign/i
  )
  assert.throws(
    () => buildAttachRequest({ ...selectedScope, workspaceId: 'workspace-b' }, draft, now, null),
    /selected workspace/i
  )
  const liveAttachment = parseCampaignAgentPublicView(publicPayload()).attachment
  assert.ok(liveAttachment)
  assert.throws(
    () => buildAttachRequest(selectedScope, draft, now, liveAttachment),
    /already attached/i
  )
})
...
test('rejects absolute paths and high-confidence credential canaries after bounded decoding', () => {
  for (const canary of [
    ...
    'artifact=/home/operator/private/key.pem',
    'artifact=%2fhome%2foperator%2fprivate%2fkey.pem',
...
test('accepts only fixed audit message codes with unique observed chronology', () => {
  ...
  ;(unknownCode.audit_events as Array<Record<string, unknown>>)[0].message_code =
    'artifact=/private/leak'
  assert.throws(
    () => parseCampaignAgentPublicView(unknownCode),
    /message code|sensitive public value/i
  )

  const mismatchedCode = publicPayload()
  ;(mismatchedCode.audit_events as Array<Record<string, unknown>>)[0].message_code =
    'liveness_changed'
  assert.throws(
    () => parseCampaignAgentPublicView(mismatchedCode),
    /message code.*kind|kind.*message code/i
  )
  ...
  ;(duplicateSequence.audit_events as Array<Record<string, unknown>>).push({
    ...
    sequence: 19,
    kind: 'liveness',
    occurred_at: '2026-07-16T20:00:40Z',
    message_code: 'liveness_changed'
  })
  assert.throws(
    () => parseCampaignAgentPublicView(duplicateSequence),
    /duplicate.*sequence|unique.*sequence/i
  )

  const futureEvent = publicPayload()
  ;(futureEvent.audit_events as Array<Record<string, unknown>>)[0].occurred_at =
    '2026-07-16T20:01:01Z'
...
test('accepts only exact canonical UTC timestamps across public and grant inputs', () => {
  for (const timestamp of [
    ...
    assert.throws(
      () => parseCampaignAgentPublicView(payload),
      /canonical UTC timestamp/i,
      timestamp
    )
  }
  assert.match(
    validateAttachDraft(createDraft(), selectedScope, 'not-a-date').join(' '),
    /canonical current time/i
  )
})
...
test('enforces confirmation issue, current-time, and expiry ordering', () => {
  ...
  futureIssued.confirmation = confirmationFor(futureIssued, {
    issuedAt: '2026-07-16T20:01:00Z',
    expiresAt: '2026-07-16T20:30:00Z'
  })
  assert.match(
    validateAttachDraft(futureIssued, selectedScope, now).join(' '),
    /not yet issued|future/i
  )
  ...
  assert.match(
    validateAttachDraft(zeroWindow, selectedScope, now).join(' '),
    /issued before it expires/i
  )
...
test('enforces the closed credential, liveness, revocation, and temporal matrix', () => {
  const invalidCases: Array<{ name: string; mutate: (payload: Record<string, unknown>) => void }> =
    [
      {
        name: 'active-revoked-liveness',
        mutate: (payload) => {
          provenancePayload(payload).liveness = 'revoked'
        }
      },
      {
        name: 'expired-live-liveness',
        mutate: (payload) => {
      ...
      {
        name: 'expired-before-observation',
        mutate: (payload) => {
          provenancePayload(payload).credential_status = 'expired'
      ...
      {
        name: 'issued-after-attached',
        mutate: (payload) => {
          provenancePayload(payload).credential_issued_at = '2026-07-16T20:00:40Z'
        }
      },
      {
        name: 'attached-after-observed',
        mutate: (payload) => {
          provenancePayload(payload).attached_at = '2026-07-16T20:02:00Z'
        }
      },
      {
        name: 'expiry-before-issued',
        mutate: (payload) => {
          provenancePayload(payload).credential_expires_at = '2026-07-16T20:00:00Z'
        }
      },
      {
      ...
      {
        name: 'revoked-with-live-liveness',
        mutate: (payload) => {
          attachmentPayload(payload).status = 'revoked'
          provenancePayload(payload).credential_status = 'revoked'
          provenancePayload(payload).liveness = 'live'
  ...
  for (const invalidCase of invalidCases) {
    const payload = invalidCase.name.startsWith('revoked')
      ? revokedPublicPayload()
      : publicPayload()
    invalidCase.mutate(payload)
    assert.throws(
      () => parseCampaignAgentPublicView(payload),
      /credential|liveness|temporal|revok|expired/i,
      invalidCase.name
    )
  }
  ...
  assert.equal(
    parseCampaignAgentPublicView(expired).attachment?.provenance.credentialStatus,
    'expired'
  )
})
...
test('reconciles duplicate attach and revoke responses by server version and idempotency receipt', () => {
  ...
  const attachMutation = {
    ...
    actorId: 'human-operator-7',
    baseAttachmentVersion: null
  }
  assert.equal(
    reconcileAuthoritativeAttachment(selectedScope, null, attached, attachMutation),
    attached
  )
  assert.equal(
    reconcileAuthoritativeAttachment(selectedScope, attached, attached, attachMutation),
    attached
  )

  const revoked = parseCampaignAgentPublicView(revokedPublicPayload()).attachment!
  const revokeMutation = {
    idempotencyKey: 'revoke-attachment-1',
    kind: 'revoke' as const,
    actorId: 'human-operator-8',
    baseAttachmentVersion: 3
  }
  const reconciled = reconcileAuthoritativeAttachment(
    selectedScope,
    attached,
    revoked,
    revokeMutation
  ...
  assert.equal(
    reconcileAuthoritativeAttachment(selectedScope, reconciled, revoked, revokeMutation),
    reconciled
  )
  assert.throws(
    () => reconcileAuthoritativeAttachment(selectedScope, revoked, attached, attachMutation),
    /older attachment version/i
  )
  assert.throws(
    () =>
      buildRevokeRequest(selectedScope, revoked, {
        actorId: 'human-operator-8',
        idempotencyKey: 'revoke-attachment-2'
      }),
    /already revoked/i
  )
})
...
test('builds and reconciles one immutable attach-revoke-reattach lifecycle with exact replay', () => {
  ...
  assert.deepEqual(
    reattached.receipts.map((receipt) => receipt.kind),
    ['attach', 'revoke', 'attach']
  )
  ...
  assert.equal(
    buildAttachRequest(selectedScope, createDraft(), now, null).baseAttachmentVersion,
    null
  )
  ...
  const reconciledRevoked = reconcileAuthoritativeAttachment(
    selectedScope,
    reconciledAttached,
    revoked,
    {
  ...
  assert.equal(
    reconcileAuthoritativeAttachment(selectedScope, reconciledRevoked, reattached, mutation),
    reattached
  )
  assert.equal(
    reconcileAuthoritativeAttachment(selectedScope, reattached, reattached, mutation),
    reattached
  )
  assert.throws(
    () =>
      reconcileAuthoritativeAttachment(selectedScope, revoked, reattached, {
        ...mutation,
        baseAttachmentVersion: null
      }),
...
test('requires reattach credentials to renew after revoke without revision rollback', () => {
  ...
  assert.throws(
    () => parseCampaignAgentPublicView(issuedBeforeRevoke),
    /credential.*issued.*revoke|revoke.*credential.*issued/i
  )
...
test('reconciliation binds selected scope and exact current mutation receipt before version handling', () => {
  ...
  for (const field of ['campaign_id', 'workspace_id'] as const) {
    const payload = publicPayload()
    ;(payload.scope as Record<string, unknown>)[field] =
      field === 'campaign_id' ? 'campaign-b' : 'workspace-b'
    const substituted = parseCampaignAgentPublicView(payload).attachment!
    assert.throws(
      () => reconcileAuthoritativeAttachment(selectedScope, attached, substituted, mutation),
      /incoming.*selected (campaign|workspace)/i
    )
    assert.throws(
      () => reconcileAuthoritativeAttachment(selectedScope, substituted, attached, mutation),
      /current.*selected (campaign|workspace)/i
    )
  }
  ...
  assert.throws(
    () =>
      reconcileAuthoritativeAttachment(selectedScope, attached, revoked, {
        idempotencyKey: 'attach-campaign-a-01',
        kind: 'attach',
        actorId: 'human-operator-7',
        baseAttachmentVersion: null
...
test('reconciliation rejects incoherent history and compares the complete equal-version attachment', () => {
  ...
  ;(attachmentPayload(duplicateKey).receipts as Array<Record<string, unknown>>)[1].idempotency_key =
    'attach-campaign-a-01'
  ...
  ;(
    attachmentPayload(wrongVersion).receipts as Array<Record<string, unknown>>
  )[1].attachment_version = 3
  assert.throws(
    () => parseCampaignAgentPublicView(wrongVersion),
    /monotonic|current attachment version/i
  )
  ...
  assert.throws(
    () =>
      reconcileAuthoritativeAttachment(selectedScope, attached, changed, {
        idempotencyKey: 'attach-campaign-a-01',
        kind: 'attach',
        actorId: 'human-operator-7',
        baseAttachmentVersion: null
...
test('reconciles a newer rolling receipt window by immutable overlap after lifetime compaction', () => {
  ...
  ;(attachmentPayload(currentPayload).receipts as Array<Record<string, unknown>>).unshift(
    {
      receipt_id: 'attach-receipt-old',
      kind: 'attach',
      actor_id: 'human-operator-1',
      occurred_at: '2026-07-16T19:59:30Z',
      idempotency_key: 'attach-old',
      attachment_version: 1,
      receipt_digest: `sha256:${'1'.repeat(64)}`
    },
    {
      receipt_id: 'revoke-receipt-old',
      kind: 'revoke',
      actor_id: 'human-operator-2',
  ...
  assert.equal(
    reconcileAuthoritativeAttachment(selectedScope, current, incoming, {
      idempotencyKey: 'reattach-campaign-a-01',
      kind: 'attach',
      actorId: 'human-operator-9',
      baseAttachmentVersion: 4
...
test('rejects receipt window metadata that hides or invents authoritative history', () => {
  for (const mutation of [{ from_version: 2 }, { through_version: 4 }, { has_earlier: false }]) {
    const payload = publicPayload()
...
test('binds current transition receipt actors to provenance and reconciliation intent', () => {
  ...
  ;(attachmentPayload(wrongAttachActor).receipts as Array<Record<string, unknown>>)[0].actor_id =
    'human-impostor'
  assert.throws(
    () => parseCampaignAgentPublicView(wrongAttachActor),
    /attach receipt actor.*attachedBy/i
  )

  const wrongRevokeActor = revokedPublicPayload()
  ;(attachmentPayload(wrongRevokeActor).receipts as Array<Record<string, unknown>>)[1].actor_id =
    'human-impostor'
  assert.throws(
    () => parseCampaignAgentPublicView(wrongRevokeActor),
    /revoke receipt actor.*revokedBy/i
  )

  const attached = parseCampaignAgentPublicView(publicPayload()).attachment!
  assert.throws(
    () =>
      reconcileAuthoritativeAttachment(selectedScope, null, attached, {
        idempotencyKey: 'attach-campaign-a-01',
        kind: 'attach',
        actorId: 'human-impostor',
        baseAttachmentVersion: null
      }),
    /expected actor/i
  )
...
test('builds exact-scope revoke payloads only for live authoritative attachments', () => {
  ...
  assert.throws(
    () =>
      buildRevokeRequest({ ...selectedScope, campaignId: 'campaign-b' }, attached, {
        actorId: 'human-operator-8',
        idempotencyKey: 'revoke-attachment-1'
      }),
    /selected campaign/i
  ...
  assert.throws(
    () =>
      buildRevokeRequest({ ...selectedScope, workspaceId: 'workspace-b' }, attached, {
        actorId: 'human-operator-8',
        idempotencyKey: 'revoke-attachment-1'
      }),
    /selected workspace/i
