...
export interface CampaignAgentHostRendererAPI {
  ...
  activate: (request: { terminalId: string; workspaceId: string; campaignId: string }) => Promise<CampaignAgentHostResult>
  revoke: (request: { terminalId: string; workspaceId: string; campaignId: string }) => Promise<CampaignAgentHostResult>
}

export type CampaignAgentLaunchResult =
  | { success: true; terminalId: string; cwd: string }
  | { success: false; error: string }
...
export async function launchCodexCampaignAgent(
  ...
  let value: unknown
  try {
    value = await host.launch(scope.cwd === undefined ? {
  ...
  try {
    ...
    if (record.success === false && exactKeys(record, ['success', 'error']) && typeof record.error === 'string') {
