...
def call(url, messages, max_tokens=200):
    start = time.time()
    try:
        r = httpx.post(f"{url}/chat/completions", json={"messages": messages, "max_tokens": max_tokens, "temperature": 0.1}, timeout=120)
        elapsed = time.time() - start
        if r.status_code != 200:
            return {"error": r.status_code, "elapsed": elapsed}
        d = r.json()
        return {"content": d["choices"][0]["message"]["content"], "tokens": d["usage"]["completion_tokens"], "elapsed": elapsed, "tps": d["usage"]["completion_tokens"] / elapsed if elapsed > 0 else 0}
...
print("=" * 60)
speed_prompts = ["List Python files in the current directory.", "Read README.md", "Search for 'main' in all files.", "Create test.py with hello world", "Run pytest"]
...
# Tool prediction
print(f"\n{'=' * 60}")
print(f"TOOL PREDICTION (30 val examples)")
print("=" * 60)
...
out = Path.home() / "bashgym" / "data" / "evaluation_results" / f"benchmark_3way_{int(time.time())}.json"
