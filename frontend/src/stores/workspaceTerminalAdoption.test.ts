...
function resetToWorkspaceB(): void {
  ...
  useTerminalStore.setState({
    panels: [panel],
    sessions: new Map([['terminal-b', terminalSession('terminal-b', panel.title, 'C:\\workspace-b')]]),
...
function adoptionAction(): WorkspaceTerminalAdoption {
  const action = (useWorkspaceStore.getState() as unknown as {
...
test('persists a late launch only to its original background workspace and materializes it on return', () => {
  ...
  assert.deepEqual(useTerminalStore.getState().panels.map((panel) => panel.terminalId), ['terminal-b'])
  ...
  assert.equal(loadWorkspaceSnapshot('workspace-b').panels.some(
    (panel) => panel.terminalId === terminal.terminalId,
  ), false)

  assert.equal(adopt('workspace-a', terminal), true)
  assert.equal(loadWorkspaceSnapshot('workspace-a').panels.filter(
    (panel) => panel.terminalId === terminal.terminalId,
  ).length, 1)
  assert.equal(useWorkspaceStore.getState().sessionIndexVersion, 1)
  ...
  assert.equal(materialized.panels.some((panel) => panel.terminalId === terminal.terminalId), true)
...
test('adopts normally in the active workspace and focuses the real existing panel on replay', () => {
  ...
  assert.equal(adopt('workspace-a', terminal), true)
  const createdPanel = useTerminalStore.getState().panels.find(
    (panel) => panel.terminalId === terminal.terminalId,
  )
  assert.ok(createdPanel)
  assert.equal(useTerminalStore.getState().sessions.get(terminal.terminalId)?.cwd, terminal.cwd)
  assert.equal(useTerminalStore.getState().sessions.get(terminal.terminalId)?.launchCommand, undefined)

  useTerminalStore.setState({ activePanelId: null, activeSessionId: null })
  assert.equal(adopt('workspace-a', terminal), true)
  assert.equal(useTerminalStore.getState().panels.filter(
    (panel) => panel.terminalId === terminal.terminalId,
  ).length, 1)
...
test('fails closed when the launch workspace was deleted before adoption completes', () => {
  ...
  assert.equal(adopt('workspace-deleted', {
