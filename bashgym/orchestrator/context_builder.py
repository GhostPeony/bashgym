...
from bashgym.orchestrator.models import TaskNode, WorkerResult
...
class WorkerContextBuilder:
    ...
    def __init__(
        ...
        spec_title: str,
        file_conflicts: list[tuple[str, str, list[str]]] | None = None,
    ):
        """
        Args:
    ...
    def build_claude_md(self, task: TaskNode) -> str:
        ...
        if siblings:
            ...
            parts.append("")
            parts.append("| Task | Title | Files | Status | Relationship |")
            parts.append("|------|-------|-------|--------|--------------|")
            for sib in siblings:
        ...
        # Conflict warnings
        task_conflicts = [
            (a, b, files) for a, b, files in self._file_conflicts if task.id in (a, b)
        ]
        if task_conflicts:
            ...
            for a, b, files in task_conflicts:
                ...
                parts.append(f"- **{other}** also touches: {', '.join(files)}")
            parts.append("")
    ...
    def build_update(
        ...
        if completed.provides:
            for contract in completed.provides:
                ...
                exports = contract.get("exports", [])
                if exports:
                    parts.append(f"Now available: `{file}` exports {', '.join(exports)}")
    ...
    def write_claude_md(
        ...
        if existing_content:
            content = orchestration_context + ORCHESTRATION_SEPARATOR + existing_content
        else:
    ...
    def append_update(
        ...
        if not claude_md_path.exists():
            logger.debug(f"No CLAUDE.md at {claude_md_path}, skipping update")
            return
    ...
    def _resolve_file_ownership(self, task: TaskNode) -> tuple[list[str], list[tuple[str, str]]]:
    ...
    def _get_dependency_chain(self, task_id: str) -> set[str]:
        ...
        # Ancestors (upstream)
        to_visit = list(
            self._nodes.get(task_id, TaskNode(id="", title="", description="")).dependencies
        )
        while to_visit:
            dep_id = to_visit.pop()
    ...
    def _describe_relationship(self, task: TaskNode, other: TaskNode) -> str:
