...
import { Search, RefreshCw, Loader2, FileText, BarChart3, Database, Play } from 'lucide-react'
...
async function fetchJSON(path: string, opts?: RequestInit) {
  try {
    ...
    try {
      return { ok: res.ok, data: JSON.parse(text), error: undefined }
    } catch {
      return { ok: false, data: undefined, error: text }
    }
  } catch (e: any) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={
              cacheStats?.cached_datasets
                ? 'Run empirical ranking on top SFT datasets'
                : 'Run a scan first'
            }
          >
            {runningEmpirical ? (
