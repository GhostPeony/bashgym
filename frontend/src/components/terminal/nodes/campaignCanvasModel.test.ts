...
function detail(): CampaignDetailState {
  return {
    ...
    studies: [{
      ...
      status: 'validated',
      stage_plan: { items: [{ stage: 'smoke_training', disposition: 'required', reason: 'Later queue item.' }] },
...
test('campaign research projection joins only durable authoritative evidence', () => {
  ...
  assert.deepEqual(research.budget.map((item) => item.resource), ['gpu_hours', 'study_count'])
...
test('projection never promotes simulation to baseline and distinguishes planned from terminal work', () => {
  ...
  source.ledger!.projects[0].runs = source.ledger!.projects[0].runs.filter((run) => run.is_simulation)
  ...
  assert.deepEqual(complete.nextAction, { kind: 'none', label: 'no further action', actionId: null })
})
