...
const model: CampaignOutcomeViewModel = {
  ...
  primaryMetricId: 'exact_count_accuracy', baselineLabel: 'Baseline', candidateLabel: 'LoRA candidate',
  baselineEvaluationId: 'evaluation-baseline-fixed64-qwen35-run1', candidateEvaluationId: 'evaluation-candidate-lora-160-qwen35-run1',
  evaluationSuiteId: 'star-count-fixed64', sameEvaluationSuite: true, decision: 'keep',
  metrics: [
    { id: 'exact_count_accuracy', baseline: 0.078125, candidate: 0.65625, delta: 0.578125, direction: 'maximize', primary: true },
    { id: 'count_accuracy', baseline: 0.359375, candidate: 0.89453125, delta: 0.53515625, direction: 'unknown', primary: false },
    { id: 'format_accuracy', baseline: 0.8125, candidate: 1, delta: 0.1875, direction: 'unknown', primary: false },
    { id: 'mean_reward', baseline: 0.38203125, candidate: 0.8998046875, delta: 0.5177734375, direction: 'unknown', primary: false },
  ],
  loss: {
...
test('detailed outcome makes success, every metric delta, and loss evidence visible', () => {
  const html = renderToStaticMarkup(createElement(CampaignOutcomeSummary, { model, density: 'detailed' }))
  ...
  assert.match(html, /aria-label="Training loss curve, 3 points, minimum 0.04011 at step 135, final 0.08392 at step 160"/)
...
test('compact outcome preserves the primary verdict, delta, and loss sparkline for the canvas node', () => {
  const html = renderToStaticMarkup(createElement(CampaignOutcomeSummary, { model, density: 'compact' }))
  assert.match(html, /aria-label="Baseline versus LoRA candidate outcome"/)
  assert.match(html, /Candidate kept/)
