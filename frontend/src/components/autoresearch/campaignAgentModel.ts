...
function normalizedCapabilities(
  values: readonly CampaignAgentCapability[]
...
export function validateCampaignAgentAuthoritativeClock(
  authoritativeNow: string,
  observedAt: string
): string | null {
  if (
    !isCanonicalCampaignAgentTimestamp(authoritativeNow) ||
    !isCanonicalCampaignAgentTimestamp(observedAt)
  ) {
...
function scopeError(selected: CampaignAgentScope, supplied: CampaignAgentScope): string | null {
  if (selected.workspaceId !== supplied.workspaceId)
    return 'Draft does not match the authoritative selected workspace.'
  if (selected.campaignId !== supplied.campaignId)
    return 'Draft does not match the authoritative selected campaign.'
  return null
}

function confirmationBindingError(draft: CampaignAgentAttachDraft, now: string): string | null {
  if (!draft.confirmation)
    return 'A current server-issued human grant confirmation receipt is required.'
  ...
  if (
    ...
    receipt.humanPrincipal.principalId === draft.agentPrincipalId
  ) {
    return 'Grant confirmation requires a distinct human principal.'
  }
  if (Date.parse(receipt.issuedAt) > Date.parse(now))
    return 'The human grant confirmation is not yet issued for the authoritative current time.'
  if (Date.parse(receipt.expiresAt) <= Date.parse(now))
    return 'The human grant confirmation receipt has expired.'
...
export function validateGrantConfirmationRequest(
  ...
  if (!draft.scope.workspaceId.trim() || !draft.scope.campaignId.trim())
    return ['Workspace and campaign scope are required.']
  if (!isKnownFamily(draft.agentFamily))
    return ['Only Codex and Hermes are conformant executable agent families.']
  if (
    !draft.agentOrigin.trim() ||
    !draft.sessionId.trim() ||
    !draft.agentPrincipalId.trim() ||
    !draft.idempotencyKey.trim()
  ) {
    return ['Agent origin, session, principal, and idempotency key are required.']
  }
  if (
    draft.requestedCapabilities.length === 0 ||
    draft.requestedCapabilities.length > campaignAgentCapabilities.length
  ) {
    return ['At least one supported capability must be requested.']
  }
  const unsupported = draft.requestedCapabilities.find(
    (capability) => !isKnownCapability(capability)
  )
  if (unsupported)
    return [
      isProhibitedCampaignAgentAction(unsupported)
        ? `Prohibited authority action requested: ${unsupported}.`
        : 'Requested capabilities must be explicit and supported.'
    ]
  if (
    new Set(draft.requestedCapabilities).size !== draft.requestedCapabilities.length ||
    new Set(draft.grantedCapabilities).size !== draft.grantedCapabilities.length
  ) {
    return ['Capability declarations cannot contain duplicates.']
  }
  if (
    !draft.grantedCapabilities.every(
      (capability) =>
        isKnownCapability(capability) && draft.requestedCapabilities.includes(capability)
    )
  ) {
...
export function validateAttachDraft(
  ...
  if (!isCanonicalCampaignAgentTimestamp(now))
    return ['Authoritative canonical current time is required.']
...
export function buildAttachRequest(
  ...
  if (currentAttachment?.status === 'attached')
    throw new Error('An agent is already attached to this campaign.')
...
export function buildRevokeRequest(
  ...
  if (!input.actorId.trim() || !input.idempotencyKey.trim())
    throw new Error('Actor and idempotency key are required.')
  return {
    action: 'revoke',
...
const sensitiveValuePattern =
  /(?:\b[A-Za-z]:\/|\/\/[^/\s]+\/[^/\s]+|(?:^|[^A-Za-z0-9/])\/(?!\/)\S+|(?:^|[^A-Za-z0-9])(?:\.\.?\/|~\/|\.ssh\/)\S*|\b(?:https?|file|ssh|s3|gs):\/\/|\bbearer\s+[A-Za-z0-9._-]+|(?:secret|token|api[_ -]?key|password|protected[_ -]?row[_ -]?id|private[_ -]?row[_ -]?id)\s*[:=]\s*\S+|\bsk-[A-Za-z0-9]{8,}|\bghp_[A-Za-z0-9]{20,}|\bgithub_pat_[A-Za-z0-9_]{20,}|\bAKIA[A-Z0-9]{16}\b|-----BEGIN(?: [A-Z]+)* PRIVATE KEY-----)/i
...
function normalizedPublicValue(value: string, label: string): string {
  ...
  if (/%[a-f0-9]{2}/i.test(normalized))
    throw new Error(
      `${label} contains a sensitive public value (percent encoding exceeds the allowed depth).`
    )
...
function publicString(value: unknown, label: string, maxLength = 128): string {
  ...
  if (sensitiveValuePattern.test(normalizedPublicValue(value, label)))
    throw new Error(`${label} contains a sensitive public value.`)
  return value
}
...
function isoString(value: unknown, label: string): string {
  ...
  if (!isCanonicalCampaignAgentTimestamp(parsed))
...
function boundedArray(value: unknown, label: string, maxLength: number): unknown[] {
  ...
  if (value.length > maxLength)
    throw new Error(`${label} must contain at most ${maxLength} entries.`)
  return value
}
...
function parseCapabilities(value: unknown, label: string): CampaignAgentCapability[] {
  const values = boundedArray(value, label, campaignAgentCapabilities.length).map((entry) =>
    publicString(entry, label)
  )
  if (values.some((entry) => !isKnownCapability(entry)))
    throw new Error(`${label} contains an unsupported or prohibited capability.`)
...
export function parseCampaignAgentGrantConfirmation(
  value: unknown
): CampaignAgentGrantConfirmationReceipt {
  const record = asExactObject(
    value,
    [
      'schemaVersion',
      'issuer',
  ...
  if (
    record.schemaVersion !== 'campaign_agent_grant_confirmation.v1' ||
    record.issuer !== 'campaign_authority'
  ) {
    throw new Error('The grant confirmation must be issued by the campaign authority.')
  }
  const humanRecord = asExactObject(
    record.humanPrincipal,
    ['principalId', 'principalType'],
    'Grant confirmation human principal'
  )
  if (humanRecord.principalType !== 'human')
    throw new Error('Grant confirmation human principal type must be human.')
  const humanPrincipalId = publicString(
    humanRecord.principalId,
    'grant_confirmation.humanPrincipal.principalId'
  )
  if (humanPrincipalId !== humanPrincipalId.trim())
    throw new Error('Grant confirmation human principal must be trimmed.')
  const scopeRecord = asExactObject(
    record.scope,
    ['campaignId', 'workspaceId'],
    'Grant confirmation scope'
  )
  ...
  const requestedCapabilities = parseCapabilities(
    ...
    'grant_confirmation.requestedCapabilities'
  )
  const grantedCapabilities = parseCapabilities(
    record.grantedCapabilities,
    'grant_confirmation.grantedCapabilities'
  )
  ...
  const agentPrincipalId = publicString(
    record.agentPrincipalId,
    'grant_confirmation.agentPrincipalId'
  )
  const receiptDigest = publicString(record.receiptDigest, 'grant_confirmation.receiptDigest')
  if (!/^sha256:[a-f0-9]{64}$/.test(receiptDigest))
    throw new Error('The grant confirmation receipt digest is invalid.')
  const capabilityDigest = publicString(
    record.capabilityDigest,
    'grant_confirmation.capabilityDigest'
  )
  if (
    capabilityDigest !==
    capabilityBindingDigest(agentPrincipalId, requestedCapabilities, grantedCapabilities)
  ) {
  ...
  if (Date.parse(issuedAt) >= Date.parse(expiresAt))
    throw new Error('The human grant confirmation must be issued before it expires.')
  return deepFreeze({
...
function parseReceipt(value: unknown): CampaignAgentPublicReceipt {
  ...
  if (!/^sha256:[a-f0-9]{64}$/.test(receiptDigest))
    throw new Error('receipt.receipt_digest is invalid.')
  return {
...
function parseAuditEvent(value: unknown): CampaignAgentPublicAuditEvent {
  const record = asExactObject(
    value,
    ['event_id', 'sequence', 'kind', 'occurred_at', 'message_code'],
    'Campaign agent audit event'
  )
  const kind = publicString(record.kind, 'audit.kind')
  if (!['attachment', 'revocation', 'credential', 'liveness'].includes(kind))
    throw new Error('audit.kind is unsupported.')
  const messageCode = publicString(record.message_code, 'audit.message_code')
  if (!(campaignAgentAuditMessageCodes as readonly string[]).includes(messageCode))
    throw new Error('audit.message_code is an unsupported fixed message code.')
  const messageCodeForKind: Record<
    CampaignAgentPublicAuditEvent['kind'],
    CampaignAgentAuditMessageCode
  > = {
...
function parseAttachment(
  ...
  scope: CampaignAgentScope,
  observedAt: string
): CampaignAgentPublicAttachment | null {
  ...
  if (record.schema_version !== 'campaign_agent_public_attachment.v1')
    throw new Error('Unsupported campaign agent attachment schema version.')
  const status = publicString(record.status, 'attachment.status')
  if (status !== 'attached' && status !== 'revoked')
    throw new Error('attachment.status is unsupported.')
  const attachmentId = publicString(record.attachment_id, 'attachment.attachment_id')
  const attachmentVersion = publicInteger(
    record.attachment_version,
    'attachment.attachment_version',
    1
  )
  const requestedCapabilities = parseCapabilities(
    record.requested_capabilities,
    'attachment.requested_capabilities'
  )
  const grantedCapabilities = parseCapabilities(
  ...
  if (!grantedCapabilities.every((capability) => requestedCapabilities.includes(capability)))
    throw new Error('Authoritative grants exceed requested capabilities.')
  ...
  if (typeof hasEarlier !== 'boolean')
    throw new Error('receipt_window.has_earlier must be a boolean.')
  const receiptWindow = {
    fromVersion: publicInteger(receiptWindowRecord.from_version, 'receipt_window.from_version', 1),
    throughVersion: publicInteger(
      receiptWindowRecord.through_version,
      'receipt_window.through_version',
      1
    ),
    hasEarlier
  }

  const provenanceRecord = asExactObject(
    record.provenance,
    [
      'agent_family',
      'agent_origin',
  ...
  if (provenanceRecord.agent_origin_status !== 'verified')
    throw new Error('Brokered agent origin must be authority-verified.')
  const credentialStatus = publicString(
    provenanceRecord.credential_status,
    'provenance.credential_status'
  )
  if (!['active', 'expired', 'revoked'].includes(credentialStatus))
    throw new Error('provenance.credential_status is unsupported.')
  const liveness = publicString(provenanceRecord.liveness, 'provenance.liveness')
  if (!['live', 'idle', 'offline', 'expired', 'revoked'].includes(liveness))
    throw new Error('provenance.liveness is unsupported.')
  if (provenanceRecord.credential_status_source !== 'campaign_authority')
    throw new Error('Credential status must come from campaign authority.')
  const grantReceiptDigest = publicString(
    provenanceRecord.grant_receipt_digest,
    'provenance.grant_receipt_digest'
  )
  if (!/^sha256:[a-f0-9]{64}$/.test(grantReceiptDigest))
    throw new Error('provenance.grant_receipt_digest is invalid.')
  ...
  const credentialIssuedAt = isoString(
    provenanceRecord.credential_issued_at,
    'provenance.credential_issued_at'
  )
  const credentialExpiresAt = isoString(
    provenanceRecord.credential_expires_at,
    'provenance.credential_expires_at'
  )
  const revocationRevision = publicInteger(
    provenanceRecord.credential_revocation_revision,
    'provenance.credential_revocation_revision'
  )
  const revokedAt =
    provenanceRecord.revoked_at === null
      ? null
      : isoString(provenanceRecord.revoked_at, 'provenance.revoked_at')
  ...
  if (Boolean(revokedAt) !== Boolean(revokedBy))
    throw new Error('Revocation actor and time must be present together.')
  if (
    issuedEpoch > attachedEpoch ||
    attachedEpoch > observedEpoch ||
    issuedEpoch >= expiresEpoch ||
    attachedEpoch >= expiresEpoch
  ) {
    throw new Error(
      'Credential temporal ordering must satisfy issued <= attached <= observed and attached < expiry.'
    )
  }
  if (credentialStatus === 'active') {
    if (expiresEpoch <= observedEpoch) throw new Error('An active credential has expired.')
    if (!['live', 'idle', 'offline'].includes(liveness))
      throw new Error('Active credential liveness must be live, idle, or offline.')
    if (hasRevocationMetadata)
      throw new Error('Active credential cannot carry revocation metadata.')
  }
  if (credentialStatus === 'expired') {
    if (expiresEpoch > observedEpoch)
      throw new Error('Expired credential status precedes credential expiry.')
    if (liveness !== 'expired') throw new Error('Expired credential liveness must be expired.')
    if (hasRevocationMetadata)
      throw new Error('Expired credential cannot carry revocation metadata.')
  }
  if (credentialStatus === 'revoked') {
    if (
      ...
      throw new Error(
        'Revoked attachment requires authoritative revocation revision, status, liveness, actor, and time.'
      )
    }
    const revokedEpoch = Date.parse(revokedAt!)
    if (revokedEpoch < attachedEpoch || revokedEpoch > observedEpoch)
      throw new Error('Revocation temporal ordering must be attached <= revoked <= observed.')
  }
  if (status === 'revoked' && credentialStatus !== 'revoked')
    throw new Error('Revoked attachment requires revoked credential status.')
  if (status === 'attached' && credentialStatus === 'revoked')
    throw new Error('Attached record contains inconsistent revocation authority.')

  const receipts = boundedArray(record.receipts, 'attachment.receipts', 20)
    .map(parseReceipt)
    .sort(
      (left, right) =>
        left.attachmentVersion - right.attachmentVersion ||
  ...
  if (new Set(receipts.map((receipt) => receipt.receiptId)).size !== receipts.length)
    throw new Error('Attachment receipts contain duplicate IDs.')
  if (new Set(receipts.map((receipt) => receipt.idempotencyKey)).size !== receipts.length)
    throw new Error('Attachment receipt history contains duplicate idempotency keys.')
  for (let index = 0; index < receipts.length; index += 1) {
    const receipt = receipts[index]
    if (index === 0 && receipt.kind !== 'attach')
      throw new Error('Attachment receipt history must begin with attach.')
    if (receipt.attachmentVersion > attachmentVersion)
      throw new Error('Receipt version exceeds the current attachment version.')
    if (index > 0) {
      const prior = receipts[index - 1]
      if (
        receipt.attachmentVersion <= prior.attachmentVersion ||
        Date.parse(receipt.occurredAt) <= Date.parse(prior.occurredAt)
      ) {
        throw new Error('Attachment receipt history must be strictly monotonic.')
      }
      if (
        receipt.attachmentVersion !== prior.attachmentVersion + 1 ||
        receipt.kind === prior.kind
      ) {
        throw new Error(
          'Attachment receipt history must use consecutive versions and alternating attach/revoke transitions.'
        )
      }
    }
  }
  ...
  if (currentReceipt.attachmentVersion !== attachmentVersion)
    throw new Error('Current receipt must match the current attachment version.')
  if (
  ...
  if (
    currentReceipt.kind !== expectedCurrentKind ||
    currentReceipt.occurredAt !== expectedCurrentTime
  ) {
  ...
  if (
    status === 'attached' &&
    precedingReceipt?.kind === 'revoke' &&
    issuedEpoch < Date.parse(precedingReceipt.occurredAt)
  ) {
    throw new Error(
      'Reattach credential must be issued at or after the preceding revoke transition.'
    )
  }
  ...
  return {
    ...
    provenance: {
      ...
      agentPrincipalId: publicString(
        provenanceRecord.agent_principal_id,
        'provenance.agent_principal_id'
      ),
      attachedAt,
      attachedBy,
      grantReceiptId: publicString(
        provenanceRecord.grant_receipt_id,
        'provenance.grant_receipt_id'
      ),
      grantReceiptDigest,
      credentialIssuedAt,
      credentialExpiresAt,
      ...
      resumeCursor: nullablePublicString(
        provenanceRecord.resume_cursor,
        'provenance.resume_cursor'
      ),
      revokedAt,
      revokedBy
    },
...
export function parseCampaignAgentPublicView(value: unknown): CampaignAgentPublicView {
  const record = asExactObject(
    ...
    'Campaign agent public view'
  )
  if (record.schema_version !== 'campaign_agent_public_view.v1')
    throw new Error('Unsupported campaign agent public view schema version.')
  ...
  const auditEvents = boundedArray(record.audit_events, 'public_view.audit_events', 50).map(
    parseAuditEvent
  )
  if (new Set(auditEvents.map((event) => event.eventId)).size !== auditEvents.length)
    throw new Error('Audit events contain duplicate IDs.')
  if (new Set(auditEvents.map((event) => event.sequence)).size !== auditEvents.length)
    throw new Error('Audit events must have unique sequences.')
  ...
  auditEvents.sort(
    (left, right) => right.sequence - left.sequence || left.eventId.localeCompare(right.eventId)
  )
  return deepFreeze({
...
export function reconcileAuthoritativeAttachment(
  ...
  const assertSelectedScope = (
    ...
    if (attachment.scope.workspaceId !== selectedScope.workspaceId)
      throw new Error(`${label} attachment does not match the selected workspace.`)
    if (attachment.scope.campaignId !== selectedScope.campaignId)
      throw new Error(`${label} attachment does not match the selected campaign.`)
  }
  const assertHistory = (attachment: CampaignAgentPublicAttachment) => {
    if (attachment.receipts.length === 0)
      throw new Error('Authoritative attachment receipt history cannot be empty.')
    if (
      new Set(attachment.receipts.map((receipt) => receipt.receiptId)).size !==
        attachment.receipts.length ||
      new Set(attachment.receipts.map((receipt) => receipt.idempotencyKey)).size !==
        attachment.receipts.length
    ) {
      throw new Error(
        'Authoritative attachment receipt history must have unique IDs and idempotency keys.'
      )
    }
    for (let index = 0; index < attachment.receipts.length; index += 1) {
      const receipt = attachment.receipts[index]
      if (index === 0 && receipt.kind !== 'attach')
        throw new Error('Authoritative attachment history must begin with attach.')
      if (index > 0) {
        ...
        if (
          ...
          throw new Error(
            'Authoritative attachment history must use consecutive alternating transitions.'
          )
        }
      }
    }
    const latest = attachment.receipts[attachment.receipts.length - 1]
    if (latest.attachmentVersion !== attachment.attachmentVersion)
      throw new Error('Latest receipt must match the current attachment version.')
    if (
      (attachment.status === 'attached' && latest.kind !== 'attach') ||
      (attachment.status === 'revoked' && latest.kind !== 'revoke')
    ) {
      throw new Error('Latest receipt kind does not match attachment status.')
    }
    const transitionActor =
      attachment.status === 'attached'
        ? attachment.provenance.attachedBy
        : attachment.provenance.revokedBy
    if (latest.actorId !== transitionActor)
      throw new Error('Latest receipt actor does not match authoritative transition provenance.')
  }
  ...
  const expectedReceipts = incoming.receipts.filter(
    (receipt) => receipt.idempotencyKey === expectedMutation.idempotencyKey
  )
  if (expectedReceipts.length !== 1)
    throw new Error('Authoritative response must contain one unique expected idempotency receipt.')
  const expectedReceipt = expectedReceipts[0]
  if (expectedReceipt.kind !== expectedMutation.kind)
    throw new Error('Expected receipt does not match the mutation kind.')
  if (expectedReceipt.attachmentVersion !== incoming.attachmentVersion)
    throw new Error('Expected receipt does not represent the incoming version.')
  const expectedActor = publicString(expectedMutation.actorId, 'Expected actor')
  const authoritativeActor =
    expectedMutation.kind === 'attach'
      ? incoming.provenance.attachedBy
      : incoming.provenance.revokedBy
  if (expectedReceipt.actorId !== expectedActor || authoritativeActor !== expectedActor) {
    throw new Error(
      'Expected actor does not match the authoritative mutation receipt and provenance.'
    )
  }
  if (expectedMutation.kind === 'attach') {
    ...
    if (priorReceipt && priorReceipt.kind !== 'revoke')
      throw new Error('Reattach must follow an authoritative revoke receipt.')
    ...
    if (!current && authoritativeBase !== null)
      throw new Error('Reattach requires the current revoked authoritative base.')
    if (
      current &&
    ...
    if (
      !priorReceipt ||
      priorReceipt.attachmentVersion !== baseVersion ||
      expectedReceipt.attachmentVersion !== baseVersion + 1
    ) {
    ...
    if (
      ...
      throw new Error(
        'Revoke mutation base version does not match the current authoritative attachment.'
      )
    }
  }
  if (!current) return incoming
  if (current.attachmentId !== incoming.attachmentId)
    throw new Error('Authoritative response changed attachment identity.')
  if (incoming.attachmentVersion < current.attachmentVersion)
    throw new Error('Cannot reconcile an older attachment version.')
  if (
    current.status === 'revoked' &&
    incoming.status === 'attached' &&
    incoming.attachmentVersion > current.attachmentVersion
  ) {
    const currentRevokedAt = current.provenance.revokedAt
    if (
      !currentRevokedAt ||
      Date.parse(incoming.provenance.credentialIssuedAt) < Date.parse(currentRevokedAt)
    ) {
      throw new Error('Reattach credential must be issued at or after the current revoke time.')
    }
    if (
      incoming.provenance.credentialRevocationRevision <
      current.provenance.credentialRevocationRevision
    ) {
      throw new Error(
        'Reattach credential revocation revision cannot rollback and must remain monotonic.'
      )
    }
  }
  if (incoming.attachmentVersion === current.attachmentVersion) {
    if (JSON.stringify(incoming) !== JSON.stringify(current))
      throw new Error('Conflicting complete authoritative attachment reused a version.')
    return current
  }
  const currentByVersion = new Map(
    current.receipts.map((receipt) => [receipt.attachmentVersion, receipt])
  )
  const overlap = incoming.receipts.filter((receipt) =>
    currentByVersion.has(receipt.attachmentVersion)
  )
  if (overlap.length === 0)
    throw new Error('Newer attachment window must overlap current authoritative history.')
  for (const receipt of overlap) {
    if (
      JSON.stringify(receipt) !== JSON.stringify(currentByVersion.get(receipt.attachmentVersion))
    ) {
