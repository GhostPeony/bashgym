...
function stateTone(state: McpConnectionState): 'neutral' | 'accent' | 'success' | 'warning' | 'error' {
...
function profileValidationError(draft: McpProfileDraft): string | null {
  ...
  if (draft.transport === 'streamable_http' && draft.oauthClientSecretRef.trim() && !draft.oauthClientId.trim()) {
...
function MetricTile({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="min-w-0 border-brutal border-border-subtle rounded-brutal bg-background-card px-2 py-1.5">
      ...
      <div className="mt-1 truncate text-[11px] font-mono font-semibold text-text-primary" title={String(value)}>
        {value}
      </div>
    </div>
  )
...
function toolPolicyPresentation(policy: McpTool['policy']): {
  ...
  if (policy === 'allow') {
    return { label: 'Ready to run', description: 'Runs without asking for another confirmation.', tone: 'success' }
  }
  if (policy === 'ask') {
    return { label: 'Asks before running', description: 'You review and approve this tool before it can make a call.', tone: 'warning' }
  }
  if (policy === 'deny') {
    return { label: 'Blocked', description: 'This tool cannot run under the current server policy.', tone: 'error' }
  }
  return { label: 'Not classified', description: 'BashGym will pause for review before this tool runs.', tone: 'neutral' }
}
...
export const McpWorkbenchNode = memo(function McpWorkbenchNode({
  ...
  const [claudeImportScope, setClaudeImportScope] = useState<'local' | 'project' | 'user'>('project')
  ...
  const persistPresentation = useCallback((updates: Record<string, unknown>) => {
  ...
  const loadProfiles = useCallback(async () => {
    ...
    if (!api) {
      ...
      setError('MCP API adapter is not configured. Start the BashGym backend and reload the canvas.')
      return
    }
    ...
    if (nextProfile) {
      ...
      await Promise.all([
        loadSnapshot(nextProfile.profile_id),
        loadOAuthStatus(nextProfile)
      ])
    } else {
  ...
  useEffect(() => {
    ...
    contentScrollRef.current.scrollTop = ui.section === 'overview'
      ? ui.overviewScrollTop
      : ui.toolsScrollTop
  ...
  const connectProfile = useCallback((targetProfile: McpProfile) => {
    const api = getConfiguredMcpWorkbenchApi()
    if (!api) return
    void runManagedOperation(
      async () => {
        if (targetProfile.transport === 'stdio') {
          const preview = await api.previewStdio(targetProfile.profile_id, targetProfile.profile_revision)
          if (!preview.ok || !preview.data) return { ok: false, error: preview.error || 'Unable to preview the stdio launch.' }
          ...
          if (!approval.ok) return { ok: false, error: approval.error || 'Unable to save the launch approval.' }
        }
  ...
  const connect = useCallback(() => {
    if (!profile) return
    if (draftDirty) {
      setError('Save these profile changes before connecting so the displayed URL and authentication settings are the ones actually tested.')
      return
    }
  ...
  const selfTest = useCallback(() => {
    ...
    void runManagedOperation(
      () => api.selfTest(),
      (completed) => setQuickTestResult(resultString(completed.result) || 'Reference MCP self-test passed.')
    )
  }, [runManagedOperation])
  ...
  const applyClaudeImportCandidate = useCallback((candidate: McpClaudeImportCandidate) => {
  ...
  const callTool = useCallback(() => {
    ...
    try {
      ...
      if (!approved && destructive) {
        typedConfirmation = window.prompt(`Type ${selectedTool.name} to approve this destructive call.`) || undefined
  ...
  }, [runManagedOperation, selectedTool, sessionId, ui.manualInput, ui.maxResultKilobytes, ui.timeoutSeconds])
  ...
  const openAdvanced = useCallback((section: McpAdvancedSection = ui.section) => {
  ...
  const selectSection = useCallback((section: McpAdvancedSection) => {
    rememberScroll()
  ...
  const profileTime = profile?.updated_at ? new Date(profile.updated_at).toLocaleString() : undefined
        ...
        <div className="space-y-2">
          {loadingProfiles ? (
            ...
            <div className="node-section text-center">
              ...
              <div className="text-[11px] font-mono font-semibold text-text-primary">No MCP selected</div>
            ...
            <>
              <div className="flex items-center gap-2 min-w-0">
                <ConfigPill tone={stateTone(connectionState)}>{stateLabel(connectionState)}</ConfigPill>
                <span className="min-w-0 flex-1 truncate text-[10px] font-mono text-text-secondary" title={profile.label}>
                  {profile.label}
                </span>
                {snapshot?.stale ? <ShieldAlert className="w-3 h-3 flex-shrink-0 text-status-warning" /> : null}
              </div>

              <div className="grid grid-cols-3 gap-1.5">
                <MetricTile label="Transport" value={profile.transport === 'stdio' ? 'stdio' : 'HTTP'} />
              ...
              {connectionVerified ? (
                <div className="rounded-brutal border-brutal border-status-success bg-status-success/10 p-2">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      <div className="text-[10px] font-mono font-bold text-status-success">Connection verified</div>
                      <div className="mt-0.5 truncate text-[10px] font-mono text-text-primary" title={serverIdentity.name}>
                        {serverIdentity.name}{serverIdentity.version ? ` · ${serverIdentity.version}` : ''}
                      </div>
                      <div className="mt-1 text-[9px] font-mono text-text-muted">
                        MCP {snapshot?.negotiated_protocol_version} · {snapshot?.captured_at ? new Date(snapshot.captured_at).toLocaleTimeString() : 'just now'}
                      </div>
                      {oauthStatus?.has_tokens ? (
                        <div className="mt-1 text-[9px] font-mono font-bold text-accent-dark">Hosted OAuth authenticated</div>
                      ) : null}
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-brutal border-brutal border-border-subtle bg-background-card p-2 text-[10px] font-mono text-text-secondary">
                  <span className="font-bold text-text-primary">Not yet verified.</span>{' '}
                  Connect to complete a capability handshake and prove the server is responding.
                </div>
              )}
              ...
              <div className="nodrag flex items-center gap-2">
                <button
                  ...
                  disabled={isWorking}
                >
                  {isWorking ? <Loader2 className="w-3 h-3 animate-spin" /> : sessionId ? <FlaskConical className="w-3 h-3" /> : <Plug className="w-3 h-3" />}
              ...
              <div className="space-y-3">
                <ConfigSection title="Connection">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <label className="node-field">
                        ...
                        <input
                          ...
                          onChange={(event) => setDraft((current) => ({ ...current, remoteUrl: event.target.value }))}
                      ...
                      <label className="node-field">
                        ...
                        <span className="text-[10px] leading-4 text-text-muted">
                          Automatic is recommended for hosted MCPs. BashGym opens the provider login page after an authentication challenge.
                        </span>
                      </label>
                      {draft.authMode === 'auto' || draft.authMode === 'oauth' ? (
                        <div className="grid grid-cols-1 gap-2 rounded-brutal border-brutal border-accent/50 bg-accent/5 p-2 md:grid-cols-2">
                          <div className="md:col-span-2 flex items-start gap-2 text-[10px] leading-4 text-text-secondary">
                            ...
                            OAuth uses protected-resource discovery, PKCE, and a loopback callback. Tokens are kept in the OS credential store, never in this profile.
                          </div>
                          <label className="node-field md:col-span-2">
                            <span className="node-field-label">Requested scopes — spaces or commas</span>
                            <input
                              className="input-brutal min-h-9 text-[11px] font-mono"
                          ...
                          <label className="node-field">
                            ...
                            <input
                              ...
                              onChange={(event) => setDraft((current) => ({ ...current, oauthCallbackPort: event.target.value }))}
                              inputMode="numeric"
                          ...
                          <label className="node-field md:col-span-2">
                            ...
                            <input
                              ...
                              onChange={(event) => setDraft((current) => ({ ...current, oauthClientSecretRef: event.target.value }))}
                      ...
                      <label className="node-field">
                        ...
                        <textarea
                          ...
                          onChange={(event) => setDraft((current) => ({ ...current, headerSecretRefs: event.target.value }))}
                          placeholder={'Authorization=MCP_TOKEN\nX-Workspace=MCP_WORKSPACE_ID'}
                        />
                        <span className="text-[10px] leading-4 text-text-muted">
                          Reference names only. Secret values belong in the OS credential store or environment.
                        </span>
                      </label>
                      <label className="flex items-start gap-2 rounded-brutal border-brutal border-border-subtle bg-background-card p-2.5">
                        ...
                        />
                        <span>
                          <span className="block text-[10px] font-mono font-bold text-text-primary">Allow private-network addresses</span>
                    ...
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      ...
                      <label className="node-field">
                        ...
                        <textarea
                          ...
                          onChange={(event) => setDraft((current) => ({ ...current, envSecretRefs: event.target.value }))}
                      ...
                      <label className="node-field md:col-span-2">
                        ...
                        <select
                          ...
                          onChange={(event) => setDraft((current) => ({ ...current, cwdPolicy: event.target.value }))}
                        >
                      ...
                      {draft.cwdPolicy === 'explicit' ? (
                        <label className="node-field md:col-span-2">
                          ...
                          <input
                            ...
                            onChange={(event) => setDraft((current) => ({ ...current, explicitCwd: event.target.value }))}
                      ...
                      <label className="node-field md:col-span-2">
                        ...
                        >
                          ...
                          <option value="preferred">Preferred — connect with a visible warning</option>
                  ...
                  <div className="node-config-action-row">
                    ...
                    >
                      {savingProfile ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle2 className="w-3 h-3" />}
                      {profile ? 'Save changes' : 'Save profile'}
                    </button>
                    <div className="node-config-action-spacer" />
                    {sessionId ? (
                      <button type="button" className="node-btn node-btn-wide" onClick={disconnect} disabled={isWorking}>
                      ...
                      >
                        {savingProfile ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plug className="w-3 h-3" />}
                        {draftDirty ? 'Save & connect' : 'Connect'}
                      </button>
                    )}
                    <button type="button" className="node-btn node-btn-wide" onClick={refresh} disabled={!profile || isWorking || draftDirty}>
                ...
                <ConfigSection title="Import Claude MCP config">
                    ...
                    <div className="space-y-2">
                      <label className="node-field">
                        ...
                        <select
                          ...
                          onChange={(event) => setClaudeImportScope(event.target.value as typeof claudeImportScope)}
                        >
                      ...
                      <button type="button" className="node-btn node-btn-wide node-btn-accent w-full justify-center" onClick={() => void previewClaudeImport()}>
                  ...
                  <p className="mt-2 text-[10px] leading-4 text-text-muted">
                    Use environment placeholders such as ${'{MCP_TOKEN}'}. Raw values are blocked and never returned by the preview.
                  </p>
                  {claudeImportCandidates.length ? (
                    <div className="mt-2 space-y-2">
                      {claudeImportCandidates.map((candidate) => (
                        <div key={candidate.server_name} className="rounded-brutal border-brutal border-border-subtle bg-background-card p-2">
                          <div className="flex items-center gap-2">
                            <span className="min-w-0 flex-1 truncate text-[10px] font-mono font-bold text-text-primary">{candidate.server_name}</span>
                            <ConfigPill tone={candidate.supported ? 'success' : 'error'}>
                              {candidate.supported ? 'ready' : 'migration needed'}
                            </ConfigPill>
                            {candidate.supported ? (
                              <button type="button" className="node-btn" onClick={() => applyClaudeImportCandidate(candidate)}>Use config</button>
                            ) : null}
                          </div>
                          {candidate.issues.length ? (
                            <ul className="mt-1 space-y-1 text-[9px] font-mono text-text-secondary">
                              {candidate.issues.map((issue) => <li key={`${issue.code}:${issue.field ?? ''}`}>• {issue.message}</li>)}
                            </ul>
                          ) : null}
                        </div>
                      ))}
                ...
                <ConfigSection title="Connection verification and troubleshooting">
                  {connectionVerified ? (
                    <div className="rounded-brutal border-brutal border-status-success bg-status-success/10 p-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="min-w-0">
                          <div className="text-xs font-mono font-bold text-status-success">Verified capability handshake</div>
                          <div className="mt-1 text-[11px] font-mono text-text-primary">
                            {serverIdentity.name}{serverIdentity.version ? ` · ${serverIdentity.version}` : ''}
                          </div>
                          <p className="mt-1 text-[10px] leading-4 text-text-secondary">
                            BashGym has a live session and a fresh MCP inventory from this server revision.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="rounded-brutal border-brutal border-status-warning/70 bg-status-warning/10 p-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <div className="text-xs font-mono font-bold text-text-primary">Connection not verified</div>
                          <p className="mt-1 text-[10px] leading-4 text-text-secondary">
                            {operation?.error || 'No fresh capability handshake is available for this profile.'}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {profile?.transport === 'streamable_http' ? (
                    <div className="mt-2 flex flex-wrap items-center gap-2 rounded-brutal border-brutal border-border-subtle bg-background-card p-2">
                      <ConfigPill tone={oauthStatus?.has_tokens ? 'success' : oauthStatus?.interactive_oauth ? 'accent' : 'neutral'}>
                        {oauthStatus?.has_tokens
                          ? 'OAuth signed in'
                      ...
                      {oauthStatus?.has_tokens ? (
                        <button type="button" className="node-btn" onClick={() => void clearOAuth()} disabled={isWorking}>
                          Clear sign-in
                        </button>
                      ) : null}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {snapshot?.drifted ? <ConfigPill tone="warning">contract drift</ConfigPill> : null}
                    {warningCount ? <ConfigPill tone="warning">{warningCount} warnings</ConfigPill> : null}
                  </div>
                  <ConfigRows>
                    <ConfigRow label="Profile revision" value={profile?.profile_revision} />
                    <ConfigRow label="Profile updated" value={profileTime} />
                    <ConfigRow label="Server" value={`${serverIdentity.name}${serverIdentity.version ? ` · ${serverIdentity.version}` : ''}`} />
                    <ConfigRow label="Session" value={sessionId} />
                    <ConfigRow label="Snapshot" value={snapshot?.snapshot_id} />
                    <ConfigRow label="Captured" value={snapshot?.captured_at ? new Date(snapshot.captured_at).toLocaleString() : undefined} />
                    ...
                    <ConfigRow label="Contract drift" value={snapshot?.drifted ? 'Detected since previous snapshot' : 'None detected'} />
                    ...
                    <ConfigRow label="Operation" value={operation ? `${operation.status}${operation.phase ? ` — ${operation.phase}` : ''}` : undefined} />
                  ...
                  {snapshot?.schema_warnings?.length ? (
                    <div className="mt-2 rounded-brutal border-brutal border-status-warning/60 bg-status-warning/10 p-2">
                      ...
                      <ul className="mt-1 space-y-1 text-[9px] font-mono text-text-secondary">
                        {snapshot.schema_warnings.map((warning) => <li key={warning}>• {warning}</li>)}
                      </ul>
                    </div>
                  ) : null}
                  {isWorking ? (
                    <button type="button" className="node-btn node-btn-wide node-btn-warning" onClick={() => void cancelOperation()}>
                  ...
                  {!connectionVerified ? (
                    <div className="mt-2 rounded-brutal border-brutal border-border-subtle bg-background-secondary p-2">
                      ...
                      <ol className="mt-1 space-y-1 text-[10px] leading-4 text-text-secondary">
                        {troubleshootingSteps.map((step, index) => <li key={step}>{index + 1}. {step}</li>)}
                      </ol>
                    </div>
                  ) : null}
                  <div className="node-config-action-row mt-2">
                    {profile ? (
                      <button type="button" className="node-btn node-btn-wide node-btn-accent" onClick={connectionVerified ? quickTest : sessionId ? refresh : connect} disabled={isWorking}>
                        {connectionVerified ? <RefreshCw className="w-3 h-3" /> : <Plug className="w-3 h-3" />}
                        {connectionVerified ? 'Verify again' : sessionId ? 'Refresh handshake' : 'Connect and verify'}
                      </button>
                    ) : null}
                    <button type="button" className="node-btn node-btn-wide" onClick={selfTest} disabled={isWorking}>
                      <FlaskConical className="w-3 h-3" />
                      Test BashGym runtime
                    </button>
                    <button type="button" className="node-btn node-btn-wide" onClick={() => void copyDiagnostics()}>
                ...
                >
                  <div className="mcp-tools-search">
                    ...
                    <input
                      ...
                      value={ui.toolQuery}
                      onChange={(event) => dispatchUi({ type: 'set_tool_query', query: event.target.value })}
                    ...
                    <div className="mcp-tools-list" aria-label="Advertised MCP tools">
                      {filteredTools.length ? filteredTools.map((tool) => (
                        ...
                        <div className="py-8 text-center text-[10px] font-mono text-text-muted">
                          {snapshot ? 'No tools match this search.' : 'Connect and refresh to inspect tools.'}
                        </div>
                      )}
                    </div>
                        ...
                        <>
                          <div>
                            ...
                            <p className="mt-2 text-xs leading-5 text-text-secondary">
                              {selectedTool.description || 'This server did not provide a tool description.'}
                            </p>
                          </div>
                          <div className="mcp-tool-insights">
                            <div className="mcp-tool-insight">
                              ...
                              <div className="mt-2">
                                <ConfigPill tone={selectedToolPolicy.tone}>{selectedToolPolicy.label}</ConfigPill>
                              </div>
                            ...
                            <div className="mcp-tool-insight">
                              ...
                              {selectedToolUsageCount > 0 ? (
                                <div className="mt-2 space-y-1 text-[10px] leading-4 text-text-secondary">
                                  <div><strong className="text-text-primary">{selectedToolUsageCount}</strong> recorded {selectedToolUsageCount === 1 ? 'call' : 'calls'}</div>
                                  <div>Last response: <strong className="text-text-primary">{formatToolLatency(selectedTool.last_latency_ms)}</strong></div>
                                  <div>Reliability: <strong className="text-text-primary">{formatToolReliability(selectedTool.error_rate)}</strong></div>
                                </div>
                              ) : (
                                <p className="mt-2 text-[10px] leading-4 text-text-secondary">
                                  No calls recorded yet. Response time and reliability appear after this tool is tested.
                                </p>
                              )}
                            </div>
                          </div>
                              ...
                              <div className="mcp-schema-grid">
                                ...
                                <label className="node-field">
                                  ...
                                  <pre
                                    className={clsx(
                                      'mcp-schema-panel',
                                      !resultString(selectedTool.output_schema) && 'mcp-schema-panel-empty'
                                    )}
                                  >
                                    {resultString(selectedTool.output_schema) || 'The server did not describe its output.'}
                                  </pre>
                                </label>
                              </div>
                            </div>
                          ...
                          <label className="node-field">
                            ...
                            <textarea
                              ...
                              onChange={(event) => dispatchUi({ type: 'set_manual_input', value: event.target.value })}
                              spellCheck={false}
                            />
                            <span className="text-[10px] leading-4 text-text-muted">
                              Start with {'{}'} when no inputs are required. BashGym pauses when approval is needed.
                            </span>
                          </label>
                          <details className="mcp-technical-details">
                              ...
                              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                                <label className="node-field">
                                  ...
                                  <input
                                    ...
                                    onChange={(event) => dispatchUi({ type: 'set_timeout_seconds', value: event.target.value })}
                                  />
                                </label>
                                <label className="node-field">
                                  ...
                                  <input
                                    ...
                                    onChange={(event) => dispatchUi({ type: 'set_max_result_kilobytes', value: event.target.value })}
                                  />
                                </label>
                              </div>
                              <p className="text-[10px] leading-4 text-text-muted">
                                Calls stop after 30 seconds and keep at most 1 MB by default. Raise either limit only for a tool you trust.
                              </p>
                            </div>
                          </details>
                          {!connectionVerified ? (
