...
test('campaign bridge allowlists exact REST routes and rejects renderer escape hatches', () => {
  ...
  validateCampaignRequest('GET', '/api/campaigns/campaign-1/control-room-snapshot', undefined, {
    workspace_id: 'workspace-a'
  })
  validateCampaignRequest('GET', '/api/campaigns/campaign-1/human-work', undefined, {
    workspace_id: 'workspace-a',
    limit: 50
  })
  validateCampaignRequest('GET', '/api/campaigns/campaign-1/agent-attachment', undefined, {
    workspace_id: 'workspace-a',
    after_sequence: 0,
    limit: 20
  })
  validateCampaignRequest('GET', '/api/campaigns/campaign-1/recovery', undefined, {
  ...
  validateCampaignRequest('POST', '/api/campaigns/setup/doctor', {
    ...
    installation_id: `ins_${'c'.repeat(32)}`,
    bindings: {
  ...
  validateCampaignRequest(
    ...
    {
      action: 'repair',
      idempotency_key: `idem_${'a'.repeat(32)}`,
      workspace_id: 'workspace-a',
      campaign_id: 'campaign-1',
      eligibility_receipt_id: `rcpt_${'b'.repeat(32)}`,
      doctor_evidence_id: `evd_${'c'.repeat(32)}`,
      expected_campaign_revision: 2,
      expected_event_cursor: 17,
      expected_aggregate_version: 9,
      expected_controller_lease_id: null,
      checkpoint_id: `ckpt_${'d'.repeat(32)}`,
  ...
  validateCampaignRequest('GET', '/api/campaigns/campaign-1/artifacts', undefined, {
    workspace_id: 'workspace-a',
  ...
  assert.throws(
    () =>
      validateCampaignRequest('GET', '/api/campaigns/setup/context', undefined, {
        workspace_id: 'workspace-a',
        session_id: 'not-a-sealed-session'
      }),
    /guided setup/i
  )
  assert.throws(
...
test('main-only campaign-agent transport uses exact authenticated routes without renderer allowlisting', async () => {
  ...
  const client = new CampaignBridgeClient(
    ...
    async (input, init) => {
      ...
      if (String(input).endsWith('/api/campaign-auth/exchange')) {
        return new Response(JSON.stringify({ raw_token: `bgca.access-1.${'a'.repeat(48)}` }), {
          status: 200
        })
      }
  ...
  await client.revokeCampaignAgentAttachment('campaign-a', 'attachment-1', {
    action: 'revoke-test'
  })
  ...
  const client = new CampaignBridgeClient(
    ...
    async (input, init) => {
      ...
      if (url.endsWith('/heartbeat')) {
        return new Response(JSON.stringify({ schemaVersion: 'campaign_agent_attachment.v1' }), {
          status: 200
        })
      }
      if (url.endsWith('/actions/observe')) {
        return new Response(JSON.stringify({ schemaVersion: 'campaign_agent_observation.v1' }), {
          status: 200
        })
      }
      return new Response(
...
test('main-only credential adapters reject unbounded inputs and sanitize every failure', async () => {
  ...
  await assert.rejects(
    ...
    (error: Error) =>
      error.message === 'Campaign agent credential request failed' &&
  ...
  const escapedEchoClient = new CampaignBridgeClient(
    ...
    `bgcb.launch.${'s'.repeat(48)}`,
    async () =>
      new Response(`{"raw_token":"bgag\\u002ecredential-2.${'y'.repeat(48)}"}`, { status: 200 })
  )
  await assert.rejects(
...
test('main-only credential adapters preserve only 401/403 authority status', async () => {
  ...
  await assert.rejects(
    ...
    (error: Error & { status?: number }) =>
      error.message === 'Campaign agent credential request failed' && error.status === undefined
  )
})
...
test('campaign bridge bounds auth exchange and campaign REST transport', async () => {
  ...
  const requestClient = new CampaignBridgeClient(
    ...
    bootstrap,
    async (input, init) =>
      String(input).endsWith('/api/campaign-auth/exchange')
        ? new Response(JSON.stringify({ raw_token: `bgca.access-1.${'a'.repeat(48)}` }), {
            status: 200
          })
  ...
  const response = await requestClient.request('GET', '/api/campaigns', undefined, {
    workspace_id: 'workspace-a'
  })
...
test('campaign bridge forwards only code-specific allowlisted error metadata', async () => {
  ...
  const response = await client.request('GET', '/api/campaigns/campaign-1/events', undefined, {
    workspace_id: 'workspace-a',
    after_cursor: 3
  })
