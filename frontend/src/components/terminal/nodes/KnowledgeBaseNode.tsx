  ...
  return (
    <>
      <button
        ...
        className={clsx(
          'knowledge-tree-row',
          selectedPath === node.path && 'knowledge-tree-row-selected'
        )}
...
export const KnowledgeBaseNode = memo(function KnowledgeBaseNode({
  data,
  selected
}: NodeProps<KnowledgeBaseNodeType>) {
  const initialConfig = useMemo(
    () => sanitizeKnowledgeConfig(data.adapterConfig),
    [data.adapterConfig]
  )
  ...
  const [section, setSection] = useState<KnowledgeSection>(
    initialConfig.selectedSection ?? 'browse'
  )
  ...
  const discoveredGbrainSources = useMemo(
    () =>
      (status?.gbrain.sources ?? []).flatMap((source) => {
  ...
  const load = useCallback(
    async (target: KnowledgeNodeConfig = config) => {
      setLoading(true)
      setError(null)
      try {
        ...
        if (!response.ok || !response.data)
          throw new Error(response.error || 'Knowledge source unavailable')
  ...
  return (
    <>
      <DataNodeShell
        ...
        buildContext={
          data.hasTerminalConnections
            ? () => buildKnowledgeContext(config, snapshot, searchResult)
            : undefined
        }
        statusBarClass={
          error ? 'bg-status-error' : ready ? 'bg-status-success' : 'bg-status-warning'
        }
      ...
      >
        <div className="grid grid-cols-3 gap-1.5">
          <div className="node-section !p-2">
            <div className="node-field-label">Source</div>
            <div className="mt-1 truncate text-[10px] font-mono text-text-primary">
              {config.provider === 'gbrain' ? 'GBrain' : 'Workspace'}
            </div>
          </div>
          <div className="node-section !p-2">
            <div className="node-field-label">Docs shown</div>
            <div className="mt-1 text-sm font-mono font-bold text-text-primary">
              {snapshot?.counts.knowledge_files ?? 0}
              {snapshot?.truncated ? '+' : ''}
            </div>
          </div>
          <div className="node-section !p-2">
            <div className="node-field-label">Folders</div>
            <div className="mt-1 text-sm font-mono font-bold text-text-primary">
              {snapshot?.counts.folders ?? 0}
              {snapshot?.truncated ? '+' : ''}
            </div>
          </div>
        </div>
        <div className="mt-2 flex items-center gap-2 rounded-brutal border-brutal border-border-subtle bg-background-card p-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-[10px] font-mono font-bold text-text-primary">
              {snapshot?.label || 'No source loaded'}
            </div>
            <div
              className="mt-0.5 truncate text-[9px] font-mono text-text-muted"
              title={snapshot?.root || config.root}
            >
              {snapshot?.root || config.root || 'Choose a source'}
            </div>
          </div>
          <ConfigPill tone={ready ? 'success' : error ? 'error' : 'warning'}>
            {ready ? 'ready' : error ? 'error' : 'loading'}
          </ConfigPill>
        </div>
        {error ? (
          <div className="mt-2 line-clamp-2 text-[9px] font-mono text-status-error">{error}</div>
        ) : null}
        <button
          type="button"
          className="node-btn node-btn-wide node-btn-accent mt-2 w-full justify-center"
      ...
      <NodeConfigModal
        ...
        title={`${title} Lab`}
        description="Browse, search, and publish cited context to connected canvas nodes"
        size="xl"
        layout="workspace"
      >
          ...
          <div className="knowledge-lab-content">
            {section === 'browse' ? (
              <div className="knowledge-browser">
                <ConfigSection
                  title={snapshot?.truncated ? 'Source tree · representative view' : 'Source tree'}
                  className="knowledge-tree-panel"
                >
                ...
                >
                  {preview ? (
                    <pre className="knowledge-preview-content">{preview.content}</pre>
                  ) : (
                    <div className="knowledge-empty-state">
                      Select a text or source file to inspect it with its provenance path.
                    </div>
                  )}
                </ConfigSection>
              </div>
            ...
            {section === 'search' ? (
              <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <label className="relative min-w-0 flex-1">
                      ...
                      <input
                        ...
                        placeholder="What should connected agents know?"
                      />
                    </label>
                    <button
                      type="button"
                ...
                <ConfigSection
                  title={searchResult ? `${searchResult.results.length} cited results` : 'Results'}
                >
                  {searchResult?.results.length ? (
                    searchResult.results.map((result) => (
                      ...
                      >
                        <span className="font-mono text-[10px] font-bold text-accent-dark">
                          {result.path}
                        </span>
                        <span className="mt-1 line-clamp-3 text-[11px] leading-5 text-text-secondary">
                          {result.snippet}
                        </span>
                      </button>
                    ))
                  ) : (
                    <div className="py-10 text-center text-[10px] text-text-muted">
                      Search results keep source paths so downstream agents can cite them.
                    </div>
                  )}
                </ConfigSection>
              </div>
              ...
              <div className="space-y-3">
                <ConfigSection title="Knowledge source">
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <label className="node-field">
                      <span className="node-field-label">Node name</span>
                      <input
                    ...
                    <label className="node-field">
                      <span className="node-field-label">Provider</span>
                      <select
                        className="input-brutal min-h-9 text-[11px] font-mono"
                        ...
                        onChange={(event) =>
                          setDraft((current) => ({
                            ...current,
                            provider: event.target.value === 'gbrain' ? 'gbrain' : 'workspace',
                            root: ''
                          }))
                        }
                      >
                        <option value="workspace">Workspace files</option>
                        <option value="gbrain">GBrain source</option>
                      </select>
                    </label>
                    <label className="node-field md:col-span-2">
                      <span className="node-field-label">Source directory</span>
                      <input
                        className="input-brutal min-h-9 text-[11px] font-mono"
                        value={draft.root}
                        onChange={(event) =>
                        ...
                        placeholder={status?.workspace_root || 'Defaults to this workspace'}
                      />
                    </label>
                  </div>
                  {draft.provider === 'gbrain' && !status?.gbrain.installed ? (
                    <div className="flex gap-2 rounded-brutal border-brutal border-status-warning/60 bg-status-warning/10 p-2 text-[10px] leading-4 text-text-secondary">
                      <AlertCircle className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-status-warning" />
                      <span>
                        GBrain is not installed on this machine. Workspace browsing remains
                        available now.
                      </span>
                    </div>
                  ) : null}
                  {discoveredGbrainSources.length ? (
                    <div className="space-y-1">
                      <div className="node-field-label">Discovered GBrain sources</div>
                      {discoveredGbrainSources.map((source) => (
                        <button
                          key={source.path}
                          type="button"
                          className="knowledge-source-option"
                          onClick={() =>
                            setDraft((current) => ({
                              ...current,
                              provider: 'gbrain',
                        ...
                        >
                          ...
                          <span className="truncate text-text-muted">{source.path}</span>
                        </button>
                      ))}
                    </div>
                  ) : null}
                  <div className="node-config-action-row">
                    ...
                    >
                      <Settings2 className="h-3 w-3" /> Save and inspect
                    </button>
                    <button
                      type="button"
                ...
                <ConfigSection title="Context contract">
                  <p className="text-[11px] leading-5 text-text-secondary">
                    Connected apps and nodes receive a read-only context packet with provider,
                    source root, counts, and cited search results. Hidden environment files are
                    excluded.
                  </p>
                  <ConfigRows>
                    <ConfigRow
                      label="Flow"
                      value="Sources → files → concepts/relations → cited context"
                    />
                    <ConfigRow
                      label="Available now"
                      value="Browse, safe preview, keyword search, canvas context"
                    />
                    ...
                    <ConfigRow
                      ...
                      value={
                        data.hasConnections
                          ? 'Published to connected nodes'
                          : 'Connect this node to publish context'
                      }
                    />
                  </ConfigRows>
                </ConfigSection>
