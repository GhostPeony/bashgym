    ...
    <div className="mt-1 space-y-px">
      {Object.entries(deltas).map(([key, delta]) => (
        <div key={key} className="flex items-center gap-1.5 text-[9px] font-mono">
          <span className="flex-1 truncate text-text-muted" title={key}>
            {metricLabel(key)}
          </span>
  ...
  const buildContext = () => {
    ...
    if (r.delta_metrics) {
      ...
      for (const [key, delta] of Object.entries(r.delta_metrics)) {
        ...
        const pair =
          base !== undefined && cand !== undefined ? `${base.toFixed(4)} → ${cand.toFixed(4)}` : ''
          ...
          <div className="space-y-1.5">
            {jobs.map((job) => {
              ...
              return (
                <div
                  key={job.job_id}
                  className="border-brutal border-border-subtle rounded-brutal px-2 py-1.5"
                >
                  <div
                    className={clsx(
                  ...
                  >
                    ...
                    <span className="flex-1 truncate text-text-secondary" title={job.model_id}>
                      {job.model_id}
                    </span>
                    {job.report && (
                      <span
                  ...
                  {job.report && (
                    <div className="text-[9px] font-mono text-text-muted mt-0.5">
                      {(job.report.candidate_pass_rate * 100).toFixed(1)}% vs base{' '}
                      {(job.report.base_pass_rate * 100).toFixed(1)}%
                    </div>
                  )}
              ...
              >
                ...
                {run.kpis ? (
                  <div className="mt-0.5 font-mono text-[9px] text-text-muted">
                    uplift {run.kpis.success_uplift >= 0 ? '+' : ''}
                    {Math.round(run.kpis.success_uplift * 100)}% · route F1{' '}
                    {Math.round(run.kpis.routing_f1 * 100)}%
                  </div>
                ) : null}
              </div>
            ))}
      ...
      >
        <ConfigSection title="Eval State">
          ...
          <ConfigRows>
            ...
            <ConfigRow
              ...
              value={
                latest?.report
                  ? `${(latest.report.candidate_pass_rate * 100).toFixed(1)}%`
                  : undefined
              }
            />
            <ConfigRow
              label="Base"
              value={
                latest?.report ? `${(latest.report.base_pass_rate * 100).toFixed(1)}%` : undefined
              }
            />
        ...
        <ConfigSection title="Live Handles">
          <ConfigRows>
            ...
            <ConfigRow
              label="Latest eval"
              value={latest ? `${API_BASE}/eval/heldout/${latest.job_id}` : undefined}
            />
        ...
        <ConfigSection title={`Skill evals (${skillRuns.length})`}>
          <ConfigRows>
            <ConfigRow
              label="Running"
              value={
                skillRuns.filter((run) => run.status === 'queued' || run.status === 'running')
                  .length
              }
            />
            ...
            <ConfigRow
              ...
              value={skillRuns[0]?.kpis?.verdict || skillRuns[0]?.status}
            />
            <ConfigRow
              label="Latest uplift"
              value={
                skillRuns[0]?.kpis
                  ? `${Math.round(skillRuns[0].kpis.success_uplift * 100)}%`
                  : undefined
              }
            />
          </ConfigRows>
