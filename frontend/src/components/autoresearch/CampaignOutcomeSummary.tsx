...
function isRateMetric(metric: CampaignOutcomeMetric): boolean {
  return (
    /accuracy|reward|rate|mrr|pass|precision|recall|f1/i.test(metric.id) &&
    [metric.baseline, metric.candidate].every(
...
function OutcomeIcon({
  ...
  verdict: CampaignOutcomeViewModel['verdict']
  className?: string
}) {
  if (verdict === 'success') return <CheckCircle2 className={className} />
  if (verdict === 'not_improved' || verdict === 'pending')
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-border-subtle px-2 py-1.5">
        <div
          className={clsx(
            'flex min-w-0 items-center gap-1.5 font-mono text-[9px] font-bold uppercase',
            model.verdict === 'success' ? 'text-status-success' : 'text-status-warning'
          )}
        >
          <OutcomeIcon verdict={model.verdict} className="h-3 w-3 shrink-0" />
          <span className="truncate">{model.verdictLabel}</span>
        </div>
        <span className="shrink-0 font-mono text-[8px] uppercase text-text-muted">
      ...
      {primary ? (
        <div className="grid grid-cols-[1fr_auto] items-end gap-2 px-2 py-1.5">
          <div className="min-w-0">
            <div className="truncate font-mono text-[7px] font-bold uppercase tracking-wide text-text-muted">
              {readable(primary.id)}
            </div>
          ...
          <div
            className={clsx(
              ...
              improves(primary) === false ? 'text-status-error' : 'text-status-success'
            )}
          >
            {formatDelta(primary)}
          </div>
    ...
    >
      <header className="flex flex-col gap-3 border-b border-border-subtle p-4 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 font-brand text-xl text-text-primary">
            {model.baselineLabel} vs {model.candidateLabel}
          </h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span
            className={clsx(
              'inline-flex items-center gap-1.5 rounded-brutal border-brutal px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase',
              verdictClasses(model.verdict)
            )}
          >
            <OutcomeIcon verdict={model.verdict} className="h-3.5 w-3.5" />
            {model.verdictLabel}
          </span>
      ...
      <div className="grid gap-px bg-border-subtle lg:grid-cols-[minmax(0,1.2fr)_minmax(20rem,0.8fr)]">
        <div className="min-w-0 bg-background-card p-4">
          <div className="grid grid-cols-[minmax(8rem,1fr)_5.5rem_6.5rem_5.5rem] gap-2 border-b-2 border-border pb-2 font-mono text-[10px] font-bold uppercase tracking-wide text-text-muted">
            <span>Metric</span>
            <span className="text-right">{model.baselineLabel}</span>
            <span className="text-right">{model.candidateLabel}</span>
            ...
            {model.metrics.map((metric) => {
              ...
              return (
                <div
                  key={metric.id}
                  className={clsx(
                    'grid grid-cols-[minmax(8rem,1fr)_5.5rem_6.5rem_5.5rem] items-baseline gap-2 py-3',
                    metric.primary && 'font-bold'
                  )}
                >
                  <span className="truncate text-sm text-text-primary" title={readable(metric.id)}>
                    {readable(metric.id)}
                    {metric.primary ? (
                  ...
                  <span className="text-right font-mono text-xs text-text-secondary">
                    {formatValue(metric, metric.baseline)}
                  </span>
                  <span className="text-right font-mono text-xs text-text-primary">
                    {formatValue(metric, metric.candidate)}
                  </span>
                  <span
                    className={clsx(
                      'flex items-center justify-end gap-1 text-right font-mono text-xs',
                      improvement === true
                        ? 'text-status-success'
                        : improvement === false
                          ? 'text-status-error'
          ...
          <div className="mt-2 grid gap-1 border-t border-border-subtle pt-3 font-mono text-[10px] text-text-muted sm:grid-cols-2">
            <div className="truncate" title={model.baselineEvaluationId || undefined}>
              Baseline eval · {model.baselineEvaluationId || 'not linked'}
            </div>
            <div className="truncate" title={model.candidateEvaluationId || undefined}>
              Candidate eval · {model.candidateEvaluationId || 'not linked'}
            </div>
          </div>
        </div>
        ...
        <div className="min-w-0 bg-background-secondary p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="font-brand text-base text-text-primary">Training loss</div>
              <div className="font-mono text-[10px] text-text-muted">
                {model.loss?.attemptId || 'No persisted series'}
              </div>
            </div>
            {model.loss ? (
              <div className="text-right font-mono text-[10px] text-text-muted">
                <div>
                  Minimum {model.loss.minimum.value.toFixed(4)} at step {model.loss.minimum.step}
                </div>
                <div>
                  Final {model.loss.final.value.toFixed(4)} at step {model.loss.final.step}
                </div>
              </div>
            ) : null}
          </div>
          {model.loss && chartData.length > 1 ? (
            ...
            <div className="mt-3 border-brutal border-border-subtle bg-background-card p-4 text-xs text-text-muted">
              No persisted loss series is available.
            </div>
          )}
...
export function CampaignOutcomeSummary({
  ...
  model: CampaignOutcomeViewModel
  density?: 'compact' | 'detailed'
}) {
  return density === 'compact' ? (
    <CompactOutcome model={model} />
  ) : (
    <DetailedOutcome model={model} />
  )
}
