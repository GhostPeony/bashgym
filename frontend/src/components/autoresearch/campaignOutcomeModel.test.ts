...
function outcomeDetail(): CampaignDetailState {
  return {
    ...
    attempts: [
      {
        attempt_id: 'attempt-lora-train',
        stage: 'full_training',
        updated_at: '2026-07-18T02:00:00Z'
      },
      {
        attempt_id: 'attempt-lora-eval',
    ...
    ledger: {
          ...
          evaluations: [
            {
              ...
              status: 'completed',
              metrics: {
                exact_count_accuracy: 0.078125,
                count_accuracy: 0.359375,
                format_accuracy: 0.8125,
                mean_reward: 0.38203125
              },
            ...
            },
            {
              ...
              status: 'completed',
              metrics: {
                exact_count_accuracy: 0.65625,
                count_accuracy: 0.89453125,
                format_accuracy: 1,
                mean_reward: 0.8998046875
              },
      ...
      autoresearch: {
        ...
        state: {
          ...
          best_proposal_id: 'candidate-lora-sft-160-qwen35-run1',
          best_metric: 0.65625,
          attempts_used: 2,
          proposals_used: 2,
          budget_used: 3,
        ...
        proposals: [
          { proposal_id: 'candidate-lora-sft-160-qwen35-run1', training_method: 'lora_sft' }
        ],
        outcomes: [
          {
            result: {
              result_id: 'baseline-result',
              proposal_id: 'baseline-fixed64',
              study_id: 'baseline-study',
              role: 'baseline',
              provenance: 'real',
              outcome: 'completed',
              metric_name: 'exact_count_accuracy',
              metric_value: 0.078125,
              actual_cost: 0,
              attempt_ids: ['run-baseline'],
              evidence_references: ['evaluation-baseline-fixed64-qwen35-run1'],
          ...
          },
          {
            result: {
              ...
              proposal_id: 'candidate-lora-sft-160-qwen35-run1',
              study_id: 'candidate-study',
              role: 'candidate',
              provenance: 'real',
              outcome: 'completed',
              metric_name: 'exact_count_accuracy',
              metric_value: 0.65625,
              actual_cost: 3,
              attempt_ids: ['attempt-lora-train', 'attempt-lora-eval'],
              evidence_references: ['evaluation-candidate-lora-160-qwen35-run1'],
              recorded_at: '2026-07-18T03:00:00Z'
            },
            decision: {
              proposal_id: 'candidate-lora-sft-160-qwen35-run1',
              decision: 'keep',
              reason_code: 'metric_improved',
              eligible_for_best: true,
              improvement: 0.578125,
          ...
          signals: [
            {
              ...
              severity: 'warning',
              summary: 'Only the terminal checkpoint was evaluated.',
              evidence_references: ['evaluation-candidate-lora-160-qwen35-run1']
            }
          ],
...
test('projects a complete same-suite baseline versus LoRA candidate result', () => {
  ...
  assert.deepEqual(
    model.metrics.map((metric) => [metric.id, metric.baseline, metric.candidate, metric.delta]),
    [
