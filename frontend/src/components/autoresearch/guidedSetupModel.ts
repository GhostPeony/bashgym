...
function parseBindings(value: unknown): GuidedSetupBindings | null {
  ...
  return Object.fromEntries(
    bindingKinds.map((kind) => [kind, item[kind]])
  ) as unknown as GuidedSetupBindings
}

function parseBindingChoice(value: unknown): GuidedSetupBindingChoice | null {
  const item = exact(
    value,
    ['logical_id', 'availability', 'selectable', 'reason_codes'],
    ['display_label', 'integration_label']
  )
  if (
    !item ||
    !safeString(item.logical_id) ||
    !['reachable', 'inaccessible', 'unknown'].includes(String(item.availability)) ||
...
function parseStepReceipt(value: unknown): GuidedSetupStepReceipt | null {
  ...
  if (
    ...
    !boundedInteger(item.version, 6) ||
    Number(item.version) < 1 ||
    item.step !== guidedSetupSteps[Number(item.version) - 1] ||
    !safeString(item.selection_id) ||
    !safeString(item.state_digest, hexDigest, 64) ||
    !(item.previous_receipt_id === null || safeString(item.previous_receipt_id, stepReceiptId)) ||
    !(
      item.previous_receipt_digest === null ||
      safeString(item.previous_receipt_digest, sealedDigest, 71)
    ) ||
    (Number(item.version) === 1) !==
      (item.previous_receipt_id === null && item.previous_receipt_digest === null) ||
    (item.previous_receipt_id === null) !== (item.previous_receipt_digest === null) ||
    !isCanonicalUtc(item.created_at) ||
    !safeString(item.receipt_digest, sealedDigest, 71)
  )
    return null
...
function parseSession(value: unknown): GuidedSetupSession | null {
  ...
  const selections = item
    ? exact(item.selections, ['template_id', 'installation_id', 'bindings'])
    : null
  ...
  if (
    !item ||
    item.schema_version !== 'guided_setup_session.v1' ||
    !safeString(item.workspace_id) ||
    !safeString(item.session_id, sessionId) ||
    !boundedInteger(item.version, 6) ||
    Number(item.version) < 1 ||
    !Array.isArray(item.completed_steps) ||
    item.completed_steps.length !== item.version ||
    !item.completed_steps.every((step, index) => step === guidedSetupSteps[index]) ||
    !selections ||
    !(selections.template_id === null || safeString(selections.template_id)) ||
    !(
      selections.installation_id === null || safeString(selections.installation_id, installationId)
    ) ||
    !bindings ||
    !bindingKinds.every((kind) => bindings[kind] === null || safeString(bindings[kind])) ||
    typeof item.ready_for_validation !== 'boolean' ||
    !safeReasonCodes(item.reason_codes) ||
    !receipt ||
    receipt.session_id !== item.session_id ||
    receipt.version !== item.version ||
    item.ready_for_validation !== (Number(item.version) === guidedSetupSteps.length) ||
    !isCanonicalUtc(item.updated_at)
  )
    return null
  const selected = [
    selections.template_id,
    selections.installation_id,
    ...bindingKinds.map((kind) => bindings[kind])
  ]
  if (
    selected.some((value, index) =>
      index < Number(item.version) ? value === null : value !== null
    )
  )
    return null
...
export function parseGuidedSetupContext(value: unknown): GuidedSetupContext | null {
  ...
  if (
    ...
    item.templates.length > guidedSetupLimit ||
  ...
  const installations = item.installations.map((value): GuidedSetupInstallation | null => {
    ...
    const truncation = row
      ? exact(row.truncation, ['truncated', 'reason_codes', 'limit_per_kind', 'kinds'])
      : null
    if (
      !row ||
      !safeString(row.installation_id, installationId) ||
      typeof row.ready !== 'boolean' ||
      !safeReasonCodes(row.reason_codes) ||
      !bindings ||
      !truncation ||
      typeof truncation.truncated !== 'boolean' ||
      !safeReasonCodes(truncation.reason_codes) ||
      truncation.limit_per_kind !== guidedSetupLimit ||
      !Array.isArray(truncation.kinds) ||
      new Set(truncation.kinds).size !== truncation.kinds.length ||
      !truncation.kinds.every((kind) => bindingKinds.includes(kind as GuidedBindingKind))
    )
      return null
    const parsedBindings = Object.fromEntries(
      bindingKinds.map((kind) => {
        const rows = bindings[kind]
        if (!Array.isArray(rows) || rows.length > Number(truncation.limit_per_kind))
          return [kind, null]
  ...
  const limits = truncation
    ? exact(truncation.limits, ['templates', 'installations', 'bindings_per_kind'])
    : null
  if (
    !truncation ||
    typeof truncation.truncated !== 'boolean' ||
    !safeReasonCodes(truncation.reason_codes) ||
    !limits ||
    limits.templates !== guidedSetupLimit ||
    limits.installations !== guidedSetupLimit ||
    limits.bindings_per_kind !== guidedSetupLimit
  )
    return null
  ...
  if (
    new Set(templates.map((row) => row!.template_id)).size !== templates.length ||
    new Set(installations.map((row) => row!.installation_id)).size !== installations.length
  )
    return null
  return {
    ...item,
    ...
    installations,
    session,
    truncation: { ...truncation, limits }
  } as unknown as GuidedSetupContext
}

function parseDoctorBase(
  value: unknown,
  withReceipt: boolean
): GuidedSetupDoctor | GuidedSetupValidation | null {
  const required = [
    'schema_version',
    'workspace_id',
    'template_id',
  ...
  if (
    ...
    !safeString(item.definition_digest, hexDigest, 64) ||
    !safeString(item.draft_digest, hexDigest, 64) ||
  ...
  const references = item.binding_references.map((value) =>
    exact(value, ['kind', 'logical_id', 'availability'])
  )
  if (
    references.some(
      (row) =>
        !row ||
        !bindingKinds.includes(row.kind as GuidedBindingKind) ||
        !safeString(row.logical_id) ||
        !['reachable', 'inaccessible', 'unknown'].includes(String(row.availability))
    )
  )
    return null
  ...
  if (
    item.ready &&
    (item.blocking_codes.length !== 0 ||
      references.some((row) => row!.availability !== 'reachable'))
  )
    return null
...
export function parseGuidedSetupMutation(value: unknown): GuidedSetupMutation | null {
  ...
  return item?.schema_version === 'guided_setup_session_mutation.v1' &&
    session &&
    receipt &&
    session.latest_receipt.receipt_id === receipt.receipt_id
    ? { schema_version: item.schema_version, session, receipt }
    : null
}
...
export function parseGuidedSetupCreation(value: unknown): GuidedSetupCreation | null {
  ...
  if (
    ...
    campaign.title.length > 240 ||
    !safeString(campaign.status) ||
...
export function buildGuidedSetupView(context: GuidedSetupContext): GuidedSetupView {
  ...
  const currentStep =
    completedCount >= guidedSetupSteps.length ? 'doctor' : guidedSetupSteps[completedCount]
  const installation = context.installations.find(
    (item) => item.installation_id === context.session?.selections.installation_id
  )
  ...
  if (currentStep === 'template') {
    ...
    options = installation.bindings[kind]
      ...
      .map((item) => ({
        id: item.logical_id,
        label: item.display_label || item.logical_id,
        detail: item.integration_label || null,
  ...
  return {
    ...
    completedCount,
    options,
    truncationReasons: [...context.truncation.reason_codes],
