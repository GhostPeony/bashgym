...
const allReceipts: SetupFlowInput['receipts'] = [
  ...
  { stepId: 'model', status: 'complete', receiptId: 'receipt-model', summary: 'Immutable model identity saved.' },
  { stepId: 'data', status: 'complete', receiptId: 'receipt-data', summary: 'Data binding saved.' },
  { stepId: 'evaluator', status: 'complete', receiptId: 'receipt-evaluator', summary: 'Evaluator binding saved.' },
  { stepId: 'compute', status: 'complete', receiptId: 'receipt-compute', summary: 'Private SSH compute registered.' },
  { stepId: 'budget_stops', status: 'complete', receiptId: 'receipt-budget', summary: 'Budget saved.' },
  { stepId: 'activation_doctor', status: 'complete', receiptId: 'receipt-doctor', summary: 'Doctor completed.' },
  { stepId: 'campaign_creation', status: 'complete', receiptId: 'receipt-campaign', summary: 'Campaign created.' },
  { stepId: 'baseline_review', status: 'complete', receiptId: 'receipt-baseline', summary: 'Baseline reviewed.' },
]
...
test('keeps the setup contract in the required durable order without a model default', () => {
  const model = buildSetupFlowModel(input({
    receipts: [],
    drafts: {
  ...
  assert.deepEqual(setupStepOrder, [
    'source', 'model', 'data', 'evaluator', 'compute', 'budget_stops',
    'activation_doctor', 'campaign_creation', 'baseline_review', 'start',
  ])
  assert.equal(model.steps[1]?.label, 'Model identity & revision')
  assert.equal(model.steps[1]?.receipt, null)
  assert.deepEqual(model.steps[1]?.editor?.fields.map((field) => field.id), ['model_ref', 'model_revision'])
  assert.equal(model.steps[1]?.editor?.fields.every((field) => field.value === ''), true)
  assert.equal(model.steps.find((step) => step.id === 'compute')?.editor?.fields[0]?.value, 'private-compute-1')
  assert.equal(model.steps.find((step) => step.id === 'activation_doctor')?.editor?.fields[0]?.value, 'registered-ssh-1')
  assert.equal(model.steps.some((step) => /qwen|recommended model/i.test(`${step.label} ${step.guidance}`)), false)
})
...
test('keeps completed receipts complete and resumes a retryable partial operation without repeating it', () => {
  const model = buildSetupFlowModel(input({
    receipts: [
      allReceipts[0]!,
      { stepId: 'model', status: 'partial', receiptId: 'receipt-model', summary: 'Revision check can resume.', retryable: true },
    ],
  ...
  assert.deepEqual(model.blockers.map((blocker) => blocker.code), ['ssh_registration_required'])
})

test('offers retry only for a retryable failed receipt and never echoes an unsafe receipt identifier', () => {
  const retryable = buildSetupFlowModel(input({
    receipts: [
      allReceipts[0]!,
      { stepId: 'model', status: 'failed', receiptId: 'receipt-model-2', summary: 'Immutable revision check failed.', retryable: true },
    ],
  }))
  assert.equal(retryable.steps[1]?.action, 'retry')
  assert.equal(retryable.steps[1]?.actionReceiptId, 'receipt-model-2')

  const unsafe = buildSetupFlowModel(input({
    receipts: [
      allReceipts[0]!,
      { stepId: 'model', status: 'partial', receiptId: '../../private-key', summary: 'Resume.', retryable: true },
    ],
  }))
...
test('enables only an explicitly mounted existing campaign mutation contract', () => {
  const withoutBridge = buildSetupFlowModel(input({
  ...
  const withBridge = buildSetupFlowModel(input({
    receipts: allReceipts.filter((receipt) => receipt.stepId !== 'campaign_creation'),
    drafts: { template_id: 'installed-template-1', campaign_id: 'campaign-1' },
...
test('treats optional integrations as optional while private SSH compute is required', () => {
  ...
  assert.equal(model.blockers.some((blocker) => blocker.code === 'nemo'), false)
})
