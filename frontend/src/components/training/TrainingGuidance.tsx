...
type GuideSectionId =
  'start' | 'capabilities' | 'strategies' | 'metrics' | 'world-models' | 'sources'
...
const capabilityRows = [
  {
    ...
    status: 'Ready',
    promise:
      'Train a first local student from gold traces using Unsloth or plain Transformers scripts.',
  ...
  },
  {
    ...
    proof:
      'One installed verl, SkyRL, or OpenRLHF smoke with saved mask telemetry and pass@k evidence.'
  },
  {
    ...
    proof:
      'Backend-emitted ECHO loss, RWML quality metrics, and correlation with held-out behavior.'
  },
  {
    capability: 'Release gates',
    status: 'Ready',
    promise:
      'Combine trace, environment, safety, and external benchmark evidence before promotion.',
    proof:
      'Heldout verdict, holdout comparison, spurious controls, tamper canaries, benchmark manifests.'
  }
]
...
const worldModelRows = [
  {
    ...
    setup:
      'Start at lambda 0.05. Watch observation loss beside pass@k rather than optimizing it alone.'
  },
  {
    name: 'RWML',
    role: 'Latent next-state reward',
    setup:
      'Keep hard transitions, subsample easy dynamics, and track embedding-distance distributions.'
  },
  {
    name: 'DPPO replay telemetry',
    role: 'Coverage check for world-model payloads',
    setup:
      'Use replay summaries to confirm masks, rewards, and observation pairs are present before trainer integration.'
  }
]
...
const sourceRows = [
  {
    ...
    leverage:
      'Dataset formats, chat-template behavior, completion-only training, and PEFT adapter setup.'
  },
  {
    name: 'Hugging Face TRL - DPO',
    href: 'https://huggingface.co/docs/trl/en/dpo_trainer',
    leverage:
      'Preference dataset shape, prompt/chosen/rejected requirements, and conversational formatting.'
  },
  {
  ...
  },
  {
    ...
    leverage:
      'Operator loop: grader, train/validation split, job monitoring, checkpoint evaluation, and iteration.'
  },
  {
    name: 'verl',
        ...
        <thead>
          <tr className="border-b border-border bg-background-tertiary">
            {headers.map((header) => (
              <th
                key={header}
                className="px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-text-muted"
              >
                {header}
              </th>
            ))}
              ...
              {row.map((cell, index) => (
                <td
                  ...
                  className={clsx(
                    ...
                    index === 0 &&
                      'font-mono text-xs font-bold uppercase tracking-wide text-text-primary'
                  )}
                >
                  {cell}
                </td>
      ...
      <div className="border-brutal border-border rounded-brutal bg-background-secondary p-5 shadow-brutal-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="font-brand text-3xl text-text-primary mt-2">
              Operator guidance for better training runs
            </h2>
            <p className="mt-2 text-sm leading-relaxed text-text-secondary max-w-3xl">
              Use these pages when choosing a strategy, setting up a run, reading metrics, or
              deciding whether a checkpoint has enough held-out evidence to trust.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:w-[440px]">
            {['SFT first', 'DPO pairs', 'GRPO variance', 'Holdout gates'].map((item) => (
              <div
                key={item}
                className="border-brutal border-border rounded-brutal bg-background-card px-3 py-2"
              >
              ...
              <div className="border-l-4 border-l-accent border-brutal border-border rounded-brutal bg-background-card p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-relaxed text-text-secondary">
                      Run SFT on clean gold traces, evaluate held-out pass@k, then use DPO or GRPO
                      only when the dataset and verifier produce a real contrast signal. World-model
                      objectives should explain dynamics, not replace release evidence.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {activeSection === 'capabilities' && (
            <>
              ...
              <div className="border-brutal border-border rounded-brutal bg-background-secondary p-4">
                <p className="font-mono text-xs uppercase tracking-widest text-text-primary">
                  Truth boundary
                </p>
                <p className="mt-2 text-sm leading-relaxed text-text-secondary">
                  DPPO and ECHO/RWML are real platform contracts, not empty labels. They should
                  still be presented as backend-dependent until an installed trainer run produces
                  artifacts. World-model quality remains diagnostic release evidence until it
                  predicts held-out pass@k and safety outcomes.
                </p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  [
                    'Yellow light',
                    'loss improves but held-out behavior is flat or truncation rises.'
                  ],
                  [
                    'Red light',
                    'tamper, leakage, zero reward variance, or verifier exploit evidence.'
                  ]
                ].map(([title, body]) => (
          ...
          {activeSection === 'world-models' && (
            <>
              ...
              <div className="border-brutal border-border rounded-brutal bg-background-secondary p-4">
                ...
                <p className="mt-2 text-sm leading-relaxed text-text-secondary">
                  Show ECHO lambda, RWML distance, easy-transition filtering, history window, and
                  replay coverage near the GRPO settings because those values only make sense in the
                  rollout context.
                </p>
              </div>
            </>
