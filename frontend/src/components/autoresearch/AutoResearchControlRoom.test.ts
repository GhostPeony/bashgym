...
const controlRoomSource = readFileSync(
  new URL('./AutoResearchControlRoom.tsx', import.meta.url),
  'utf8'
)
...
test('pagination follows mutable page state, exposes safe fields, and reports retryable failures', () => {
  ...
  const artifact = {
    ...
    producer_action_id: 'action-safe',
    sha256: 'a'.repeat(64),
    size_bytes: 2048,
...
test('puts the durable baseline-versus-candidate outcome and completed loss before operational drill-down', () => {
  const outcome: CampaignOutcomeViewModel = {
    ...
    primaryMetricId: 'recall-at-10',
    baselineLabel: 'Baseline',
    candidateLabel: 'LoRA candidate',
    baselineEvaluationId: 'evaluation-baseline',
    candidateEvaluationId: 'evaluation-candidate',
    evaluationSuiteId: 'fixed64',
    ...
    buildControlRoomModel({
      snapshot: controlRoomSnapshot({
        ...
        readiness: {
          ...base.readiness,
          launch_ready: false,
          blocking_codes: ['campaign_compute_unreachable']
        }
      }),
      freshness: 'live',
      error: null
...
test('mounts durable human oversight in a collapsible row below the primary grid', () => {
  const html = renderToStaticMarkup(
    createElement(ControlRoomContent, {
      ...
      onLoadEvents: () => {},
      onLoadArtifacts: () => {},
      humanOversight: createElement(
        'section',
        { 'aria-label': 'Durable human oversight' },
        'Human queue projection'
      )
    })
  )
  assert.match(html, /Human oversight/)
...
test('keeps recovery visibility in the primary work column without replacing campaign evidence', () => {
  const html = renderToStaticMarkup(
    createElement(ControlRoomContent, {
      ...
      onLoadEvents: () => {},
      onLoadArtifacts: () => {},
      campaignRecovery: createElement(
        'section',
        { 'aria-label': 'Durable campaign recovery' },
  ...
  const errorHtml = renderToStaticMarkup(
    createElement(ControlRoomContent, {
      model: buildControlRoomModel({
        snapshot: null,
        freshness: 'error',
        error: 'The authenticated campaign service could not start.'
      }),
      campaigns: fleet,
      selectedCampaignId: 'campaign-1',
      events: [],
  ...
  const html = renderToStaticMarkup(
    createElement(ControlRoomContent, {
      model: buildControlRoomModel({
        snapshot: null,
        freshness: 'offline',
        error: 'The authenticated campaign service could not reconnect.'
      }),
      campaigns: fleet,
      selectedCampaignId: 'campaign-1',
      events: [],
...
test('mounts recovery observability for every selected campaign without a hard-coded disabled gate', () => {
  ...
  assert.match(controlRoomSource, /selectedCampaignId[\s\S]{0,300}recoverySnapshots/)
...
test('canonical campaign selection remains authoritative while preferred loading is deferred or fails', () => {
  ...
  const campaigns = [fleet[0], { ...fleet[0], campaign_id: 'campaign-b', title: 'Campaign B' }]
  assert.equal(
    shouldCanonicalizeControlRoomSelection({
