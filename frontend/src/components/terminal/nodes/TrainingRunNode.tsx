...
function statusTone(status?: string): 'neutral' | 'accent' | 'success' | 'warning' | 'error' {
  ...
  if (status === 'running' || status === 'starting' || status === 'queued' || status === 'pending')
    return 'accent'
  return 'neutral'
}
...
function draftFromTraining(node: TrainingNodeConfig, run: TrainingRun | null): TrainingDraft {
  ...
  const backend: TrainingBackend = config?.useRemoteSSH
    ...
    : config?.useNemoGym
      ? 'cloud'
      : node.backend ||
        (node.computeTarget?.startsWith('ssh:')
          ? 'remote'
          : node.computeTarget === 'cloud'
            ? 'cloud'
            : 'local')
  return {
    ...
    gradientAccumulationSteps:
      config?.gradientAccumulationSteps ?? node.gradientAccumulationSteps ?? 8,
...
async function buildTrainingContext(
  ...
  const runLossHistory = run?.metricsHistory?.length
    ? run.metricsHistory.map((point) => ({
        step: point.step,
        loss: point.loss,
        evalLoss: point.evalLoss
      }))
    : lossHistory
  ...
  lines.push('')
  lines.push(
    `- status: ${run.status}${m?.simulation ? ' (simulation mode — no real GPU training)' : ''}`
  )
  ...
  if (run.endTime) {
    lines.push(
      `- ended: ${new Date(run.endTime).toISOString()} (duration ${fmtDuration(run.endTime - run.startTime)})`
    )
  } else {
  ...
  lines.push('', '### Config')
  lines.push(
    `- epochs ${c.epochs}, batch size ${c.batchSize}, lr ${c.learningRate}, max seq ${c.maxSeqLength}`
  )
  if (c.loraRank != null) {
    lines.push(
      `- LoRA: rank ${c.loraRank}, alpha ${c.loraAlpha ?? '?'}${c.load4Bit ? ', 4-bit (QLoRA)' : ''}`
    )
  }
  if (c.strategy === 'dpo' && c.dpoBeta != null) lines.push(`- DPO beta: ${c.dpoBeta}`)
  if (c.strategy === 'grpo') {
    lines.push(
      `- GRPO: loss ${c.grpoLossType ?? 'grpo'}, generations ${c.grpoNumGenerations ?? '?'}, backend ${c.grpoBackend ?? 'auto'}${c.grpoUseVllm ? ' (vLLM)' : ''}`
    )
  }
  ...
  if (m) {
    ...
    lines.push(
      `- step ${m.step}/${m.totalSteps || '?'} (epoch ${m.epoch.toFixed(2)}${m.eta ? `, ETA ${m.eta}` : ''})`
    )
    lines.push(
      `- loss ${m.loss.toFixed(4)}${m.evalLoss != null ? `, eval loss ${m.evalLoss.toFixed(4)}` : ''} (grad norm ${m.gradNorm}, lr ${m.learningRate})`
    )
    if (runLossHistory.length > 1) {
      ...
      lines.push(
        `- loss trend: ${first.loss.toFixed(4)} @step ${first.step} → min ${min.loss.toFixed(4)} @step ${min.step} → ${last.loss.toFixed(4)} @step ${last.step}`
      )
      lines.push(
        `- last ${Math.min(10, runLossHistory.length)} losses: ${runLossHistory
          .slice(-10)
          .map((p) => p.loss.toFixed(4))
          .join(', ')}`
      )
    }
    const hw: string[] = []
    ...
    if (g) {
      ...
      if (g.reward != null)
        parts.push(
          `reward ${g.reward.toFixed(3)}${g.rewardStd != null ? ` ± ${g.rewardStd.toFixed(3)}` : ''}`
        )
  ...
  if (target?.startsWith('ssh:') || target === 'cloud') {
    lines.push(
      `- artifacts: on the ${target} host; downloaded artifacts land under ~/.bashgym/models/${run.id}/`
    )
  } else {
    lines.push(
      `- artifacts: ~/.bashgym/models/${run.id}/ (training script, metrics.jsonl, checkpoint-*/, final/)`
    )
  }
...
export const TrainingRunNode = memo(function TrainingRunNode({
  data,
  selected
}: NodeProps<TrainingRunNodeType>) {
  ...
  const run = nodeConfig.runId
    ? currentRun?.id === nodeConfig.runId
      ? currentRun
      : (runs.find((candidate) => candidate.id === nodeConfig.runId) ?? null)
    : null
  ...
  const launchDraft = async () => {
    ...
    try {
      ...
      const livePanel = useTerminalStore
        .getState()
        .panels.find((panel) => panel.id === data.panelId)
  ...
  return (
    <>
      <DataNodeShell
        ...
        statusBarClass={
          status ? (STATUS_BAR[status] ?? 'bg-background-tertiary') : 'bg-background-tertiary'
        }
        hue={hueFor('training')}
              ...
              {observedProgress?.total ? (
                <div>
                  <div className="mb-0.5 flex justify-between font-mono text-[9px] text-text-muted">
                    ...
                    <span>
                      {observedProgress.current}/{observedProgress.total}
                    </span>
                  </div>
                  <div className="h-1.5 overflow-hidden border-brutal border-border-subtle bg-background-tertiary">
                    <div
                      className="h-full bg-accent"
                      style={{ width: `${observedProgressPct}%` }}
                    />
                  </div>
                </div>
              ) : null}
              <div className="grid grid-cols-2 gap-1 text-[9px] font-mono text-text-muted">
                <span className="truncate" title={nodeConfig.baseModel}>
                  model {nodeConfig.baseModel || 'TBD'}
                </span>
                <span className="truncate" title={nodeConfig.datasetPath}>
                  data {nodeConfig.datasetPath || 'TBD'}
                </span>
                <span className="truncate" title={nodeConfig.originAgent}>
                  origin {nodeConfig.originAgent || nodeConfig.originTerminalId || 'canvas'}
                </span>
                <span className="truncate" title={nodeConfig.correlationId}>
                  intent {nodeConfig.correlationId || 'pending'}
                </span>
              </div>
            </div>
          ) : (
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-mono">
              <span
                className={clsx(
                  ...
                  (run.status === 'starting' || run.status === 'idle') &&
              ...
              <span className="text-text-secondary uppercase">
                {run.config.strategy ?? nodeConfig.strategy ?? ''}
              </span>
              {computeTarget && (
                <span
                  className="flex items-center gap-0.5 px-1 py-px border-brutal border-border-subtle rounded-brutal text-text-secondary"
              ...
              <span className="flex-1 truncate text-text-muted text-right" title={run.id}>
                {run.id}
              </span>
            </div>
      ...
      <NodeConfigModal
        ...
        footer={
          <div className="flex w-full items-center justify-between gap-3">
            <span className="min-w-0 font-mono text-[10px] text-status-error">{configError}</span>
      ...
      >
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <label className="node-field md:col-span-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, baseModel: event.target.value }))
                }
            ...
            <label className="node-field md:col-span-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, datasetPath: event.target.value }))
                }
            ...
            <label className="node-field">
              ...
              <input
                ...
                value={draft.epochs}
                onChange={(event) =>
                  setDraft((current) => ({ ...current, epochs: Number(event.target.value) }))
                }
                disabled={!canLaunch}
              />
            </label>
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, batchSize: Number(event.target.value) }))
                }
                disabled={!canLaunch}
              />
            </label>
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, learningRate: Number(event.target.value) }))
                }
                disabled={!canLaunch}
              />
            </label>
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, maxSeqLength: Number(event.target.value) }))
                }
                disabled={!canLaunch}
              />
            </label>
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, loraRank: Number(event.target.value) }))
                }
                disabled={!canLaunch}
              />
            </label>
            <label className="node-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    gradientAccumulationSteps: Number(event.target.value)
                  }))
                }
                disabled={!canLaunch}
              />
            ...
            <label className="flex items-center gap-2 border-brutal border-border-subtle bg-background-card px-3 py-2 md:col-span-2">
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, load4Bit: event.target.checked }))
                }
                disabled={!canLaunch}
              />
              <span className="font-mono text-[11px] text-text-secondary">
                Use 4-bit QLoRA loading
              </span>
            </label>
          </div>
          {!canLaunch ? (
            <p className="font-mono text-[10px] text-text-muted">
              This node is bound to a run. Use the live controls below; add a new Training node for
              another launch.
            </p>
          ) : null}
        </ConfigSection>
        ...
        <ConfigSection title="Storage & Hugging Face">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            {draft.artifactRetention === 'adapter_checkpoint' ||
            draft.artifactRetention === 'full_run' ? (
              <label className="node-field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      checkpointLimit: Number(event.target.value)
                    }))
                  }
                  disabled={!canLaunch}
                />
            ...
            {draft.artifactRetention === 'deployable' || draft.artifactRetention === 'full_run' ? (
              <label className="flex items-center gap-2 border-brutal border-border-subtle bg-background-card px-3 py-2">
                <input
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, autoExportGGUF: event.target.checked }))
                  }
                  disabled={!canLaunch}
                />
            ...
            <label className="flex items-center gap-2 border-brutal border-border-subtle bg-background-card px-3 py-2 md:col-span-2">
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, autoPushHF: event.target.checked }))
                }
                disabled={!canLaunch}
              />
              <span className="font-mono text-[11px] text-text-secondary">
              ...
              <>
                <label className="node-field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setDraft((current) => ({ ...current, hfRepoName: event.target.value }))
                    }
                ...
                <label className="flex items-center gap-2 border-brutal border-border-subtle bg-background-card px-3 py-2 md:col-span-2">
                  <input
                    ...
                    onChange={(event) =>
                      setDraft((current) => ({ ...current, hfPrivate: event.target.checked }))
                    }
                    disabled={!canLaunch}
                  />
                  <span className="font-mono text-[11px] text-text-secondary">
                    Private repository
                    {draft.hfPrivate ? '' : ' — public release requires license and data review'}
                  </span>
                </label>
              </>
            ) : null}
            <p className="font-mono text-[10px] text-text-muted md:col-span-2">
              The base model cache is shared. Merged weights and GGUF exports are new model-sized
              copies.
            </p>
          </div>
        </ConfigSection>
        ...
        <ConfigSection title="Run State">
          <div className="flex flex-wrap gap-1.5">
            ...
            <ConfigPill tone="neutral">
              {run?.config.strategy || nodeConfig.strategy || 'sft'}
            </ConfigPill>
          ...
          <ConfigRows>
            ...
            <ConfigRow
              label="Origin"
              value={
                nodeConfig.originAgent ||
                nodeConfig.originTerminalId ||
                nodeConfig.originPanelId ||
                'canvas'
              }
            />
            ...
            <ConfigRow
              ...
              value={nodeConfig.runtimeDiscovered ? 'workspace process observer' : undefined}
            />
            <ConfigRow
              label="Started"
            ...
            <ConfigRow
              label="Progress"
              value={
                m ? `${m.step}/${m.totalSteps || '?'} steps (${progress.toFixed(1)}%)` : undefined
              }
            />
          </ConfigRows>
        </ConfigSection>
        ...
        <ConfigSection title="Training Config">
          <ConfigRows>
            ...
            <ConfigRow
              label="Learning rate"
              value={run?.config.learningRate ?? nodeConfig.learningRate}
            />
            <ConfigRow
              label="Max sequence"
              value={run?.config.maxSeqLength ?? nodeConfig.maxSeqLength}
            />
        ...
        <ConfigSection title="Live Handles">
          <ConfigRows>
            <ConfigRow
              label="Status API"
              value={run ? `${API_BASE}/training/${run.id}` : `${API_BASE}/training/runs`}
            />
            <ConfigRow
              label="Metrics API"
              value={run ? `${API_BASE}/training/runs/${run.id}/metrics` : undefined}
            />
            <ConfigRow
              label="Analysis API"
              value={run ? `${API_BASE}/training/runs/${run.id}/analysis` : undefined}
            />
