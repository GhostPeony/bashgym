...
1. **S3 eval runner** — `bashgym/eval/heldout.py` is **built + tested** (`b3ef6c0`): the model-agnostic core (inject base/candidate predictors → `score_predictions` → `run_heldout_eval` → clustered CI → gate → `HeldoutReport`) is done. *Remaining = the serving seam + registry record:*
   ...
   - **Registry record needs a small schema decision (don't guess):** `bashgym/models/profile.py` has `BenchmarkResult` (has a float `metrics` dict, semantically a public benchmark) and `CustomEvalResult` (semantically a custom eval, but *no* metrics field) — neither cleanly holds a *comparative* ship-verdict (base vs candidate, `trace_delta`, CI, `ship`, reasons). Clean path: add a dedicated `heldout_evals: list[dict]` field to `ModelProfile` (+ `to_dict`/`from_dict`/`save`) and a `ModelRegistry.record_heldout_eval(model_id, HeldoutReport)` mirroring `add_benchmark_result`. Then the deploy gate queries the latest verdict to block regressions.
