...
test('load selects preferred, prior, then first and hydrates one snapshot', async () => {
  reset()
  installList(() => [
    campaign('workspace-a', 'campaign-first'),
    campaign('workspace-a', 'campaign-preferred')
  ])
...
test('compact refresh calls only the atomic snapshot endpoint', async () => {
  ...
  for (const name of forbidden) {
    campaignApi[name] = (async () => {
      throw new Error(`${name} must not hydrate compact state`)
    }) as never
  }
  campaignApi.snapshot = async (workspaceId, campaignId) => ({
    ok: true,
  ...
  await useCampaignStore.getState().load('workspace-a')
  const detail =
    useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-workspace-a']
...
test('first failure distinguishes offline from contract error', async () => {
  ...
  for (const code of [
    ...
    campaignApi.snapshot = async () => ({
      ok: false,
      code,
      error: 'AutoResearch campaign backend is unavailable.'
    })
    await useCampaignStore.getState().load('workspace-a')
    assert.equal(
      useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-workspace-a']
...
test('aggregate version and request generation guards prevent regression', async () => {
  ...
  campaignApi.snapshot = async () => ({
    ...
    data: controlRoomSnapshot({
      aggregate_version: 8,
      campaign: { ...controlRoomSnapshot().campaign, aggregate_version: 8 }
    })
  })
  await useCampaignStore.getState().refresh('workspace-a', 'campaign-1')
  let detail = useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-1']
  ...
  const slow = new Promise<ReturnType<typeof controlRoomSnapshot>>((resolve) => {
    release = resolve
  })
  let calls = 0
  campaignApi.snapshot = async () => {
    ...
    return calls === 1
      ...
      : {
          ...
          data: controlRoomSnapshot({
            ...
            campaign: { ...controlRoomSnapshot().campaign, aggregate_version: 11 }
          })
        }
  }
  ...
  release(
    controlRoomSnapshot({
      aggregate_version: 11,
      latest_event_cursor: 51,
      campaign: { ...controlRoomSnapshot().campaign, aggregate_version: 11 }
    })
  )
  await Promise.all([earlier, later])
  assert.equal(
    useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-1']?.snapshot
      ?.aggregate_version,
    11
  )
})
...
test('workspace projections stay isolated', async () => {
  ...
  await Promise.all([
    useCampaignStore.getState().load('workspace-a'),
    useCampaignStore.getState().load('workspace-b')
  ])
  assert.equal(
    useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-workspace-a']?.snapshot
      ?.workspace_id,
    'workspace-a'
  )
  assert.equal(
    useCampaignStore.getState().workspaces['workspace-b']?.details['campaign-workspace-b']?.snapshot
      ?.workspace_id,
    'workspace-b'
  )
})
...
test('event and artifact pages append with durable dedupe without changing decisions', async () => {
  ...
  const detail = useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-1']
  assert.deepEqual(
    detail?.pages.events.map((item) => item.event.event_id),
    ['event-1', 'event-2']
  )
...
test('concurrent event page loads share one request and forward each durable event once', async () => {
  ...
  assert.equal(
    useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-1']?.pages
      .eventsLoading,
    true
  )
  ...
  assert.deepEqual(
    pages?.events.map((item) => item.event.event_id),
    ['event-1']
  )
...
test('active attempt metrics load incrementally, share one request, and stay bounded', async () => {
  ...
  const first = useCampaignStore
    .getState()
    .loadAttemptMetrics('workspace-a', 'campaign-1', 'attempt-1')
  const duplicate = useCampaignStore
    .getState()
    .loadAttemptMetrics('workspace-a', 'campaign-1', 'attempt-1')
        ...
        values: [
          {
            ...
            source: 'training_metrics.jsonl',
            value: 0.26,
            observed_at: '2026-07-16T18:01:05Z'
          },
  ...
  const values =
    useCampaignStore.getState().workspaces['workspace-a']!.details['campaign-1']!.lossByAttempt[
      'attempt-1'
    ]
...
test('late metric responses cannot cross an active-attempt change', async () => {
  ...
  const pending = useCampaignStore
    .getState()
    .loadAttemptMetrics('workspace-a', 'campaign-1', 'attempt-1')
        ...
        'workspace-a': {
          ...
          details: {
            ...
            'campaign-1': {
              ...
              snapshot: {
                ...detail.snapshot!,
                active_work: { ...detail.snapshot!.active_work!, attempt_id: 'attempt-2' }
              }
            }
          }
        }
  ...
  page.resolve({
    ...
    data: {
      ...
      next_after_step: 1,
      values: [
        { step: 1, source: 'training_metrics.jsonl', value: 1, observed_at: '2026-07-16T18:00:00Z' }
      ]
    }
  })
  await pending
...
test('transition acknowledges then refreshes the snapshot', async () => {
  ...
  campaignApi.transition = async () => ({
    ok: true,
    data: { campaign: campaign('workspace-a', 'campaign-1'), event: {}, replayed: false }
  })
  await useCampaignStore.getState().load('workspace-a', 'campaign-1')
  await useCampaignStore.getState().handleSubscription('workspace-a', true, 1)
  assert.equal(
    await useCampaignStore.getState().transition('workspace-a', 'campaign-1', 'pause', 1),
    true
  )
...
test('a rejected transition reconciles the server-owned gate before allowing another attempt', async () => {
  ...
  campaignApi.transition = async () => ({
    ...
    error: 'Campaign request failed with HTTP 409'
  })
  campaignApi.snapshot = async () => {
    reconciled += 1
    return {
  ...
  assert.equal(
    await useCampaignStore.getState().transition('workspace-a', 'campaign-1', 'start', 7),
    false
  )
...
test('snapshot runtime validation rejects crossed regressions and identity/schema mismatches', async () => {
  ...
  for (const invalid of [
    controlRoomSnapshot({
      ...
      latest_event_cursor: 49,
      campaign: { ...controlRoomSnapshot().campaign, aggregate_version: 10 }
    }),
    controlRoomSnapshot({
      aggregate_version: 8,
      latest_event_cursor: 51,
    ...
    controlRoomSnapshot({
      ...
      campaign: { ...controlRoomSnapshot().campaign, campaign_id: 'campaign-b' }
    }),
    { ...controlRoomSnapshot(), schema_version: 'control_room_snapshot.v2' },
    {
      ...
      aggregate_version: 9,
      latest_event_cursor: 50,
      campaign: { campaign_id: 'campaign-1', aggregate_version: 9 },
      controller: {},
      readiness: {}
    }
  ]) {
    ...
    assert.equal(
      await useCampaignStore.getState().transition('workspace-a', 'campaign-1', 'pause', 9),
      false
    )
  }
...
test('campaign-created reloads the fleet then reconciles the exact hinted campaign', async () => {
  ...
  assert.equal(
    useCampaignStore.getState().workspaces['workspace-a']?.details['campaign-new']?.snapshot
      ?.campaign_id,
    'campaign-new'
  )
})
...
test('disconnect during retry backoff settles the old generation and a newer hint reconciles', async () => {
  reset()
  installList(() => [campaign('workspace-a', 'campaign-1')])
  const snapshotAt = (cursor: number, version: number) =>
    controlRoomSnapshot({
