...
export type HumanPromotionState =
  'blocked_by_review' | 'awaiting_human_decision' | 'promoted' | 'not_required'
...
const RUBRIC_DECISIONS: Readonly<Record<RubricChoicePublicV1['choice_id'], HumanReviewDecision>> =
  Object.freeze({
    left: 'prefer_left',
    right: 'prefer_right',
...
function parseSample(value: unknown): BlindedSamplePublicV1 | null {
  ...
  if (
    !isBoundedString(value.label, HUMAN_WORK_LIMITS.maxLabel) ||
    !isBoundedString(value.display, HUMAN_WORK_LIMITS.maxSampleDisplay)
  )
    return null
...
function parseRubricChoice(value: unknown): RubricChoicePublicV1 | null {
  if (!hasExactKeys(value, ['choice_id', 'label']) || typeof value.choice_id !== 'string')
    return null
  if (
    !reviewDecisionForChoice(value.choice_id) ||
    !isBoundedString(value.label, HUMAN_WORK_LIMITS.maxLabel)
  )
    return null
...
function parseRubric(value: unknown): VersionedRubricPublicV1 | null {
  ...
  if (
    ...
    !RUBRIC_ID.test(value.rubric_id) ||
    !isPositiveInteger(value.version)
  )
    return null
  if (
    !isBoundedString(value.instructions, HUMAN_WORK_LIMITS.maxInstructions) ||
    !Array.isArray(value.choices) ||
    value.choices.length < 1 ||
    value.choices.length > HUMAN_WORK_LIMITS.maxRubricChoices
  )
    return null
...
function parseReceipt(
  ...
  if (
    ...
    value.campaign_id !== context.campaignId ||
    value.work_id !== context.workId
  )
    return null
  if (
    value.campaign_revision !== context.campaignRevision ||
    value.item_version !== context.itemVersion ||
    value.rubric_version !== context.rubric.version
  )
    return null
  if (
    value.decision !== 'prefer_left' &&
    value.decision !== 'prefer_right' &&
    value.decision !== 'no_material_difference'
  )
    return null
  ...
  if (typeof value.receipt_digest !== 'string' || !RECEIPT_DIGEST.test(value.receipt_digest))
    return null
  return {
...
function lifecycleIsValid(item: HumanWorkItemPublicV1): boolean {
  if (item.state === 'pending')
    return (
      item.lease_expires_at === null && !item.claimed_by_current_reviewer && item.receipt === null
    )
...
function parseItem(
  ...
  if (
    ...
    return null
  if (typeof value.blocking !== 'boolean' || typeof value.claimed_by_current_reviewer !== 'boolean')
    return null
  if (value.lease_expires_at !== null && !isCanonicalUtc(value.lease_expires_at)) return null
  if (
    typeof value.claim_idempotency_key !== 'string' ||
    !IDEMPOTENCY_KEY.test(value.claim_idempotency_key)
  )
    return null
  if (
    typeof value.submit_idempotency_key !== 'string' ||
    !IDEMPOTENCY_KEY.test(value.submit_idempotency_key)
  )
    return null
  const rubric = parseRubric(value.rubric)
  if (
    !rubric ||
    !hasExactKeys(value.sample, ['prompt', 'left', 'right']) ||
    !isBoundedString(value.sample.prompt, HUMAN_WORK_LIMITS.maxPrompt)
  )
    return null
...
function parseMutation(value: unknown): HumanWorkQueuePublicV1['mutation'] | undefined {
  ...
  if (
    (value.action !== 'claim' && value.action !== 'submit' && value.action !== 'promotion') ||
    (value.state !== 'pending' && value.state !== 'conflict' && value.state !== 'error')
  )
    return undefined
...
export function parseHumanWorkQueue(value: unknown): ParsedHumanWorkQueuePublicV1 | null {
  ...
  if (
    ...
    !DURABLE_ID.test(value.workspace_id) ||
    typeof value.campaign_id !== 'string' ||
    !DURABLE_ID.test(value.campaign_id)
  )
    return null
  if (!isPositiveInteger(value.campaign_revision)) return null
  if (
    !hasExactKeys(value.reviewer, ['authenticated', 'review_capability']) ||
    typeof value.reviewer.authenticated !== 'boolean' ||
    typeof value.reviewer.review_capability !== 'boolean'
  )
    return null
  ...
  const items = value.items.map((entry) =>
    parseItem(entry, {
      ...
      campaignId: value.campaign_id as string
    })
  )
  ...
  if (
    !hasExactKeys(value.promotion, ['state', 'version', 'eligible_receipt_id', 'idempotency_key'])
  )
    return null
  const promotionRecord = value.promotion
  if (
    promotionRecord.state !== 'blocked_by_review' &&
    promotionRecord.state !== 'awaiting_human_decision' &&
    promotionRecord.state !== 'promoted' &&
    promotionRecord.state !== 'not_required'
  )
    return null
  if (
    !isPositiveInteger(promotionRecord.version) ||
    typeof promotionRecord.idempotency_key !== 'string' ||
    !IDEMPOTENCY_KEY.test(promotionRecord.idempotency_key)
  )
    return null
  if (
    promotionRecord.eligible_receipt_id !== null &&
    (typeof promotionRecord.eligible_receipt_id !== 'string' ||
      !RECEIPT_ID.test(promotionRecord.eligible_receipt_id))
  )
    return null
  ...
  if (promotionRecord.state === 'awaiting_human_decision' || promotionRecord.state === 'promoted') {
    if (
      eligibleReceipts.length !== 1 ||
      eligibleReceipts[0]!.campaign_revision !== value.campaign_revision
    )
      return null
  ...
  const result: HumanWorkQueuePublicV1 = {
    ...
    campaign_revision: value.campaign_revision,
    reviewer: {
      authenticated: value.reviewer.authenticated,
      review_capability: value.reviewer.review_capability
    },
    items,
    promotion: {
...
function effectiveState(
  ...
  if (
    ...
    Date.parse(item.lease_expires_at) <= now
  )
    return 'expired'
  return item.state
}
...
function mutationMessage(
  mutation: DeepReadonly<HumanWorkQueuePublicV1['mutation']>
): string | null {
  if (!mutation) return null
  if (mutation.state === 'pending')
    return `The ${mutation.action} request is pending. Wait for authoritative reconciliation.`
  if (mutation.state === 'conflict')
    return 'The review version changed before the request completed. Reconcile and use the current item version.'
...
export function buildHumanOversightModel(input: {
  ...
  const { queue } = input
  const scopeValid =
    input.authority.workspaceId === queue.workspace_id &&
    input.authority.campaignId === queue.campaign_id
  const timeValid = isCanonicalUtc(input.now)
  const now = timeValid ? Date.parse(input.now) : Number.NaN
  const revisionInvalid = queue.items.some(
    (entry) => entry.campaign_revision !== queue.campaign_revision
  )
  const leaseLifecycleInvalid =
    timeValid &&
    queue.items.some(
      (entry) =>
        entry.state === 'expired' &&
        entry.lease_expires_at !== null &&
        Date.parse(entry.lease_expires_at) > now
    )
  const futureReceipt =
    timeValid &&
    queue.items.some((entry) => entry.receipt && Date.parse(entry.receipt.sealed_at) > now)
  ...
  const items: HumanWorkItemViewModel[] = queue.items.map((entry) => {
    ...
    const claim: HumanWorkClaimRequest | null =
      mutationsEnabled && (state === 'pending' || state === 'expired')
        ? {
            workspaceId: queue.workspace_id,
            campaignId: queue.campaign_id,
            workId: entry.work_id,
        ...
        : null
    const submit: HumanWorkSubmitRequest | null =
      mutationsEnabled && state === 'claimed' && entry.claimed_by_current_reviewer
        ? {
            workspaceId: queue.workspace_id,
            campaignId: queue.campaign_id,
            workId: entry.work_id,
    ...
    return {
      ...
      offeredDecisions: entry.rubric.choices.map((choice) =>
        reviewDecisionForChoice(choice.choice_id)!
      ),
      receiptCurrent,
      responseBinding,
  ...
  const eligible = items.filter(
    (entry) =>
      entry.item.receipt?.receipt_id === queue.promotion.eligible_receipt_id &&
      receiptIsCurrent(entry.item, queue.campaign_revision, now, timeValid)
  )
  const receipt = eligible.length === 1 ? eligible[0]!.item.receipt : null
  const promotion: HumanPromotionRequestBinding | null =
    mutationsEnabled && queue.promotion.state === 'awaiting_human_decision' && receipt
      ? {
          workspaceId: queue.workspace_id,
          campaignId: queue.campaign_id,
  ...
  const freshnessMessage =
    input.freshness === 'live'
      ...
      : {
          reconciling:
            'Campaign state is reconciling. Wait for a live authoritative refresh before changing review work.',
          stale: 'Campaign state is stale. Reconcile the campaign before changing review work.',
          offline:
            'Campaign service is offline. Reconnect and reconcile before changing review work.',
          error:
            'Campaign state has an error. Resolve it and reconcile before changing review work.'
        }[input.freshness]
        ...
        : revisionInvalid
          ...
          : leaseLifecycleInvalid
            ...
            : futureReceipt
              ...
              : conflictMessage ||
                (!queue.reviewer.authenticated
                  ? 'Sign in with the authenticated desktop reviewer before changing review work.'
                  : null) ||
                (!queue.reviewer.review_capability
                  ? 'A human reviewer capability is required; agents cannot approve or promote human work.'
                  : null) ||
  ...
  const blocksPromotion =
    blocksComparison ||
    queue.promotion.state === 'blocked_by_review' ||
    queue.promotion.state === 'awaiting_human_decision'
  const result = {
    queue,
    scope: {
      workspaceId: queue.workspace_id,
  ...
  } as Omit<HumanOversightViewModel, typeof OVERSIGHT_MODEL_BRAND> &
    Partial<Pick<HumanOversightViewModel, typeof OVERSIGHT_MODEL_BRAND>>
