...
const TRAINING_GUIDANCE: Record<
  TrainingStrategy,
  {
    fit: string
    start: string
...
> = {
  sft: {
    ...
    start:
      'Local QLoRA: LR 2e-4 for adapters, 1-3 epochs, LoRA r=16 or 32, alpha=r or 2r, max length 2k-8k.',
    metrics:
      'Watch train/eval loss, grad norm, token count, and whether tool output or final actions are truncated.',
    caution:
      'If loss falls but held-out pass@k is flat, improve trace quality or masking before adding RL.'
  },
  dpo: {
    fit: 'Preference learning from chosen/rejected answers for the same prompt.',
    start:
      'Use beta=0.1, adapter LR around 1e-5, and enough max length to keep both preference completions intact.',
    metrics:
      'Watch chosen reward, rejected reward, reward margin, preference accuracy, and chosen/rejected logprobs.',
  ...
  grpo: {
    ...
    start:
      'Terminal RL: DAPO, group size 8-32, active sampling, zero-std filtering, token-level loss, FP32 LM head.',
    metrics:
      'Watch reward, reward_std, frac_reward_zero_std, KL, pass@1/pass@k, timeouts, tamper rate, and tool-call count.',
    caution:
      'Use RL when the base model sometimes succeeds. If every group is all pass or all fail, fix data or task difficulty first.'
  },
  distillation: {
    fit: 'Teacher-guided transfer from a larger model into a smaller student.',
    start:
      'Use this for hard tasks where pass@k is zero or when a stronger teacher can create cleaner reasoning traces.',
    metrics:
      'Watch student loss, held-out pass@k, teacher agreement, and whether distilled traces preserve safe tool use.',
  ...
  session_distillation: {
    ...
    start:
      'Use alpha 0.7, temperature 1.0, target-span-only masking, and confidence >= 0.6 for failed commands, retry loops, and recovery pivots.',
    metrics:
      'Watch masked KL, hard CE, accepted/skipped records, reader confidence, held-out decision accuracy, and pass@k on terminal tasks.',
    caution:
      'Do not use it as proof by loss alone. Promote only when held-out behavior improves against SFT or classic distillation.'
  },
  cascade: {
    fit: 'Domain-staged GRPO that trains easier terminal skills before harder multi-step tasks.',
    start:
      'Begin in simulate mode, then run short real stages with verifier-backed domains and stable GRPO settings.',
    metrics:
      'Watch per-domain pass@k, forgetting between stages, verifier errors, and final unified release-gate evidence.',
...
export function TrainingConfig({ onClose, onStart, onOpenGuides }: TrainingConfigProps) {
  ...
  const [managedPlatform, setManagedPlatform] = useState<'together' | 'openai' | 'fireworks'>(
    'together'
  )
  const [managedAccountId, setManagedAccountId] = useState('')
  const [managedMsg, setManagedMsg] = useState<string | null>(null)
  const [managedJob, setManagedJob] = useState<{
    platform: 'together' | 'openai' | 'fireworks'
    jobId: string
  } | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (trainingBackend === 'managed') {
      ...
      if (result.ok && result.data?.job_id) {
        setManagedMsg(
          `Submitted ${result.data.backend} job ${result.data.job_id} — ${result.data.status}`
        )
    ...
    >
      <div className="card bg-background-card w-[min(96vw,1180px)] max-h-[92vh] overflow-hidden">
        ...
        <form
          ...
          className="overflow-y-auto max-h-[calc(92vh-148px)] p-5 md:p-6"
        >
            ...
            <div className="space-y-5 mb-2">
                  ...
                  <p className="text-sm text-text-primary">
                    ...
                    {recs.unified_memory
                      ...
                      : recs.cuda_available
                        ? 'VRAM'
                        : '(CPU)'}{' '}
                    · recommended: {recs.recommended_quantization}, batch{' '}
                    {recs.recommended_batch_size}
                  </p>
                  {recs.warning && (
                    <p className="font-mono text-xs text-status-warning mt-1">{recs.warning}</p>
                  )}
                </div>
              )}
              ...
              <div>
                ...
                {!isExplicitBaseModel(config.baseModel) ? (
                  <p className="mt-2 font-mono text-xs text-status-warning">
                    Choose the exact trainable base. BashGym never selects a model for you.
                  </p>
                ) : null}
                {recs?.recommended_models?.length ? (
                  <p className="font-mono text-xs text-text-muted mt-2">
                    Recommended for your hardware: {recs.recommended_models.slice(0, 3).join(', ')}
                  </p>
                ) : null}
              </div>
              ...
              <p className="font-mono text-xs text-text-muted">
                Everything else uses hardware-tuned defaults (
                {recs?.recommended_quantization ?? 'QLoRA'}, batch{' '}
                {recs?.recommended_batch_size ?? 1}, local compute). Switch to Advanced to customize
                any of it.
              </p>
            </div>
          )}
          ...
          >
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  ...
                  >
                    ...
                    <span className="block font-mono text-xs mt-1 text-text-muted">
                      User-provided
                    </span>
                  </button>
                  <button
                  ...
                  <div className="card p-3 space-y-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, securityDatasetType: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, securityDatasetPath: e.target.value })
                        }
                        className="input w-full"
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              securityConversionMode: e.target.value as 'direct' | 'enriched'
                            })
                          }
                          className="input w-full"
                        >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              securityMaxSamples: e.target.value
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, securityBalanceClasses: e.target.checked })
                        }
                        className="accent-primary"
                      />
                      <span className="text-sm text-text-secondary">
                        Balance malicious/benign classes
                      </span>
                    </label>
                  </div>
                )}
                ...
                <div className="card p-3 mt-3 flex items-start gap-2 border-l-4 border-l-accent">
                  ...
                  <p className="font-mono text-xs text-text-secondary">
                    {dataSource === 'traces' &&
                      'Auto-generate training data from your collected gold traces.'}
                    {dataSource === 'dataset_path' &&
                      'Provide a pre-formatted JSONL file for training.'}
                    {dataSource === 'security_dataset' &&
                      'Ingest a public security dataset (EMBER, PhishTank, etc.) directly into training examples.'}
                  </p>
                </div>
              </div>
              ...
              {dataSource === 'traces' && (
                <div>
                      ...
                      <div className="space-y-1 max-h-32 overflow-y-auto">
                        {availableRepos.length === 0 ? (
                          <p className="text-sm text-text-muted py-2">
                            No traces collected yet. Work on some tasks first!
                          </p>
                        ) : (
                  ...
                  <div className="card p-3 mt-3 flex items-start gap-2 border-l-4 border-l-accent">
                    ...
                    <p className="font-mono text-xs text-text-secondary">
                      {trainingScope === 'all' &&
                        'Train on all collected traces for a versatile generalist agent.'}
                      {trainingScope === 'selected' &&
                        'Train on selected repos for focused expertise across projects.'}
                      {trainingScope === 'single' &&
                        'Train on a single repo for a highly specialized codebase expert.'}
                    </p>
                  </div>
                </div>
              )}
                ...
                {trainingBackend === 'managed' && (
                  <div className="mt-4 card p-3 border-l-4 border-l-accent space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setManagedPlatform(e.target.value as 'together' | 'openai' | 'fireworks')
                      }
                      className="input w-full"
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {strategies.map((s) => (
                    ...
                    >
                      <span className="block font-mono text-xs font-bold uppercase truncate">
                        {s.label}
                      </span>
                      <span className="block font-mono text-[10px] mt-0.5 text-text-muted truncate">
                        {s.description}
                      </span>
                    </button>
                  ))}
            ...
            <div className="space-y-5 xl:border-l xl:border-border xl:pl-6">
              ...
              <div>
                ...
                <p className="font-mono text-xs text-text-muted mt-2">
                  ...
                  {ollamaModels.length > 0 &&
                    ' After training, deploy to Ollama for local or private inference.'}
                </p>
              </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={
                      dataSource === 'dataset_path'
                        ? '/path/to/train.jsonl'
                        : 'Leave empty to auto-generate from traces'
                    }
                  />
                </div>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, warmupRatio: parseFloat(e.target.value) })
                      }
                      className="input w-full"
                      min={0}
                      max={1}
                    ...
                    <p className="font-mono text-xs text-text-muted mt-1">
                      Fraction of total steps for warmup (0-1)
                    </p>
                  </div>
                  <div>
                    ...
                    />
                    <p className="font-mono text-xs text-text-muted mt-1">
                      Effective batch = batch_size x this
                    </p>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, saveSteps: parseInt(e.target.value) })
                      }
                      className="input w-full"
                      min={10}
                    />
                  ...
                  <div className="md:col-span-2 2xl:col-span-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, maxSeqLength: parseInt(e.target.value) })
                      }
                      className="input w-full"
                      min={512}
                      max={8192}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, loraAlpha: parseInt(e.target.value) })
                      }
                      className="input w-full"
                      min={4}
                      max={256}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, loraDropout: parseFloat(e.target.value) })
                      }
                      className="input w-full"
                      min={0}
                      max={0.5}
                ...
                <div className="card p-4 border-l-4 border-l-accent">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, dpoBeta: parseFloat(e.target.value) })
                      }
                      className="input w-full"
                      min={0.01}
                      max={1}
                    ...
                    />
                    <p className="font-mono text-xs text-text-muted mt-1">
                      Controls how much the model can diverge from the reference. Lower = more
                      conservative.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, grpoTemperature: parseFloat(e.target.value) })
                        }
                        className="input w-full"
                        min={0.1}
                        max={2}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            dppoBinaryTvThreshold: parseFloat(e.target.value)
                          })
                        }
                        className="input w-full"
                        min={0}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({
                            ...config,
                            dppoBinaryKlThreshold: parseFloat(e.target.value)
                          })
                        }
                        className="input w-full"
                        min={0}
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, promptsPerRolloutBatch: parseInt(e.target.value) })
                        }
                        className="input w-full"
                        min={1}
                        max={128}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, maxToolCallsPerEpisode: parseInt(e.target.value) })
                        }
                        className="input w-full"
                        min={1}
                        max={512}
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, filterZeroStdGroups: e.target.checked })
                        }
                        className="accent-primary"
                      />
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, interleavedThinking: e.target.checked })
                        }
                        className="accent-primary"
                      />
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setConfig({ ...config, sftWarmStartPolicy: e.target.value })
                        }
                        className="input w-full"
                      >
                  ...
                  <div className="mt-5 pt-4 border-t border-border">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        ...
                        <p className="font-mono text-xs text-text-muted mt-1">
                          ECHO predicts terminal observations; RWML rewards accurate next-state
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({ ...config, echoAuxLambda: parseFloat(e.target.value) })
                          }
                          className="input w-full"
                          min={0}
                          step={0.01}
                        ...
                        <select
                          value={
                            ...
                            ((config.rwmlEmbeddingModel ?? '') !== '' &&
                              !RWML_EMBEDDING_MODEL_OPTIONS.some(
                                (model) => model.value === config.rwmlEmbeddingModel
                              ))
                              ? '__custom__'
                              : (config.rwmlEmbeddingModel ?? '')
                          }
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              rwmlDistanceThreshold: parseFloat(e.target.value)
                            })
                          }
                          className="input w-full"
                          min={0.01}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              rwmlEasyPassRateThreshold: parseFloat(e.target.value)
                            })
                          }
                          className="input w-full"
                          min={0}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              rwmlEasyKeepProbability: parseFloat(e.target.value)
                            })
                          }
                          className="input w-full"
                          min={0}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({ ...config, rwmlHistoryWindow: parseInt(e.target.value) })
                          }
                          className="input w-full"
                          min={0}
                          max={64}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({ ...config, rwmlKlBeta: parseFloat(e.target.value) })
                          }
                          className="input w-full"
                          min={0}
                          step={0.01}
                      ...
                      <div className="font-mono text-xs text-text-muted self-end">
                        Track ECHO loss, RWML reward/pass rate, embedding-distance distribution,
                        hard-transition coverage, and held-out terminal pass@1/pass@k.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <label className="flex items-center gap-2 mt-1">
                ...
                />
                <span className="text-sm text-text-secondary">
                  Liger fused cross-entropy (plain backend) — avoids 262k-vocab OOM for Gemma;
                ...
                <div className="card p-4 border-l-4 border-l-accent">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({ ...config, teacherTemperature: parseFloat(e.target.value) })
                          }
                          className="input w-full"
                          min={0.5}
                          max={10}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({ ...config, distillationAlpha: parseFloat(e.target.value) })
                          }
                          className="input w-full"
                          min={0}
                          max={1}
                    ...
                    <div className="card p-3 flex items-start gap-2 border-l-4 border-l-accent">
                      <Info className="w-4 h-4 text-accent flex-shrink-0 mt-0.5" />
                      <p className="font-mono text-xs text-text-secondary">
                        Knowledge distillation transfers capabilities from a large teacher model to
                        your smaller student model. The student learns to match the teacher's
                        probability distributions, not just the correct answers.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {config.strategy === 'session_distillation' && (
                <div className="card p-4 border-l-4 border-l-accent">
                  ...
                  <div className="space-y-4">
                    <p className="font-mono text-xs text-text-secondary">
                      Converts failed trace decisions into hint-injected records, then trains the
                      original context toward the same action tokens rescored under that hint.
                    </p>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              sessionDistillationAlpha: parseFloat(e.target.value)
                            })
                          }
                          className="input w-full"
                          min={0}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              sessionDistillationTemperature: parseFloat(e.target.value)
                            })
                          }
                          className="input w-full"
                          min={0.1}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              sessionDistillationMinConfidence: parseFloat(e.target.value)
                            })
                          }
                          className="input w-full"
                          min={0}
              ...
              {config.strategy === 'cascade' && (
                <div className="card p-4 border-l-4 border-l-accent">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        },
                        {
                          ...
                          desc: 'Grep, glob exploration',
                          reward: 'execution'
                        },
                        {
                          id: 'multi_step_reasoning',
                          label: 'Multi-Step Reasoning',
                          desc: 'Planning, multi-tool chains',
                          reward: 'verification'
                        }
                      ].map((domain) => (
                        <label
                          key={domain.id}
                          className="flex items-start gap-3 p-2 card cursor-pointer hover-press transition-press"
                        >
                          <input
                            type="checkbox"
                  ...
                  <div className="card p-3 flex items-start gap-2 border-l-4 border-l-accent">
                    <Info className="w-4 h-4 text-accent flex-shrink-0 mt-0.5" />
                    <p className="font-mono text-xs text-text-secondary">
                      Cascade RL trains domain-by-domain using GRPO, with each stage building on the
                      previous checkpoint. After all stages complete, use MOPD distillation to merge
                      domain experts into a unified model.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="mt-4 p-3 card border-l-4 border-l-accent space-y-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        artifactRetention: e.target.value as NonNullable<
                          TrainingConfigType['artifactRetention']
                        >
                      })
                    }
                    className="input w-full"
                  >
                    <option value="adapter_only">
                      Adapter only — smallest, recommended for experiments
                    </option>
                    ...
                    <option value="full_run">
                      Full run — adapter + merged model + checkpoint history
                    </option>
                  </select>
                  <p className="font-mono text-xs text-text-muted mt-1">
                    The base model cache is shared. Merged weights and GGUF exports create another
                    model-sized copy per run.
                  </p>
                </div>

                {(config.artifactRetention === 'adapter_checkpoint' ||
                  config.artifactRetention === 'full_run') && (
                  <div>
                    <label className="block font-mono text-xs text-text-muted mb-2">
                      Checkpoint limit
                    </label>
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, checkpointLimit: parseInt(e.target.value) })
                      }
                      className="input w-full"
                    />
                  </div>
                ...
                {(config.artifactRetention === 'deployable' ||
                  config.artifactRetention === 'full_run') && (
                  <div className="space-y-2">
                    <label className="flex items-center gap-2">
                      <input
                      ...
                      />
                      <span className="text-sm text-text-secondary">
                        Also export a GGUF deployment copy
                      </span>
                    </label>
              ...
              <div className="mt-4 p-3 card border-l-4 border-l-accent">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setConfig({
                        ...
                        ...(e.target.checked
                          ? { artifactRetention: 'deployable', autoExportGGUF: true }
                          : {})
                      })
                    }
                    className="accent-primary"
                  />
                  <span className="text-sm text-text-secondary">
                    Auto-deploy to Ollama after training
                  </span>
                </div>
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setConfig({
                          ...config,
                          hfUploadArtifact: e.target.value as NonNullable<
                            TrainingConfigType['hfUploadArtifact']
                          >
                        })
                      }
                      className="input w-full"
                    >
                      <option value="auto">
                        Upload deployable model when present, otherwise adapter
                      </option>
                    ...
                    {config.hfPrivate === false && (
                      <p className="font-mono text-xs text-status-warning">
                        Public upload: verify the base-model license, dataset provenance, and
                        generated model card first.
                      </p>
                    )}
                  </div>
                )}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
          ...
          <button
            ...
            disabled={!isExplicitBaseModel(config.baseModel)}
          >
            Start Training
          </button>
        </div>
