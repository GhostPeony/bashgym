...
export function createCampaignFreshness(input: {
  ...
  return {
    freshness: input.connected ? (input.hasSnapshot ? 'stale' : 'reconciling') : 'offline',
    generation: 0,
...
export function expireCampaignCursor(
  state: CampaignReconciliationState
): CampaignReconciliationState {
  return {
    ...state,
...
export function finishCampaignReconciliation(
  ...
  const covered =
    result.eventCursor >= state.targetCursor && result.aggregateVersion >= state.targetVersion
  return {
    ...state,
...
function isIdentifierArray(value: unknown, maxLength: number): value is string[] {
  return Array.isArray(value) && value.length <= maxLength && value.every(isCampaignIdentifier)
}
...
function isDecisionBlocker(value: unknown): boolean {
  if (
    !isPlainRecord(value) ||
    !hasExactKeys(value, ['schema_version', 'code', 'summary', 'evidence_ids', 'secondary_codes'])
  )
    return false
  return (
    value.schema_version === 'decision_blocker.v1' &&
    ...
    new Set(value.secondary_codes as string[]).size ===
      (value.secondary_codes as string[]).length &&
...
function isCampaignSummary(value: unknown, campaignId: string, aggregateVersion: number): boolean {
  if (
    ...
    return false
  return (
    value.schema_version === 'campaign_summary.v1' &&
...
function isControllerObservation(value: unknown): boolean {
  if (
    ...
    return false
  return (
    value.schema_version === 'controller_observation.v1' &&
...
function isReadinessSummary(value: unknown): boolean {
  if (
    ...
    return false
  return (
    value.schema_version === 'readiness_summary.v1' &&
...
function isSafeBinding(value: unknown): boolean {
  ...
  if (
    !isPlainRecord(value) ||
    !hasExactKeys(value, ['schema_version', 'binding_id', 'immutable_digest', 'display_label'])
  )
    return false
  return (
    value.schema_version === 'safe_binding_identity.v1' &&
...
function isBindingSummary(value: unknown): boolean {
  if (
    !isPlainRecord(value) ||
    !hasExactKeys(value, ['schema_version', 'model', 'data', 'evaluator', 'source', 'compute'])
  )
    return false
  return (
    value.schema_version === 'binding_summary.v1' &&
...
function isJourneyPhase(value: unknown): boolean {
  if (
    ...
    return false
  return (
    value.schema_version === 'journey_phase_summary.v1' &&
    ['setup', 'baseline', 'experiments', 'human_review', 'decision'].includes(
      String(value.phase_id)
    ) &&
    ['not_started', 'ready', 'active', 'blocked', 'complete', 'failed', 'skipped'].includes(
      String(value.state)
    ) &&
...
function isProcessIdentity(value: unknown): boolean {
  if (
    !isPlainRecord(value) ||
    !hasExactKeys(value, ['schema_version', 'run_id', 'compute_profile_id', 'state'])
  )
    return false
  return (
    value.schema_version === 'opaque_process_identity.v1' &&
    isCampaignIdentifier(value.run_id) &&
    isCampaignIdentifier(value.compute_profile_id) &&
    ['launching', 'running', 'completed', 'failed', 'cancelled', 'unknown'].includes(
      String(value.state)
    )
  )
}
...
function isActiveWork(value: unknown): boolean {
  ...
  return (
    value.schema_version === 'active_work_summary.v1' &&
    ['study_id', 'proposal_id', 'action_id', 'attempt_id'].every((key) =>
      isNullableIdentifier(value[key])
    ) &&
...
function isCandidate(value: unknown): boolean {
  ...
  if (
    ...
    return false
  return (
    value.schema_version === 'candidate_summary.v1' &&
    ...
    (value.comparison_verdict === null ||
      ['passed', 'failed', 'insufficient_evidence'].includes(String(value.comparison_verdict))) &&
...
function isMetric(value: unknown): boolean {
  if (
    ...
    return false
  return (
    value.schema_version === 'metric_descriptor.v1' &&
...
function isBudget(value: unknown): boolean {
  if (!isPlainRecord(value) || !hasExactKeys(value, ['schema_version', 'resources', 'blocked']))
    return false
  if (
    value.schema_version !== 'budget_summary.v1' ||
  ...
  return value.resources.every((resource) => {
    if (
      ...
      return false
    return (
      resource.schema_version === 'budget_resource_summary.v1' &&
...
function isHumanWorkSummary(value: unknown): boolean {
  if (
    !isPlainRecord(value) ||
    !hasExactKeys(value, ['schema_version', 'blocking_count', 'open_count', 'newest'])
  )
    return false
  if (
    value.schema_version !== 'human_work_summary.v1' ||
    !isSafeInteger(value.blocking_count) ||
    !isSafeInteger(value.open_count) ||
    !Array.isArray(value.newest) ||
    value.newest.length > 10
  )
    return false
  return value.newest.every((item) => {
    if (
      ...
      return false
    return (
      item.schema_version === 'human_work_item_summary.v1' &&
...
function isAgentSummary(value: unknown): boolean {
  if (
    ...
    return false
  return (
    value.schema_version === 'attached_agent_summary.v1' &&
    ['session_id', 'actor_id', 'origin_id', 'bundle_id'].every((key) =>
      isCampaignIdentifier(value[key])
    ) &&
...
function isCollectionCursor(value: unknown): boolean {
  if (
    !isPlainRecord(value) ||
    !hasExactKeys(value, ['schema_version', 'count', 'next_cursor', 'has_more'])
  )
    return false
  return (
    value.schema_version === 'collection_cursor.v1' &&
...
function isCollectionSummary(value: unknown): boolean {
  ...
  return (
    value.schema_version === 'collection_summary.v1' &&
    keys.every((key) => isCollectionCursor(value[key]))
  )
}
...
function isDecisionSurface(value: unknown): boolean {
  if (
    ...
    return false
  if (
    value.schema_version !== 'decision_surface.v1' ||
    !['bashgym', 'human', 'none'].includes(String(value.execution_owner)) ||
    !['bashgym', 'agent', 'human', 'none'].includes(String(value.attention_owner)) ||
    ...
    return false
  if (
    new Set(value.recovery_actions as string[]).size !== (value.recovery_actions as string[]).length
  )
    return false
  if (
    !value.next_actions.every((action) => {
      if (
        ...
        return false
      return (
        action.schema_version === 'decision_action.v1' &&
    ...
    return false
  return (
    new Set((value.next_actions as Record<string, unknown>[]).map((action) => action.action))
      .size === value.next_actions.length
  )
}
...
export function parseCampaignHint(value: unknown): CampaignHintV1 | null {
  ...
  if (!isCampaignIdentifier(value.workspace_id) || !isCampaignIdentifier(value.campaign_id))
    return null
  if (!Number.isSafeInteger(value.event_cursor) || Number(value.event_cursor) < 1) return null
  if (!Number.isSafeInteger(value.aggregate_version) || Number(value.aggregate_version) < 1)
    return null
...
export function validateCampaignSnapshot(
  ...
  if (
    ...
    value.schema_version !== 'control_room_snapshot.v1'
  )
    return null
  ...
  if (!Number.isSafeInteger(value.aggregate_version) || Number(value.aggregate_version) < 1)
    return null
  if (!isSafeInteger(value.manifest_revision, 1) || !isSafeInteger(value.authorization_revision, 1))
    return null
  if (!isCanonicalUtcTimestamp(value.snapshot_at)) return null
  if (!Number.isSafeInteger(value.latest_event_cursor) || Number(value.latest_event_cursor) < 0)
    return null
  if (!isCampaignSummary(value.campaign, campaignId, Number(value.aggregate_version))) return null
  if (!isControllerObservation(value.controller) || !isReadinessSummary(value.readiness))
    return null
  if (!isBindingSummary(value.bindings)) return null
  if (
    !Array.isArray(value.journey) ||
    value.journey.length !== 5 ||
    !value.journey.every(isJourneyPhase)
  )
    return null
  if (
    value.journey.map((phase) => (phase as Record<string, unknown>).phase_id).join('|') !==
    'setup|baseline|experiments|human_review|decision'
  )
    return null
  if ((value.campaign as Record<string, unknown>).manifest_revision !== value.manifest_revision)
    return null
  if (
    !isActiveWork(value.active_work) ||
    !isCandidate(value.champion) ||
    !isCandidate(value.candidate)
  )
    return null
  if (!Array.isArray(value.metrics) || value.metrics.length > 64 || !value.metrics.every(isMetric))
    return null
  ...
  if (
    ...
    return null
  if (!isCollectionSummary(value.collections) || !isDecisionSurface(value.decision_surface))
    return null
