...
test('campaign API sends only fully qualified allowlisted bridge routes', async () => {
  ...
  try {
    ...
    await campaignApi.claimHumanWork({
      ...
      expectedCampaignRevision: 1, expectedVersion: 1, expectedState: 'pending',
      idempotencyKey: 'idem_0123456789abcdef',
    })
    await campaignApi.submitHumanWork({
      workspaceId: 'workspace-a', campaignId: 'campaign-1', workId: 'hw_0123456789abcdef',
      expectedCampaignRevision: 1, expectedVersion: 2, expectedRubricVersion: 1,
      expectedState: 'claimed', idempotencyKey: 'idem_abcdef0123456789',
    }, 'prefer_left', 'Rubric evidence.')
    await campaignApi.decideHumanPromotion({
      workspaceId: 'workspace-a', campaignId: 'campaign-1', receiptId: 'hrc_0123456789abcdef',
      workId: 'hw_0123456789abcdef', expectedCampaignRevision: 1, expectedItemVersion: 3,
      expectedRubricVersion: 1, expectedPromotionVersion: 2,
      expectedPromotionState: 'awaiting_human_decision', idempotencyKey: 'idem_1111111111111111',
    }, 'hold')
    await campaignApi.recovery('workspace-a', 'campaign-1')
    await campaignApi.requestRecovery({
      action: 'repair', idempotencyKey: `idem_${'a'.repeat(32)}`,
      workspaceId: 'workspace-a', campaignId: 'campaign-1',
      eligibilityReceiptId: `rcpt_${'b'.repeat(32)}`,
      doctorEvidenceId: `evd_${'c'.repeat(32)}`,
      expectedCampaignRevision: 2, expectedEventCursor: 17,
      expectedAggregateVersion: 9, expectedControllerLeaseId: null,
      checkpointId: `ckpt_${'d'.repeat(32)}`, artifactId: `art_${'e'.repeat(32)}`,
      humanConfirmed: true,
    })
  } finally {
...
test('campaign agent view uses the authenticated bridge and rejects an invalid public projection', async () => {
  ...
  globals.window = { bashgym: { campaignRequest: async (...args: unknown[]) => {
    calls.push(args)
  ...
  try {
    ...
    assert.deepEqual(calls[0], [
      'GET', '/api/campaigns/campaign-1/agent-attachment', undefined,
      { workspace_id: 'workspace-a', after_sequence: 0, limit: 20 }, undefined,
    ])

    globals.window = { bashgym: { campaignRequest: async () => ({
      ok: true, status: 200, data: { ...payload, raw_token: `bgag.${'a'.repeat(40)}` },
    }) } }
...
test('guided setup API uses exact authenticated routes, stable idempotency, and strict context parsing', async () => {
  ...
  const globals = globalThis as unknown as Record<string, unknown>
  const priorWindow = globals.window
  const required = { model: 'model.registered', data: 'data.registered', compute: 'compute.private', evaluation: 'evaluation.registered' }
  const context = {
    schema_version: 'guided_setup_context.v1', workspace_id: 'workspace-a',
    templates: [{ schema_version: 'guided_setup_template.v1', template_id: 'template-modern', definition_digest: 'a'.repeat(64), quality_claim_eligible: true, required_bindings: required }],
    installations: [], session: null, reason_codes: ['setup_session_not_started'],
    truncation: { truncated: false, reason_codes: [], limits: { templates: 32, installations: 32, bindings_per_kind: 32 } },
  }
  globals.window = {
    bashgym: {
      campaignRequest: async (...args: unknown[]) => {
  ...
  try {
    const loaded = await campaignApi.guidedSetupContext('workspace-a', `setupsess_${'a'.repeat(32)}`)
    assert.equal(loaded.ok, true)
    assert.equal(loaded.data?.workspace_id, 'workspace-a')
    assert.deepEqual(calls[0], [
      'GET', '/api/campaigns/setup/context', undefined,
      { workspace_id: 'workspace-a', session_id: `setupsess_${'a'.repeat(32)}` }, undefined,
    ])

    await campaignApi.advanceGuidedSetupSession({
      workspaceId: 'workspace-a', sessionId: `setupsess_${'a'.repeat(32)}`,
      expectedVersion: 0, step: 'template', selectionId: 'template-modern',
    ...
    globals.window = { bashgym: { campaignRequest: async () => ({ ok: true, status: 200, data: { ...context, private_path: 'C:/secret' } }) } }
...
test('guided setup creation returns only the safe campaign handoff', async () => {
  ...
  globals.window = { bashgym: { campaignRequest: async (...args: unknown[]) => {
    calls.push(args)
    return { ok: true, status: 200, data: {
      campaign: { workspace_id: 'workspace-a', campaign_id: 'campaign-new', title: 'New campaign', status: 'ready' },
      event: { event_id: 'event-new' }, replayed: false,
      setup: { schema_version: 'guided_setup_creation.v1', validation_receipt_id: `setuprcpt_${'a'.repeat(32)}`,
        binding_references: { model: 'model.registered', data: 'data.registered', compute: 'compute.private', evaluation: 'evaluation.registered' } },
    } }
  } } }
  try {
    const response = await campaignApi.createGuidedSetup({
      workspaceId: 'workspace-a', campaignId: 'campaign-new', title: 'New campaign',
      validationReceiptId: `setuprcpt_${'a'.repeat(32)}`, idempotencyKey: `idem_${'b'.repeat(32)}`,
    })
    assert.equal(response.ok, true)
    assert.equal(response.data?.campaign_id, 'campaign-new')
    assert.deepEqual(calls[0], [
      'POST', '/api/campaigns/setup/create', {
