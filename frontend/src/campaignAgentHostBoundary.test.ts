...
const campaignAgentRoutesPath = fileURLToPath(
  new URL('../../bashgym/api/campaign_agent_routes.py', import.meta.url)
)
const campaignAgentServicePath = fileURLToPath(
  new URL('../../bashgym/campaigns/campaign_agents.py', import.meta.url)
)
const campaignClientPath = fileURLToPath(
  new URL('../../bashgym/campaigns/client.py', import.meta.url)
)
const campaignMcpPath = fileURLToPath(
  new URL('../../bashgym/mcp/campaign_server.py', import.meta.url)
)
...
test('main reads only attested PTY identity state and tears authority down on every lifecycle boundary', () => {
  ...
  assert.match(
    main.slice(revokeStart, revokeEnd),
    /revokeCampaignAgentTerminal\(terminalId, 'explicit_revoke'\)/
  )
  assert.doesNotMatch(
    main,
    /pty\.write\([^)]*bgag|process\.argv[^\n]*bgag|writeFileSync\([^\n]*bgag/
  )
})
...
test('renderer-triggered command preparation cannot attest a campaign agent identity', () => {
  ...
  assert.doesNotMatch(
    prepareLaunchHandler,
    /campaignAgentIdentity|bindMainOwnedCampaignAgentIdentity/
  )
...
test('desktop launch is a main-owned direct Codex PTY behind a fixed read-only MCP proxy', () => {
  ...
  assert.match(
    main,
    /artifacts:\s*\(args\)\s*=>\s*campaignAgentHostController\.artifacts\(terminalId, args\)/
  )
...
test('renderer launch contract exposes only campaign scope and a public terminal result', () => {
  assert.match(
    preload,
    /launch:\s*\(request\)\s*=>\s*ipcRenderer\.invoke\(['"]campaign-agent-host:launch['"],\s*request\)/
  )
  ...
  assert.match(
    launchHandler,
    /launchedTerminalId[\s\S]*?closeCampaignAgentMcpHost\(launchedTerminalId\)/
  )
  assert.doesNotMatch(
    launchHandler,
...
test('eligible campaign-agent sessions are filtered to the selected campaign scope', () => {
  assert.match(
    preload,
    /eligible:\s*\(request\)\s*=>\s*ipcRenderer\.invoke\(['"]campaign-agent-host:eligible['"],\s*request\)/
  )
...
test('main binds heartbeat and fixed actions while lifecycle closure locks the MCP host', () => {
  assert.match(
    main,
    /heartbeat:\s*\(credential, body\)\s*=>\s*campaignBridgeClient\.heartbeatCampaignAgent\(credential, body\)/
  )
  assert.match(
    main,
    /observe:\s*\(credential\)\s*=>\s*campaignBridgeClient\.observeCampaignAsAgent\(credential\)/
  )
  assert.match(
    main,
    /artifacts:\s*\(credential, query\)\s*=>\s*campaignBridgeClient\.listCampaignArtifactsAsAgent\(credential, query\)/
  )
...
test('bgag authorization is limited to fixed, read-only campaign-agent action adapters', () => {
  ...
  const credentialRoutes = [
    ...campaignAgentRoutes.matchAll(/@campaign_agent_credential_router\.(?:get|post)\("([^"]+)"/g)
  ].map((match) => match[1])
