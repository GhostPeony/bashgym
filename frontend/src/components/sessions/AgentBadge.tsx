...
import { kindChipClass } from './kindStyles'

/* Brand marks from the CC0-licensed simple-icons set (24x24 viewBox),
   used nominatively to identify which CLI drives a session. */
...
interface AgentBadgeProps {
  ...
  /** Icon size in tailwind units (default w-3 h-3) */
  size?: 'sm' | 'md'
  className?: string
}
...
export function AgentBadge({ kind, size = 'sm', className }: AgentBadgeProps) {
  ...
  return (
    <span
      className={clsx(
        'inline-flex items-center justify-center flex-shrink-0 p-[3px] border-brutal rounded-brutal',
        kindChipClass(kind),
        className
      )}
      title={label}
    >
      {kind === 'claude' ? (
        <AnthropicLogo className={iconClass} />
      ) : kind === 'codex' ? (
